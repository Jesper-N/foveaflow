import type { en } from "./en";

export const bn = {
  "1 minute Lilac Chaser: hold center focus and notice peripheral change.":
    "১ মিনিট লাইলাক চেজার: মাঝখানে ফোকাস ধরে রাখুন, চারপাশের বদল খেয়াল করুন।",
  "1 minute Multiple Distractions: stay locked on the brightest target.":
    "১ মিনিট একাধিক বিভ্রান্তি: সবচেয়ে উজ্জ্বল লক্ষ্যে চোখ রাখুন।",
  "2 minutes Reaction Jumps: find each new target position quickly.":
    "২ মিনিট দ্রুত ফোকাস বদল: প্রতিটি নতুন লক্ষ্য দ্রুত খুঁজুন।",
  "3 minutes Smooth Pursuit: follow the target smoothly.":
    "৩ মিনিট মসৃণ অনুসরণ: লক্ষ্যটিকে মসৃণভাবে অনুসরণ করুন।",
  "7-minute FPS eye warmup": "৭ মিনিটের FPS চোখের ওয়ার্মআপ",
  "About FoveaFlow": "FoveaFlow সম্পর্কে",
  "About FoveaFlow eye trainer": "FoveaFlow চোখের অনুশীলন টুল সম্পর্কে",
  "Acceptable use": "গ্রহণযোগ্য ব্যবহার",
  "Adjust the settings without guesswork": "অনুমান না করে সেটিং বদলান",
  Agreement: "সম্মতি",
  "Alternating pace": "পাল্টানো গতি",
  Alternative: "বিকল্প",
  App: "অ্যাপ",
  "App actions": "অ্যাপের কাজ",
  "Availability and warranty": "ব্যবহারযোগ্যতা ও ওয়ারেন্টি",
  "Ball color": "বলের রং",
  "Best fit": "সবচেয়ে মানানসই",
  "Best uses for": "যে কাজে সবচেয়ে ভালো",
  BlinkCamp: "BlinkCamp",
  "BlinkCamp has its own exercise set and a simpler public workflow.":
    "BlinkCamp-এর নিজস্ব অনুশীলন সেট আছে, আর পাবলিক ব্যবহারপদ্ধতি সহজ।",
  "BlinkCamp is more focused on straightforward eye-training routines than distractor and letter-overlay customization.":
    "BlinkCamp বিভ্রান্তিকারী ও অক্ষর ওভারলে বদলানোর চেয়ে সরাসরি চোখের অনুশীলন রুটিনে বেশি জোর দেয়।",
  "BlinkCamp is organized around its own routine interface.":
    "BlinkCamp নিজস্ব রুটিন ইন্টারফেস ঘিরে সাজানো।",
  "BlinkCamp keeps setup lighter and does not expose the same viewing-distance and screen-scale controls.":
    "BlinkCamp সেটআপ হালকা রাখে এবং একই ধরনের দেখার দূরত্ব ও স্ক্রিন স্কেল কন্ট্রোল দেখায় না।",
  "BlinkCamp keeps the public controls simpler, with the main visible controls focused on speed and size.":
    "BlinkCamp পাবলিক কন্ট্রোলগুলো সহজ রাখে; দেখা যায় মূলত গতি ও সাইজের কন্ট্রোল।",
  "BlinkCamp uses a simpler routine-based setup rather than exposing the same path and motion-behavior controls.":
    "BlinkCamp একই পথ ও চলন-আচরণের কন্ট্রোল না দিয়ে সহজ রুটিন-ভিত্তিক সেটআপ ব্যবহার করে।",
  Blue: "নীল",
  Bold: "বোল্ড",
  Bounce: "বাউন্স",
  "Bounce Smooth Pursuit Drill": "বাউন্স মসৃণ অনুসরণ ড্রিল",
  "Bounce Smooth Pursuit Eye Training": "বাউন্স মসৃণ অনুসরণ চোখের অনুশীলন",
  "Bounce adds repeated reversals at the edges. It is useful when you want more direction changes and less continuous flow than Circle or Wave.":
    "বাউন্স প্রান্তে বারবার দিক বদলায়। বৃত্ত বা ঢেউ-এর চেয়ে বেশি দিক বদল এবং কম টানা প্রবাহ চাইলে এটি কাজে লাগে।",
  "Browser only, no account": "শুধু ব্রাউজার, অ্যাকাউন্ট লাগবে না",
  "Build and reset": "গড়ে ওঠা ও রিসেট",
  "By using FoveaFlow, you agree to these terms. If you do not agree, do not use the site.":
    "FoveaFlow ব্যবহার করলে আপনি এই শর্তগুলো মেনে নিচ্ছেন। একমত না হলে সাইট ব্যবহার করবেন না।",
  "CSS pixels/cm": "CSS পিক্সেল/সেমি",
  "Can FoveaFlow help with tired eyes from screen work?":
    "স্ক্রিনে কাজ করে চোখ ক্লান্ত হলে FoveaFlow কি সাহায্য করতে পারে?",
  "Can FoveaFlow improve eyesight or reaction time?":
    "FoveaFlow কি দৃষ্টিশক্তি বা প্রতিক্রিয়ার সময় উন্নত করতে পারে?",
  "Can I adjust speed and target size?":
    "আমি কি গতি ও লক্ষ্যের সাইজ বদলাতে পারি?",
  "Can I calibrate the session to my setup?":
    "আমি কি নিজের সেটআপ অনুযায়ী সেশন ক্যালিব্রেট করতে পারি?",
  "Can I change how the target looks?":
    "লক্ষ্যটি দেখতে কেমন হবে, তা কি বদলাতে পারি?",
  "Can I change paths and motion behavior?":
    "আমি কি পথ ও চলনের ধরন বদলাতে পারি?",
  "Can I link straight to a drill?":
    "আমি কি সরাসরি কোনো ড্রিলের লিংক দিতে পারি?",
  "Can I start in the browser?": "আমি কি ব্রাউজারেই শুরু করতে পারি?",
  "Can I train with distractors or letters?":
    "আমি কি বিভ্রান্তিকারী বা অক্ষর দিয়ে অনুশীলন করতে পারি?",
  "Can I tune target appearance and calibration?":
    "লক্ষ্যের চেহারা ও ক্যালিব্রেশন কি বদলাতে পারি?",
  "Can I use FoveaFlow on a phone?": "ফোনে কি FoveaFlow ব্যবহার করতে পারি?",
  "Can I use it free in the browser?":
    "ব্রাউজারে কি এটি বিনা খরচে ব্যবহার করতে পারি?",
  "Change language": "ভাষা বদলান",
  "Change speed and target size first. They usually have the biggest effect on difficulty and control.":
    "প্রথমে গতি ও লক্ষ্যের সাইজ বদলান। সাধারণত কঠিনতা ও নিয়ন্ত্রণে এগুলোর প্রভাব সবচেয়ে বেশি।",
  "Change your saved FoveaFlow settings.": "সংরক্ষিত FoveaFlow সেটিং বদলান।",
  "Changes to these terms": "এই শর্তের পরিবর্তন",
  Children: "শিশু",
  "Choose BlinkCamp if you want a simple browser tool centered on basic eye-training routines and speed-and-size adjustment.":
    "মৌলিক চোখের অনুশীলন রুটিন এবং গতি-সাইজ বদলানোর সহজ ব্রাউজার টুল চাইলে BlinkCamp বেছে নিন।",
  "Choose FoveaFlow if you want a lightweight web app with direct access to visual tracking, reaction, distractor, and fixation drills.":
    "ভিজ্যুয়াল ট্র্যাকিং, প্রতিক্রিয়া, বিভ্রান্তিকারী ও ফিক্সেশন ড্রিলে সরাসরি যেতে চাইলে হালকা ওয়েব অ্যাপ হিসেবে FoveaFlow বেছে নিন।",
  "Choose FoveaFlow when you want direct drill links, adjustable target behavior, and a clean full-screen canvas for short visual tracking sessions.":
    "সরাসরি ড্রিল লিংক, বদলানো যায় এমন লক্ষ্য আচরণ এবং ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য পরিষ্কার ফুল-স্ক্রিন ক্যানভাস চাইলে FoveaFlow বেছে নিন।",
  "Choose Reaction Jumps when you want discrete target changes rather than continuous motion.":
    "টানা চলনের বদলে আলাদা আলাদা লক্ষ্য বদল চাইলে দ্রুত ফোকাস বদল বেছে নিন।",
  "Choose a drill by the result you want":
    "আপনি যে ফল চান, সে অনুযায়ী ড্রিল বেছে নিন",
  "Choose a path, set the speed and target style, then use it for a short visual tracking session.":
    "একটি পথ বেছে নিন, গতি ও লক্ষ্যের ধরন সেট করুন, তারপর ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনে ব্যবহার করুন।",
  "Choose an easy path when you want rhythm and control. Choose a harder path when you want more direction changes and more target-search demand.":
    "ছন্দ ও নিয়ন্ত্রণ চাইলে সহজ পথ নিন। বেশি দিক বদল ও লক্ষ্য খোঁজার চাপ চাইলে কঠিন পথ নিন।",
  Circle: "বৃত্ত",
  "Circle Smooth Pursuit Drill": "বৃত্ত মসৃণ অনুসরণ ড্রিল",
  "Circle Smooth Pursuit Eye Training": "বৃত্ত মসৃণ অনুসরণ চোখের অনুশীলন",
  "Circle is the easiest repeating Smooth Pursuit pattern and the best starting point for most users. The loop is predictable, which makes it useful for warmups and speed changes.":
    "বৃত্ত সবচেয়ে সহজ পুনরাবৃত্ত মসৃণ অনুসরণ প্যাটার্ন এবং বেশির ভাগ ব্যবহারকারীর জন্য ভালো শুরু। লুপটি আগে থেকে বোঝা যায়, তাই ওয়ার্মআপ ও গতি বদলের জন্য কাজে লাগে।",
  Close: "বন্ধ করুন",
  "Cloudflare Web Analytics": "Cloudflare Web Analytics",
  "Cloudflare Web Analytics may load a small beacon script. Cloudflare says Web Analytics measures page views and performance without collecting or using visitor personal data.":
    "Cloudflare Web Analytics একটি ছোট beacon স্ক্রিপ্ট লোড করতে পারে। Cloudflare বলছে, Web Analytics ভিজিটরের ব্যক্তিগত ডেটা সংগ্রহ বা ব্যবহার না করে পেজ ভিউ ও পারফরম্যান্স মাপে।",
  "Cloudflare cookie reference": "Cloudflare কুকি রেফারেন্স",
  "Cloudflare data collection docs": "Cloudflare ডেটা সংগ্রহ ডকুমেন্ট",
  "Cloudflare hosting and analytics": "Cloudflare হোস্টিং ও অ্যানালিটিক্স",
  Clover: "ক্লোভার",
  "Clover Smooth Pursuit Drill": "ক্লোভার মসৃণ অনুসরণ ড্রিল",
  "Clover Smooth Pursuit Eye Training": "ক্লোভার মসৃণ অনুসরণ চোখের অনুশীলন",
  "Clover creates repeated looping lobes for continuous motion with more shape variation than Circle.":
    "ক্লোভার টানা চলনের জন্য বারবার লুপ করা লোব তৈরি করে; এতে বৃত্ত-এর চেয়ে আকৃতির বদল বেশি।",
  "Compare FoveaFlow and BlinkCamp for free browser-based eye training, visual tracking, FPS warmups, reaction jumps, and distractor tracking.":
    "বিনা খরচে ব্রাউজার-ভিত্তিক চোখের অনুশীলন, ভিজ্যুয়াল ট্র্যাকিং, FPS ওয়ার্মআপ, দ্রুত ফোকাস বদল ও বিভ্রান্তিকারী ট্র্যাকিং-এর জন্য FoveaFlow এবং BlinkCamp তুলনা করুন।",
  "Compare FoveaFlow and EyeTrainer.gg for FPS eye training, visual tracking, reaction drills, distraction control, and browser-based warmups.":
    "FPS চোখের অনুশীলন, ভিজ্যুয়াল ট্র্যাকিং, প্রতিক্রিয়া ড্রিল, বিভ্রান্তি নিয়ন্ত্রণ ও ব্রাউজার-ভিত্তিক ওয়ার্মআপের জন্য FoveaFlow এবং EyeTrainer.gg তুলনা করুন।",
  "Compare all drills": "সব ড্রিল তুলনা করুন",
  "Compare drills": "ড্রিল তুলনা করুন",
  Comparison: "তুলনা",
  Contact: "যোগাযোগ",
  "Control sections": "কন্ট্রোল বিভাগ",
  Controls: "কন্ট্রোল",
  "Controls for speed units, target size, shape, color, opacity, trail length, motion path, motion behavior, distractor count, distractor brightness, letter overlays, viewing distance, and screen scale.":
    "গতির একক, লক্ষ্যের সাইজ, আকৃতি, রং, অস্বচ্ছতা, ট্রেইলের দৈর্ঘ্য, চলার পথ, চলনের ধরন, বিভ্রান্তিকারীর সংখ্যা, বিভ্রান্তিকারীর উজ্জ্বলতা, অক্ষর ওভারলে, দেখার দূরত্ব ও স্ক্রিন স্কেলের কন্ট্রোল।",
  "Controls for speed units, target size, shape, color, opacity, trail length, motion path, motion behavior, distractors, letter overlays, viewing distance, screen scale, and mode-specific options such as Lilac Chaser ball scale.":
    "গতির একক, লক্ষ্যের সাইজ, আকৃতি, রং, অস্বচ্ছতা, ট্রেইলের দৈর্ঘ্য, চলার পথ, চলনের ধরন, বিভ্রান্তিকারী, অক্ষর ওভারলে, দেখার দূরত্ব, স্ক্রিন স্কেল এবং লাইলাক চেজার বল স্কেলের মতো মোডভিত্তিক অপশনের কন্ট্রোল।",
  Cookies: "কুকি",
  "Corner Tour Smooth Pursuit Drill": "চার কোণ ঘোরা মসৃণ অনুসরণ ড্রিল",
  "Corner Tour Smooth Pursuit Eye Training":
    "চার কোণ ঘোরা মসৃণ অনুসরণ চোখের অনুশীলন",
  "Corner Tour gives each corner of the display a deliberate role, making the route spacious and structured.":
    "চার কোণ ঘোরা ডিসপ্লের প্রতিটি কোণকে আলাদা ভূমিকা দেয়, তাই পথটি খোলা ও গোছানো লাগে।",
  "Corner tour": "কর্নার ট্যুর",
  Cross: "ক্রস",
  Cyan: "সায়ান",
  "Dark mode": "ডার্ক মোড",
  "Data is used to run the site, keep it secure, understand whether pages load correctly, and see which public pages people use. FoveaFlow does not sell visitor data.":
    "সাইট চালানো, নিরাপদ রাখা, পেজ ঠিকমতো লোড হচ্ছে কি না বোঝা এবং মানুষ কোন পাবলিক পেজ ব্যবহার করছে তা দেখার জন্য ডেটা ব্যবহার হয়। FoveaFlow ভিজিটর ডেটা বিক্রি করে না।",
  "Data we do not collect": "যে ডেটা আমরা সংগ্রহ করি না",
  Defaults: "ডিফল্ট",
  Details: "বিস্তারিত",
  Diagonal: "তির্যক",
  "Diagonal Smooth Pursuit Drill": "তির্যক মসৃণ অনুসরণ ড্রিল",
  "Diagonal Smooth Pursuit Eye Training": "তির্যক মসৃণ অনুসরণ চোখের অনুশীলন",
  "Diagonal uses longer corner-to-corner motion, so it emphasizes broader screen coverage and clean tracking over distance.":
    "তির্যক লম্বা কোণ-থেকে-কোণ চলন ব্যবহার করে, তাই বড় স্ক্রিন এলাকা জুড়ে পরিষ্কারভাবে অনুসরণে জোর পড়ে।",
  Diamond: "ডায়মন্ড",
  "Diamond Loop Smooth Pursuit Drill": "ডায়মন্ড লুপ মসৃণ অনুসরণ ড্রিল",
  "Diamond Loop Smooth Pursuit Eye Training":
    "ডায়মন্ড লুপ মসৃণ অনুসরণ চোখের অনুশীলন",
  "Diamond Loop combines a simple repeating route with clear corner transitions and sharper shifts than Circle.":
    "ডায়মন্ড লুপ একটি সহজ পুনরাবৃত্ত পথের সঙ্গে পরিষ্কার কোণ বদল ও বৃত্ত-এর চেয়ে তীক্ষ্ণ দিক পরিবর্তন মেশায়।",
  "Diamond loop": "ডায়মন্ড লুপ",
  "Direct routes": "সরাসরি পথ",
  Direction: "দিক",
  "Distractor Tracking Eye Training": "বিভ্রান্তিকারী ট্র্যাকিং চোখের অনুশীলন",
  "Distractor color": "বিভ্রান্তিকারীর রং",
  "Distractor color brightness": "বিভ্রান্তিকারীর রঙের উজ্জ্বলতা",
  Distractors: "বিভ্রান্তিকারী",
  "Do I need an account or app install?":
    "অ্যাকাউন্ট বা অ্যাপ ইনস্টল কি লাগবে?",
  "Do I need an account or install?": "অ্যাকাউন্ট বা ইনস্টল কি লাগবে?",
  "Do not attack, overload, scrape aggressively, or try to gain unauthorized access to the site or its infrastructure.":
    "সাইট বা এর অবকাঠামোয় আক্রমণ করবেন না, অতিরিক্ত চাপ দেবেন না, আক্রমণাত্মকভাবে স্ক্র্যাপ করবেন না, বা অনুমতি ছাড়া প্রবেশের চেষ্টা করবেন না।",
  "Do not follow the balls with your eyes.": "চোখ দিয়ে বলগুলো অনুসরণ করবেন না।",
  "Down-left Sweep Smooth Pursuit Drill": "নিচে-বামে Sweep মসৃণ অনুসরণ ড্রিল",
  "Down-left Sweep Smooth Pursuit Eye Training":
    "নিচে-বামে Sweep মসৃণ অনুসরণ চোখের অনুশীলন",
  "Down-left Sweep moves from the top-right corner toward the bottom-left corner on a simple diagonal line.":
    "নিচে-বামে সুইপ একটি সহজ তির্যক রেখায় ওপরের-ডান কোণ থেকে নিচের-বাম কোণের দিকে যায়।",
  "Down-left sweep": "নিচে-বামে সুইপ",
  "Down-right Sweep Smooth Pursuit Drill": "নিচে-ডানে Sweep মসৃণ অনুসরণ ড্রিল",
  "Down-right Sweep Smooth Pursuit Eye Training":
    "নিচে-ডানে Sweep মসৃণ অনুসরণ চোখের অনুশীলন",
  "Down-right Sweep moves from the top-left corner toward the bottom-right corner on a simple diagonal line.":
    "নিচে-ডানে সুইপ একটি সহজ তির্যক রেখায় ওপরের-বাম কোণ থেকে নিচের-ডান কোণের দিকে যায়।",
  "Down-right sweep": "নিচে-ডানে সুইপ",
  Drill: "ড্রিল",
  Drills: "ড্রিল",
  "Edge Loop Smooth Pursuit Drill": "প্রান্ত লুপ মসৃণ অনুসরণ ড্রিল",
  "Edge Loop Smooth Pursuit Eye Training":
    "প্রান্ত লুপ মসৃণ অনুসরণ চোখের অনুশীলন",
  "Edge Loop pushes the target around the perimeter, making the drill more spacious and edge-focused than center-heavy loops.":
    "প্রান্ত লুপ লক্ষ্যকে প্রান্তের চারপাশে চালায়, তাই এটি মাঝামাঝি ঘোরার লুপের চেয়ে বেশি খোলা ও প্রান্ত-কেন্দ্রিক লাগে।",
  "Edge loop": "এজ লুপ",
  Ellipse: "উপবৃত্ত",
  "Ellipse Smooth Pursuit Drill": "উপবৃত্ত মসৃণ অনুসরণ ড্রিল",
  "Ellipse Smooth Pursuit Eye Training": "উপবৃত্ত মসৃণ অনুসরণ চোখের অনুশীলন",
  "Ellipse keeps the calm rhythm of Circle but changes the width and height of the motion. It is a good bridge pattern when you want a familiar loop with more range.":
    "উপবৃত্ত বৃত্ত-এর শান্ত ছন্দ রাখে, কিন্তু চলনের প্রস্থ ও উচ্চতা বদলায়। পরিচিত লুপের সঙ্গে বেশি পরিসর চাইলে এটি ভালো মাঝামাঝি প্যাটার্ন।",
  "EyeTrainer.gg": "EyeTrainer.gg",
  "EyeTrainer.gg also offers simple browser patterns and promotes its upcoming Steam version from the public page.":
    "EyeTrainer.gg-তেও সহজ ব্রাউজার প্যাটার্ন আছে এবং পাবলিক পেজে তাদের আসন্ন Steam সংস্করণ দেখানো হয়।",
  "EyeTrainer.gg focuses the public browser tool on simple pattern practice rather than detailed target and display calibration.":
    "EyeTrainer.gg পাবলিক ব্রাউজার টুলে বিস্তারিত লক্ষ্য ও ডিসপ্লে ক্যালিব্রেশনের বদলে সহজ প্যাটার্ন অনুশীলনে জোর দেয়।",
  "EyeTrainer.gg provides simple pattern options, including patterns such as Vertical Waves.":
    "EyeTrainer.gg সহজ প্যাটার্ন অপশন দেয়, যেমন উল্লম্ব ঢেউ।",
  FAQ: "প্রশ্নোত্তর",
  "FPS Eye Training Warmup": "FPS চোখের অনুশীলন ওয়ার্মআপ",
  "FPS Eye Training Warmup (HIGH FPS)": "FPS চোখের অনুশীলন ওয়ার্মআপ (HIGH FPS)",
  "FPS warmup": "FPS ওয়ার্মআপ",
  "Feature comparison": "ফিচার তুলনা",
  "Figure Eight Smooth Pursuit Drill": "আটের আকৃতি মসৃণ অনুসরণ ড্রিল",
  "Figure Eight Smooth Pursuit Eye Training":
    "আটের আকৃতি মসৃণ অনুসরণ চোখের অনুশীলন",
  "Figure Eight adds a crossover point, which means the target passes through the center and changes direction more often than a simple loop.":
    "আটের আকৃতি-এ একটি ক্রসওভার পয়েন্ট থাকে, মানে লক্ষ্যটি মাঝখান দিয়ে যায় এবং সহজ লুপের চেয়ে বেশি বার দিক বদলায়।",
  "Figure eight": "ফিগার এইট",
  "Focused guides for FPS and alternatives": "FPS ও বিকল্প নিয়ে ফোকাসড গাইড",
  "Follow it like Smooth Pursuit, but do not let the darker balls pull your eyes away.":
    "মসৃণ অনুসরণের মতো অনুসরণ করুন, তবে গাঢ় বলগুলো যেন চোখ সরিয়ে না নেয়।",
  "Follow one moving target and train steady visual tracking.":
    "একটি চলমান লক্ষ্য অনুসরণ করুন এবং স্থির ভিজ্যুয়াল ট্র্যাকিং অনুশীলন করুন।",
  "For project questions, use the GitHub repository. Do not post private information in a public issue.":
    "প্রজেক্ট নিয়ে প্রশ্ন থাকলে GitHub repository ব্যবহার করুন। পাবলিক issue-তে ব্যক্তিগত তথ্য দেবেন না।",
  "FoveaFlow Guide": "FoveaFlow গাইড",
  "FoveaFlow and BlinkCamp are both free browser-based eye training tools. FoveaFlow is the stronger fit when you want direct drill links, FPS warmup use, reaction jumps, distractor tracking, and deeper control over how the target moves and appears.":
    "FoveaFlow ও BlinkCamp দুটিই বিনা খরচে ব্রাউজার-ভিত্তিক চোখের অনুশীলন টুল। সরাসরি ড্রিল লিংক, FPS ওয়ার্মআপ, দ্রুত ফোকাস বদল, বিভ্রান্তিকারী ট্র্যাকিং এবং লক্ষ্য কীভাবে চলে ও দেখতে কেমন হয় তার বেশি নিয়ন্ত্রণ চাইলে FoveaFlow বেশি মানানসই।",
  "FoveaFlow can be a short active break during long screen sessions. If screen use causes pain, dizziness, headaches, or ongoing symptoms, stop and get professional advice.":
    "লম্বা স্ক্রিন সেশনের মাঝে FoveaFlow ছোট সক্রিয় বিরতি হতে পারে। স্ক্রিন ব্যবহারে ব্যথা, মাথা ঘোরা, মাথাব্যথা বা চলমান কোনো উপসর্গ হলে থামুন এবং পেশাদার পরামর্শ নিন।",
  "FoveaFlow can be used without sending personal details. It is not built to collect personal information from children.":
    "ব্যক্তিগত তথ্য না পাঠিয়েও FoveaFlow ব্যবহার করা যায়। এটি শিশুদের ব্যক্তিগত তথ্য সংগ্রহের জন্য বানানো নয়।",
  "FoveaFlow does not set advertising cookies. Cloudflare may set security cookies when it needs them to keep the site available and safe.":
    "FoveaFlow বিজ্ঞাপন কুকি সেট করে না। সাইট চালু ও নিরাপদ রাখতে প্রয়োজন হলে Cloudflare নিরাপত্তা কুকি সেট করতে পারে।",
  "FoveaFlow eye trainer animation for visual tracking practice":
    "ভিজ্যুয়াল ট্র্যাকিং অনুশীলনের জন্য FoveaFlow চোখের অনুশীলন অ্যানিমেশন",
  "FoveaFlow eye trainer animation for visual tracking practice. Use Pause motion to stop target movement before changing controls.":
    "ভিজ্যুয়াল ট্র্যাকিং অনুশীলনের জন্য FoveaFlow চোখের অনুশীলন অ্যানিমেশন। কন্ট্রোল বদলানোর আগে লক্ষ্য থামাতে Pause motion ব্যবহার করুন।",
  "FoveaFlow eye trainer app": "FoveaFlow চোখের অনুশীলন অ্যাপ",
  "FoveaFlow guide": "FoveaFlow গাইড",
  "FoveaFlow home": "FoveaFlow হোম",
  "FoveaFlow is a free browser tool for visual tracking practice. It shows moving targets, reaction jumps, Lilac Chaser fixation practice, and distractor tracking patterns on a screen.":
    "FoveaFlow ভিজ্যুয়াল ট্র্যাকিং অনুশীলনের জন্য বিনা খরচের ব্রাউজার টুল। এটি স্ক্রিনে চলমান লক্ষ্য, দ্রুত ফোকাস বদল, লাইলাক চেজার ফিক্সেশন অনুশীলন এবং বিভ্রান্তিকারী ট্র্যাকিং প্যাটার্ন দেখায়।",
  "FoveaFlow is a free browser tool. Use it safely, stop if it feels bad, and do not treat it as medical care.":
    "FoveaFlow বিনা খরচের ব্রাউজার টুল। নিরাপদে ব্যবহার করুন, অস্বস্তি লাগলে থামুন, এবং এটিকে চিকিৎসা ভাববেন না।",
  "FoveaFlow is a free browser-based FPS eye training warmup. Use Smooth Pursuit for moving-target tracking, Reaction Jumps for quick refocus, and Multiple Distractions for staying locked on the right target through visual clutter.":
    "FoveaFlow বিনা খরচে ব্রাউজার-ভিত্তিক FPS চোখের ওয়ার্মআপ। চলমান লক্ষ্য অনুসরণে মসৃণ অনুসরণ, দ্রুত রিফোকাসে দ্রুত ফোকাস বদল, আর ভিজ্যুয়াল ভিড়ের মধ্যে ঠিক লক্ষ্যে চোখ রাখতে একাধিক বিভ্রান্তি ব্যবহার করুন।",
  "FoveaFlow is a free browser-based eye trainer for FPS warmups, visual tracking, reaction jumps, distractor tracking, and peripheral awareness. It is a practical alternative if you want to start in the browser with no account or install.":
    "FoveaFlow FPS ওয়ার্মআপ, ভিজ্যুয়াল ট্র্যাকিং, দ্রুত ফোকাস বদল, বিভ্রান্তিকারী ট্র্যাকিং এবং পারিপার্শ্বিক সচেতনতার জন্য বিনা খরচে ব্রাউজার-ভিত্তিক চোখের অনুশীলন টুল। অ্যাকাউন্ট বা ইনস্টল ছাড়া ব্রাউজারেই শুরু করতে চাইলে এটি ব্যবহারযোগ্য বিকল্প।",
  "FoveaFlow is a free online eye trainer for visual tracking, quick refocus, peripheral awareness, and focus under distraction. It runs in the browser with no account or install.":
    "FoveaFlow ভিজ্যুয়াল ট্র্যাকিং, দ্রুত রিফোকাস, পারিপার্শ্বিক সচেতনতা এবং বিভ্রান্তির মধ্যে ফোকাসের জন্য বিনা খরচের অনলাইন চোখের অনুশীলন টুল। অ্যাকাউন্ট বা ইনস্টল ছাড়াই ব্রাউজারে চলে।",
  "FoveaFlow is a free online eye training app for visual tracking, focus, reaction speed, and peripheral awareness. It includes Smooth Pursuit, Reaction Jumps, Lilac Chaser, and distractor tracking with no account or install.":
    "FoveaFlow ভিজ্যুয়াল ট্র্যাকিং, ফোকাস, প্রতিক্রিয়ার গতি এবং পারিপার্শ্বিক সচেতনতার জন্য বিনা খরচের অনলাইন চোখের অনুশীলন অ্যাপ। এতে মসৃণ অনুসরণ, দ্রুত ফোকাস বদল, লাইলাক চেজার এবং বিভ্রান্তিকারী ট্র্যাকিং আছে, অ্যাকাউন্ট বা ইনস্টল লাগে না।",
  "FoveaFlow is built to work without an account. The app keeps your settings in your browser and uses Cloudflare to serve the site.":
    "FoveaFlow অ্যাকাউন্ট ছাড়া কাজ করার জন্য বানানো। অ্যাপটি আপনার সেটিং ব্রাউজারে রাখে এবং সাইট চালাতে Cloudflare ব্যবহার করে।",
  "FoveaFlow is free to use, requires no account or install, and stores settings locally in your browser.":
    "FoveaFlow ব্যবহার করতে খরচ নেই, অ্যাকাউন্ট বা ইনস্টল লাগে না, এবং সেটিং আপনার ব্রাউজারেই রাখা হয়।",
  "FoveaFlow is free to use. There is no account, paid plan, subscription, or in-app purchase.":
    "FoveaFlow বিনা খরচে ব্যবহার করা যায়। অ্যাকাউন্ট, পেইড প্ল্যান, সাবস্ক্রিপশন বা ইন-অ্যাপ কেনাকাটা নেই।",
  "FoveaFlow is free. No account, no install.":
    "FoveaFlow বিনা খরচে। অ্যাকাউন্ট নেই, ইনস্টল নেই।",
  "FoveaFlow is free. No account, no paid plan.":
    "FoveaFlow বিনা খরচে। অ্যাকাউন্ট নেই, পেইড প্ল্যান নেই।",
  "FoveaFlow is not medical advice, diagnosis, treatment, vision therapy, or a medical device. It does not replace an optometrist, ophthalmologist, doctor, therapist, or other qualified professional.":
    "FoveaFlow চিকিৎসা পরামর্শ, রোগনির্ণয়, চিকিৎসা, ভিশন থেরাপি বা চিকিৎসা যন্ত্র নয়। এটি অপটোমেট্রিস্ট, চক্ষু বিশেষজ্ঞ, ডাক্তার, থেরাপিস্ট বা অন্য যোগ্য পেশাদারের বিকল্প নয়।",
  "FoveaFlow may help you train visual skills like tracking, refocusing, peripheral awareness, processing speed, and reaction timing. Results vary, and it is not a replacement for professional care if you have an eye condition or ongoing symptoms.":
    "FoveaFlow ট্র্যাকিং, রিফোকাস, পারিপার্শ্বিক সচেতনতা, প্রসেসিং গতি এবং প্রতিক্রিয়ার সময়ের মতো ভিজ্যুয়াল দক্ষতা অনুশীলনে সাহায্য করতে পারে। ফল সবার এক রকম নয়, আর চোখের সমস্যা বা চলমান উপসর্গ থাকলে এটি পেশাদার চিকিৎসার বিকল্প নয়।",
  "FoveaFlow on GitHub": "GitHub-এ FoveaFlow",
  "FoveaFlow overview": "FoveaFlow সারাংশ",
  "FoveaFlow saves local controls for speed, size, shape, color, opacity, trails, paths, distractors, letters, and display scale.":
    "FoveaFlow গতি, সাইজ, আকৃতি, রং, অস্বচ্ছতা, ট্রেইল, পথ, বিভ্রান্তিকারী, অক্ষর ও ডিসপ্লে স্কেলের লোকাল কন্ট্রোল সংরক্ষণ করে।",
  "FoveaFlow stores settings locally in your browser so the app can remember them on the current device. That can include the selected mode, motion pattern, speed, target size, color, opacity, trail setting, viewing distance, screen scale, and theme.":
    "FoveaFlow সেটিং আপনার ব্রাউজারে লোকালি রাখে, যাতে এই ডিভাইসে অ্যাপটি সেগুলো মনে রাখতে পারে। এর মধ্যে নির্বাচিত মোড, চলন প্যাটার্ন, গতি, লক্ষ্যের সাইজ, রং, অস্বচ্ছতা, ট্রেইল সেটিং, দেখার দূরত্ব, স্ক্রিন স্কেল ও থিম থাকতে পারে।",
  "FoveaFlow vs BlinkCamp": "FoveaFlow বনাম BlinkCamp",
  "FoveaFlow vs EyeTrainer.gg": "FoveaFlow বনাম EyeTrainer.gg",
  "FoveaFlow, free online eye trainer":
    "FoveaFlow, বিনা খরচের অনলাইন চোখের অনুশীলন টুল",
  "Free access": "বিনা খরচে ব্যবহার",
  "Free browser tool": "বিনা খরচের ব্রাউজার টুল",
  "Free online eye trainer": "বিনা খরচের অনলাইন চোখের অনুশীলন টুল",
  "Free online eye trainer for quick refocus, target acquisition, and fast visual reaction practice. Browser-based with no account or install.":
    "দ্রুত রিফোকাস, লক্ষ্য ধরার ক্ষমতা এবং দ্রুত ভিজ্যুয়াল প্রতিক্রিয়া অনুশীলনের জন্য বিনা খরচের অনলাইন চোখের অনুশীলন টুল। অ্যাকাউন্ট বা ইনস্টল ছাড়া ব্রাউজার-ভিত্তিক।",
  "Free online eye trainer for selective attention, distractor tracking, and focus under visual clutter. Browser-based with no account or install.":
    "নির্বাচিত মনোযোগ, বিভ্রান্তিকারী ট্র্যাকিং এবং ভিজ্যুয়াল ভিড়ের মধ্যে ফোকাসের জন্য বিনা খরচের অনলাইন চোখের অনুশীলন টুল। অ্যাকাউন্ট বা ইনস্টল ছাড়া ব্রাউজার-ভিত্তিক।",
  "Free online eye trainer for smooth pursuit, visual tracking, and steady moving-target practice. Browser-based with no account or install.":
    "মসৃণ অনুসরণ, ভিজ্যুয়াল ট্র্যাকিং এবং স্থিরভাবে চলমান লক্ষ্য অনুসরণের জন্য বিনা খরচের অনলাইন চোখের অনুশীলন টুল। অ্যাকাউন্ট বা ইনস্টল ছাড়া ব্রাউজার-ভিত্তিক।",
  "Free online eye trainer for steady fixation, peripheral awareness, and noticing change outside center focus. Browser-based with no account or install.":
    "স্থির ফিক্সেশন, পারিপার্শ্বিক সচেতনতা এবং কেন্দ্রীয় ফোকাসের বাইরে বদল খেয়াল করার জন্য বিনা খরচের অনলাইন চোখের অনুশীলন টুল। অ্যাকাউন্ট বা ইনস্টল ছাড়া ব্রাউজার-ভিত্তিক।",
  "Game awareness": "গেম সচেতনতা",
  "Gamer warm-up": "গেমার ওয়ার্মআপ",
  Gamers: "গেমার",
  "Give tired screen eyes a quick active break after reading, meetings, or too many tabs.":
    "পড়া, মিটিং বা বেশি ট্যাবের পর ক্লান্ত চোখকে ছোট সক্রিয় বিরতি দিন।",
  Gold: "সোনালি",
  "Guide FAQ": "গাইড প্রশ্নোত্তর",
  "Guide navigation": "গাইড নেভিগেশন",
  "Hard Turns Smooth Pursuit Drill": "তীক্ষ্ণ মোড় মসৃণ অনুসরণ ড্রিল",
  "Hard Turns Smooth Pursuit Eye Training":
    "তীক্ষ্ণ মোড় মসৃণ অনুসরণ চোখের অনুশীলন",
  "Hard Turns is one of the most demanding Smooth Pursuit patterns because the target changes direction abruptly.":
    "তীক্ষ্ণ মোড় সবচেয়ে কঠিন মসৃণ অনুসরণ প্যাটার্নগুলোর একটি, কারণ লক্ষ্য হঠাৎ দিক বদলায়।",
  "Hard turns": "হার্ড টার্নস",
  "Header target size": "হেডারে লক্ষ্যের সাইজ",
  "Header target speed": "হেডারে লক্ষ্যের গতি",
  Heavy: "ভারী",
  "Hold steady at the center and notice change around it.":
    "মাঝখানে স্থির থাকুন এবং চারপাশের বদল খেয়াল করুন।",
  "Hold the right target even when the screen gets busy.":
    "স্ক্রিন ব্যস্ত হলেও ঠিক লক্ষ্য ধরে রাখুন।",
  "Horizontal Sweep Smooth Pursuit Drill": "অনুভূমিক সুইপ মসৃণ অনুসরণ ড্রিল",
  "Horizontal Sweep Smooth Pursuit Eye Training":
    "অনুভূমিক সুইপ মসৃণ অনুসরণ চোখের অনুশীলন",
  "Horizontal Sweep keeps the motion simple and broad for left-right tracking across a larger visual range.":
    "অনুভূমিক সুইপ বড় ভিজ্যুয়াল পরিসরে বাম-ডান ট্র্যাকিংয়ের জন্য চলনকে সহজ ও বিস্তৃত রাখে।",
  "Horizontal sweep": "আড়াআড়ি সুইপ",
  Hourglass: "আওয়ারগ্লাস",
  "Hourglass Smooth Pursuit Drill": "বালুঘড়ি মসৃণ অনুসরণ ড্রিল",
  "Hourglass Smooth Pursuit Eye Training": "বালুঘড়ি মসৃণ অনুসরণ চোখের অনুশীলন",
  "Hourglass narrows through the middle and opens back out, creating repeated crossing behavior with a constrained shape.":
    "বালুঘড়ি মাঝখানে সরু হয় এবং আবার বাইরে খুলে যায়, ফলে সীমিত আকৃতির মধ্যে বারবার ক্রসিং তৈরি হয়।",
  "How data is used": "ডেটা কীভাবে ব্যবহার হয়",
  "How do I make it easier?": "এটি কীভাবে সহজ করব?",
  "How do I make it harder?": "এটি কীভাবে কঠিন করব?",
  "How do I make this pattern easier?": "এই প্যাটার্ন কীভাবে সহজ করব?",
  "How do I make this pattern harder?": "এই প্যাটার্ন কীভাবে কঠিন করব?",
  "How each drill works": "প্রতিটি ড্রিল কীভাবে কাজ করে",
  "How long should a session be?": "একটি সেশন কতক্ষণ হওয়া উচিত?",
  "How much can I customize?": "আমি কতটা বদলাতে পারি?",
  "How should beginners start?": "নতুনরা কীভাবে শুরু করবে?",
  "How to use": "কীভাবে ব্যবহার করবেন",
  "IT professionals": "IT পেশাজীবী",
  "If JavaScript is turned off, the moving target app will not run. The guide and policy pages still work as normal pages.":
    "JavaScript বন্ধ থাকলে চলমান লক্ষ্য অ্যাপ চলবে না। গাইড ও নীতির পেজগুলো সাধারণ পেজের মতোই কাজ করবে।",
  "If you have a vision condition, recent eye injury, surgery, neurological symptoms, or any concern about using moving visual targets, ask a qualified professional before using the app.":
    "আপনার দৃষ্টি-সংক্রান্ত সমস্যা, সাম্প্রতিক চোখের আঘাত, সার্জারি, স্নায়বিক উপসর্গ, বা চলমান ভিজ্যুয়াল লক্ষ্য ব্যবহার নিয়ে কোনো উদ্বেগ থাকলে অ্যাপ ব্যবহারের আগে যোগ্য পেশাদারের পরামর্শ নিন।",
  "Is FoveaFlow free?": "FoveaFlow কি বিনা খরচে?",
  "Is FoveaFlow good for gamers?": "FoveaFlow কি গেমারদের জন্য ভালো?",
  "Is FoveaFlow useful for IT professionals?":
    "FoveaFlow কি IT পেশাজীবীদের কাজে লাগে?",
  "Is Reaction Jumps useful before games?":
    "গেমের আগে দ্রুত ফোকাস বদল কি কাজে লাগে?",
  "Is Smooth Pursuit good as a warmup?": "ওয়ার্মআপ হিসেবে মসৃণ অনুসরণ কি ভালো?",
  "Is it aimed at FPS eye training?": "এটি কি FPS চোখের অনুশীলনের জন্য?",
  "Is there a public source link?": "পাবলিক সোর্স লিংক আছে?",
  "Is this a medical tool?": "এটি কি চিকিৎসা টুল?",
  "Is this good for gamers?": "এটি কি গেমারদের জন্য ভালো?",
  "Is this meant to improve eyesight?": "এটি কি দৃষ্টিশক্তি বাড়ানোর জন্য?",
  "Is this medical treatment?": "এটি কি চিকিৎসা?",
  "Is this the same as medical peripheral-vision therapy?":
    "এটি কি চিকিৎসাগত peripheral-vision therapy-এর মতো?",
  "It is a Smooth Pursuit pattern page that loads the matching path so you can start that style of moving-target practice immediately.":
    "এটি একটি মসৃণ অনুসরণ প্যাটার্ন পেজ, যা মিল থাকা পথ লোড করে যাতে আপনি সঙ্গে সঙ্গে সেই ধরনের চলমান লক্ষ্য অনুশীলন শুরু করতে পারেন।",
  "It is the task of following the correct target while similar moving objects compete for your attention.":
    "এটি এমন একটি কাজ, যেখানে একই রকম চলমান বস্তু মনোযোগ টানতে চাইলে আপনাকে ঠিক লক্ষ্য অনুসরণ করতে হয়।",
  "It trains selective attention, target identity, and steady tracking under clutter.":
    "এটি ভিড়ের মধ্যে নির্বাচিত মনোযোগ, লক্ষ্যের পরিচয় ধরে রাখা এবং স্থির ট্র্যাকিং অনুশীলন করায়।",
  "Keep it short": "ছোট রাখুন",
  "Keep sessions short and deliberate. The goal is focused practice, not pushing through discomfort.":
    "সেশন ছোট ও সচেতনভাবে করুন। লক্ষ্য হলো ফোকাসড অনুশীলন, অস্বস্তি সহ্য করে চালিয়ে যাওয়া নয়।",
  "Keep the safety line clear": "নিরাপত্তার সীমা পরিষ্কার রাখুন",
  "Keep your head still and let your eyes do the work.":
    "মাথা স্থির রাখুন, কাজটা চোখকে করতে দিন।",
  "Keep your head still and lock onto the main, brightest ball.":
    "মাথা স্থির রাখুন এবং মূল, সবচেয়ে উজ্জ্বল বলটিতে চোখ রাখুন।",
  "Keep your head still and start with your eyes on the ball.":
    "মাথা স্থির রাখুন এবং চোখ বলের ওপর রেখে শুরু করুন।",
  Language: "ভাষা",
  "Legal pages": "আইনি পেজ",
  "Let the disappearing gap move around the fixed circle. With steady focus, the colored balls may fade and the missing spot can look like a moving green afterimage.":
    "অদৃশ্য হওয়া ফাঁকটিকে স্থির বৃত্তের চারপাশে চলতে দিন। ফোকাস স্থির থাকলে রঙিন বলগুলো ফিকে লাগতে পারে এবং খালি জায়গাটি চলমান সবুজ আফটারইমেজের মতো দেখা যেতে পারে।",
  Letter: "অক্ষর",
  "Letter color": "অক্ষরের রং",
  "Letter text size": "অক্ষরের টেক্সট সাইজ",
  "Letter weight": "অক্ষরের ওজন",
  "Lilac Chaser": "লাইলাক চেজার",
  "Lilac Chaser Fixation and Peripheral Awareness":
    "লাইলাক চেজার ফিক্সেশন ও পারিপার্শ্বিক সচেতনতা",
  "Lilac Chaser ball color": "লাইলাক চেজার বলের রং",
  "Lilac Chaser for fixation and peripheral awareness.":
    "ফিক্সেশন ও পারিপার্শ্বিক সচেতনতার জন্য লাইলাক চেজার।",
  "Lilac Chaser is a fixation drill where you keep your gaze on a central cross while the outer pattern changes.":
    "লাইলাক চেজার একটি ফিক্সেশন ড্রিল, যেখানে বাইরের প্যাটার্ন বদলালেও আপনি মাঝের ক্রসে তাকিয়ে থাকেন।",
  "Lilac Chaser is a peripheral vision and focus drill. Keep your eyes on the center cross while one ball disappears at a time around a fixed circle. With steady fixation, many people perceive a moving green afterimage where the missing ball is.":
    "লাইলাক চেজার পারিপার্শ্বিক দৃষ্টি ও ফোকাসের ড্রিল। স্থির বৃত্তের চারপাশে একবারে একটি বল অদৃশ্য হওয়ার সময় মাঝের ক্রসে চোখ রাখুন। ফিক্সেশন স্থির থাকলে অনেকেই যেখানে বলটি নেই, সেখানে চলমান সবুজ আফটারইমেজ দেখেন।",
  "Lilac Chaser is different from the moving-target drills. Instead of following an object, you keep your eyes on the center cross while the outer ring changes.":
    "লাইলাক চেজার চলমান লক্ষ্য ড্রিলগুলোর থেকে আলাদা। কোনো বস্তু অনুসরণ না করে বাইরের রিং বদলানোর সময় আপনি মাঝের ক্রসে চোখ রাখেন।",
  "Lilac Chaser is for fixation and peripheral awareness.":
    "লাইলাক চেজার ফিক্সেশন ও পারিপার্শ্বিক সচেতনতার জন্য।",
  "Lilac Chaser is the best choice when you want to hold your gaze on the center and notice change away from it.":
    "মাঝখানে তাকিয়ে থেকে তার বাইরে বদল খেয়াল করতে চাইলে লাইলাক চেজার সবচেয়ে ভালো পছন্দ।",
  "Lilac Chaser scale": "লাইলাক চেজার স্কেল",
  "Lilac Chaser trains fixation, peripheral awareness, visual attention, and noticing change away from the point you are looking at. For gaming, it can be a short warmup for catching movement near the edge of your vision without constantly shifting your gaze.":
    "লাইলাক চেজার ফিক্সেশন, পারিপার্শ্বিক সচেতনতা, ভিজ্যুয়াল মনোযোগ এবং আপনি যেখানে তাকিয়ে আছেন তার বাইরে বদল খেয়াল করা অনুশীলন করায়। গেমিংয়ের ক্ষেত্রে বারবার দৃষ্টি না সরিয়ে দৃষ্টির প্রান্তের নড়াচড়া ধরার ছোট ওয়ার্মআপ হিসেবে এটি কাজে লাগতে পারে।",
  "Lilac chaser illusion": "লাইলাক চেজার বিভ্রম",
  Lissajous: "Lissajous",
  "Lissajous Smooth Pursuit Drill": "Lissajous মসৃণ অনুসরণ ড্রিল",
  "Lissajous Smooth Pursuit Eye Training":
    "Lissajous মসৃণ অনুসরণ চোখের অনুশীলন",
  "Lissajous is one of the more complex flowing patterns because the target moves through a looping path that changes its relationship to the center over time.":
    "Lissajous বেশি জটিল প্রবাহমান প্যাটার্নগুলোর একটি, কারণ লক্ষ্যটি এমন লুপিং পথে চলে যার কেন্দ্রের সঙ্গে সম্পর্ক সময়ের সঙ্গে বদলায়।",
  "Local settings with no account or install.":
    "অ্যাকাউন্ট বা ইনস্টল ছাড়াই লোকাল সেটিং।",
  "Look only at the black cross in the middle.":
    "শুধু মাঝের কালো ক্রসের দিকে তাকান।",
  "Lower the speed and increase target size so you have more time to settle on each jump.":
    "গতি কমান এবং লক্ষ্যের সাইজ বাড়ান, যাতে প্রতিটি জাম্পে ফোকাস করতে বেশি সময় পান।",
  "Lower the speed, increase target size, and keep the trail visible until you can stay on target comfortably.":
    "গতি কমান, লক্ষ্যের সাইজ বাড়ান, এবং আরামে লক্ষ্য ধরে রাখতে পারা পর্যন্ত ট্রেইল দৃশ্যমান রাখুন।",
  Magenta: "ম্যাজেন্টা",
  Medium: "মাঝারি",
  "Mode guide": "মোড গাইড",
  "More pages": "আরও পেজ",
  Motion: "চলন",
  "Motion and target": "চলন ও লক্ষ্য",
  "Motion feel": "চলনের অনুভূতি",
  "Motion path": "চলনের পথ",
  "Multiple Distractions": "একাধিক বিভ্রান্তি",
  "Multiple Distractions for focus under visual clutter.":
    "ভিজ্যুয়াল ভিড়ের মধ্যে ফোকাসের জন্য একাধিক বিভ্রান্তি।",
  "Multiple Distractions for staying with a target through clutter.":
    "ভিড়ের মধ্যেও লক্ষ্য ধরে রাখার জন্য একাধিক বিভ্রান্তি।",
  "Multiple Distractions includes distractor count and brightness controls. Letter overlays include letter color, weight, and scale controls.":
    "একাধিক বিভ্রান্তিতে বিভ্রান্তিকারীর সংখ্যা ও উজ্জ্বলতার কন্ট্রোল আছে। অক্ষর ওভারলেতে অক্ষরের রং, ওজন ও স্কেল কন্ট্রোল আছে।",
  "Multiple Distractions is FoveaFlow's clutter drill. One target matters most, but other moving objects share the screen and try to pull your attention away.":
    "একাধিক বিভ্রান্তি হলো FoveaFlow-এর ভিজ্যুয়াল ভিড় ড্রিল। একটি লক্ষ্যই সবচেয়ে গুরুত্বপূর্ণ, কিন্তু অন্য চলমান বস্তুগুলো একই স্ক্রিনে থাকে এবং আপনার মনোযোগ সরাতে চায়।",
  "Multiple Distractions is focus training under visual clutter. Follow the brightest ball while darker balls move through the same space and compete for your attention.":
    "একাধিক বিভ্রান্তি ভিজ্যুয়াল ভিড়ের মধ্যে ফোকাসের অনুশীলন। গাঢ় বলগুলো একই জায়গায় ঘুরে আপনার মনোযোগ টানতে চাইলে সবচেয়ে উজ্জ্বল বলটি অনুসরণ করুন।",
  "Multiple Distractions is for staying with the right target when the screen is busy.":
    "স্ক্রিন ব্যস্ত হলে ঠিক লক্ষ্য ধরে রাখার জন্য একাধিক বিভ্রান্তি।",
  "Multiple Distractions is the best choice for practicing selective attention under visual clutter.":
    "ভিজ্যুয়াল ভিড়ের মধ্যে নির্বাচিত মনোযোগ অনুশীলনের জন্য একাধিক বিভ্রান্তি সবচেয়ে ভালো পছন্দ।",
  "Multiple Distractions trains selective attention, visual tracking under clutter, and target identity. The job is not just following motion. You also have to keep choosing the right object when similar objects compete for attention.":
    "একাধিক বিভ্রান্তি নির্বাচিত মনোযোগ, ভিড়ের মধ্যে ভিজ্যুয়াল ট্র্যাকিং এবং লক্ষ্য চেনা অনুশীলন করায়। এখানে চলন অনুসরণ করতে হয়, পাশাপাশি একই রকম বস্তু মনোযোগ চাইলে বারবার ঠিক বস্তুটি বেছে রাখতে হয়।",
  "Multiple object tracking": "একাধিক বস্তু ট্র্যাকিং",
  "No account is needed for the simple browser patterns shown publicly. Steam is promoted for the upcoming app.":
    "পাবলিকভাবে দেখানো সহজ ব্রাউজার প্যাটার্নের জন্য অ্যাকাউন্ট লাগে না। আসন্ন অ্যাপের জন্য Steam প্রচার করা হচ্ছে।",
  "No. FoveaFlow is practice software and should not replace professional care.":
    "না। FoveaFlow অনুশীলনের সফটওয়্যার, পেশাদার চিকিৎসার বিকল্প নয়।",
  "No. FoveaFlow runs in the browser and stores settings locally.":
    "না। FoveaFlow ব্রাউজারে চলে এবং সেটিং লোকালি রাখে।",
  "No. It is a browser-based practice drill and not medical treatment.":
    "না। এটি ব্রাউজার-ভিত্তিক অনুশীলন ড্রিল, চিকিৎসা নয়।",
  "No. It is designed for practice, not to promise eyesight improvement.":
    "না। এটি অনুশীলনের জন্য বানানো, দৃষ্টিশক্তি উন্নতির প্রতিশ্রুতি দেওয়ার জন্য নয়।",
  "No. It is practice software, not a treatment or diagnostic tool.":
    "না। এটি অনুশীলনের সফটওয়্যার, চিকিৎসা বা রোগনির্ণয়ের টুল নয়।",
  "No. Keep your eyes on the center cross and let the visual effect happen in the periphery.":
    "না। মাঝের ক্রসে চোখ রাখুন এবং প্রান্তে ভিজ্যুয়াল প্রভাব হতে দিন।",
  "No. The tool runs in a modern browser and stores settings locally in your browser.":
    "না। টুলটি আধুনিক ব্রাউজারে চলে এবং আপনার ব্রাউজারে সেটিং লোকালি রাখে।",
  "Not medical care": "চিকিৎসা নয়",
  "On this page": "এই পেজে",
  Opacity: "অস্বচ্ছতা",
  Open: "খুলুন",
  "Open FPS warmup": "FPS ওয়ার্মআপ খুলুন",
  "Open Smooth Pursuit": "মসৃণ অনুসরণ খুলুন",
  "Open controls": "কন্ট্রোল খুলুন",
  "Open guide": "গাইড খুলুন",
  "Open the full guide": "পুরো গাইড খুলুন",
  Overview: "সারাংশ",
  "Page navigation": "পেজ নেভিগেশন",
  "Path, direction changes, steady motion, speed waves, bursts, build/reset, and size pulse options are available depending on the drill.":
    "ড্রিল অনুযায়ী পথ, দিক বদল, স্থির চলন, স্পিড ওয়েভ, ছোট বার্স্ট, build/reset এবং সাইজ পালস অপশন পাওয়া যায়।",
  "Pattern pages start Smooth Pursuit with that path selected. Reaction jumps, Multiple Distractions, and Lilac Chaser have their own direct URLs.":
    "প্যাটার্ন পেজগুলো নির্বাচিত পথসহ মসৃণ অনুসরণ শুরু করে। দ্রুত ফোকাস বদল, একাধিক বিভ্রান্তি এবং লাইলাক চেজারের নিজস্ব সরাসরি URL আছে।",
  Pause: "বিরতি",
  "Pause motion": "চলন থামান",
  "People on screens all day": "সারাদিন স্ক্রিনে থাকা মানুষ",
  "Peripheral awareness": "পারিপার্শ্বিক সচেতনতা",
  "Pick a drill and tune the target": "একটি ড্রিল বেছে নিন এবং লক্ষ্য সেট করুন",
  "Practice pages": "অনুশীলন পেজ",
  "Practice software, not medical care. Stop if you feel eye strain, dizziness, headache, nausea, or any other discomfort.":
    "অনুশীলনের সফটওয়্যার, চিকিৎসা নয়। চোখে চাপ, মাথা ঘোরা, মাথাব্যথা, বমিভাব বা অন্য কোনো অস্বস্তি হলে থামুন।",
  "Practice the Bounce smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "বাউন্স মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Circle smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "বৃত্ত মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Clover smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "ক্লোভার মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Corner tour smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "চার কোণ ঘোরা মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Diagonal smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "তির্যক মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Diamond loop smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "ডায়মন্ড লুপ মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Down-left sweep smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "নিচে-বামে সুইপ মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Down-right sweep smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "নিচে-ডানে সুইপ মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Edge loop smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "প্রান্ত লুপ মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Ellipse smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "উপবৃত্ত মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Figure eight smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "আটের আকৃতি মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Hard turns smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "তীক্ষ্ণ মোড় মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Horizontal sweep smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "অনুভূমিক সুইপ মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Hourglass smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "বালুঘড়ি মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Lissajous smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "Lissajous মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Random smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "এলোমেলো মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Stair steps smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "সিঁড়ি ধাপ মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Vertical sweep smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "উল্লম্ব সুইপ মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Wave smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "ঢেউ মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  "Practice the Zigzag smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "জিগজ্যাগ মসৃণ অনুসরণ প্যাটার্ন অনলাইনে অনুশীলন করুন। ছোট ভিজ্যুয়াল ট্র্যাকিং সেশনের জন্য গতি, লক্ষ্যের সাইজ, রং, ট্রেইল ও স্ক্রিন স্কেল বদলান।",
  Predictive: "আগে থেকে অনুমানযোগ্য",
  Privacy: "গোপনীয়তা",
  "Privacy Policy": "গোপনীয়তা নীতি",
  Question: "প্রশ্ন",
  "Quick answers": "দ্রুত উত্তর",
  "Quick refocus": "দ্রুত রিফোকাস",
  "Raise the speed and reduce target size so you must find each new position faster.":
    "গতি বাড়ান এবং লক্ষ্যের সাইজ কমান, যাতে প্রতিটি নতুন অবস্থান আরও দ্রুত খুঁজতে হয়।",
  "Raise the speed, reduce target size, or switch from an easy loop to a more demanding path.":
    "গতি বাড়ান, লক্ষ্যের সাইজ কমান, অথবা সহজ লুপ থেকে বেশি কঠিন পথে যান।",
  Random: "র‍্যান্ডম",
  "Random Smooth Pursuit Drill": "এলোমেলো মসৃণ অনুসরণ ড্রিল",
  "Random Smooth Pursuit Eye Training": "এলোমেলো মসৃণ অনুসরণ চোখের অনুশীলন",
  "Random removes the comfort of a repeating loop. Because the target changes direction and position less predictably, the drill adds more target-search work than a simple circle or ellipse.":
    "এলোমেলো পুনরাবৃত্ত লুপের আরাম সরিয়ে দেয়। লক্ষ্য কম অনুমানযোগ্যভাবে দিক ও অবস্থান বদলায়, তাই সহজ circle বা ellipse-এর চেয়ে এখানে লক্ষ্য খোঁজার কাজ বেশি।",
  "Reaction Jumps": "দ্রুত ফোকাস বদল",
  "Reaction Jumps Eye Training": "দ্রুত ফোকাস বদল চোখের অনুশীলন",
  "Reaction Jumps for quick refocus.": "দ্রুত রিফোকাসের জন্য দ্রুত ফোকাস বদল।",
  "Reaction Jumps is best when you want to find a new target position quickly and lock on before the next move.":
    "নতুন লক্ষ্য অবস্থান দ্রুত খুঁজে পরের মুভের আগে চোখ স্থির করতে চাইলে দ্রুত ফোকাস বদল সবচেয়ে ভালো।",
  "Reaction Jumps is for reaction time training, fast refocus, and target switching.":
    "দ্রুত ফোকাস বদল প্রতিক্রিয়ার সময়, দ্রুত রিফোকাস এবং লক্ষ্য বদলানোর অনুশীলনের জন্য।",
  "Reaction Jumps is the drill to use when moving smoothly is not the point. The target holds still, then jumps to a new location.":
    "মসৃণ চলন অনুসরণই লক্ষ্য না হলে দ্রুত ফোকাস বদল ব্যবহার করুন। লক্ষ্যটি স্থির থাকে, তারপর নতুন জায়গায় লাফ দেয়।",
  "Reaction Jumps trains quick refocus. The target holds still, then jumps to a new spot so you can find it fast and focus before the next move.":
    "দ্রুত ফোকাস বদল দ্রুত রিফোকাস অনুশীলন করায়। লক্ষ্যটি স্থির থাকে, তারপর নতুন জায়গায় লাফ দেয়, যাতে আপনি দ্রুত খুঁজে পরের মুভের আগে ফোকাস করতে পারেন।",
  "Reaction Jumps trains quick target acquisition and quick refocus from one target position to the next.":
    "দ্রুত ফোকাস বদল দ্রুত লক্ষ্য ধরা এবং এক অবস্থান থেকে আরেক অবস্থানে দ্রুত রিফোকাস অনুশীলন করায়।",
  "Reaction Jumps trains quick target acquisition, saccadic eye movement, peripheral detection, and fast refocusing. It is useful when you want to react to a new visual target without moving your head first.":
    "দ্রুত ফোকাস বদল দ্রুত লক্ষ্য ধরা, saccadic eye movement, পারিপার্শ্বিক শনাক্তকরণ এবং দ্রুত রিফোকাস অনুশীলন করায়। মাথা আগে না সরিয়ে নতুন ভিজ্যুয়াল লক্ষ্যে প্রতিক্রিয়া দিতে চাইলে এটি কাজে লাগে।",
  "Reaction Jumps works well as a short pre-game warmup or a fast visual reset between tasks.":
    "গেমের আগে ছোট ওয়ার্মআপ বা কাজের মাঝে দ্রুত ভিজ্যুয়াল রিসেট হিসেবে দ্রুত ফোকাস বদল ভালো কাজ করে।",
  "Reaction jumps": "দ্রুত ফোকাস বদল",
  "Reaction warm-up": "রিঅ্যাকশন ওয়ার্মআপ",
  "Read full guide": "পুরো গাইড পড়ুন",
  "Read guide FAQ": "গাইড প্রশ্নোত্তর পড়ুন",
  "Read source": "সোর্স পড়ুন",
  "Read the full guide": "পুরো গাইড পড়ুন",
  "Read the guide": "গাইড পড়ুন",
  Red: "লাল",
  References: "রেফারেন্স",
  Regular: "রেগুলার",
  "Research and background reading": "গবেষণা ও পটভূমি পড়া",
  "Reset to defaults": "ডিফল্টে ফিরিয়ে নিন",
  "Restore the selected drill to its default behavior, visuals, calibration, and saved local settings.":
    "নির্বাচিত ড্রিলকে তার ডিফল্ট আচরণ, ভিজ্যুয়াল, ক্যালিব্রেশন এবং সংরক্ষিত লোকাল সেটিংয়ে ফিরিয়ে নিন।",
  Resume: "আবার চালু করুন",
  "Resume motion": "চলন আবার চালু করুন",
  "Reveal controls": "কন্ট্রোল দেখান",
  "Reverse motion direction": "চলনের দিক উল্টান",
  Ring: "রিং",
  "Role of peripheral vision in saccade planning":
    "স্যাকাড পরিকল্পনায় পারিপার্শ্বিক দৃষ্টির ভূমিকা",
  "Saccadic reaction time factors": "স্যাকাডিক প্রতিক্রিয়া সময়ের কারণ",
  Safety: "নিরাপত্তা",
  Scale: "স্কেল",
  "Screen break": "স্ক্রিন বিরতি",
  "Screen reset": "স্ক্রিন রিসেট",
  "Screen scale": "স্ক্রিন স্কেল",
  "Screen-work reset": "স্ক্রিন কাজের রিসেট",
  "Select language": "ভাষা বেছে নিন",
  "Selective attention": "নির্বাচিত মনোযোগ",
  Semibold: "সেমিবোল্ড",
  Session: "সেশন",
  "Settings saved in your browser": "সেটিং আপনার ব্রাউজারে সংরক্ষিত",
  Shape: "আকৃতি",
  "Shape, color, opacity, trail length, and trail behavior are adjustable.":
    "আকৃতি, রং, অস্বচ্ছতা, ট্রেইলের দৈর্ঘ্য এবং ট্রেইলের আচরণ বদলানো যায়।",
  "Sharpen focus between code, logs, dashboards, terminals, tickets, and multi-monitor work.":
    "কোড, লগ, ড্যাশবোর্ড, টার্মিনাল, টিকিট এবং একাধিক মনিটরের কাজের মাঝে ফোকাস ঝালিয়ে নিন।",
  "Sharpen selective focus by tracking the brightest target through moving distractions.":
    "চলমান বিভ্রান্তির মধ্যে সবচেয়ে উজ্জ্বল লক্ষ্য অনুসরণ করে নির্বাচিত ফোকাস ঝালিয়ে নিন।",
  "Sharpen your visual warmup before FPS games with tracking, refocus, peripheral awareness, and character movement reading drills.":
    "FPS গেমের আগে ট্র্যাকিং, রিফোকাস, পারিপার্শ্বিক সচেতনতা এবং চরিত্রের চলন পড়ার ড্রিল দিয়ে ভিজ্যুয়াল ওয়ার্মআপ ঝালিয়ে নিন।",
  "Short bursts": "ছোট বার্স্ট",
  "Should I follow the outer shapes?": "বাইরের আকৃতিগুলো কি অনুসরণ করব?",
  "Show target letters": "লক্ষ্যের অক্ষর দেখান",
  "Show trail": "ট্রেইল দেখান",
  Size: "সাইজ",
  "Size pulse": "সাইজ পালস",
  "Smooth Pursuit": "মসৃণ অনুসরণ",
  "Smooth Pursuit Eye Training": "মসৃণ অনুসরণ চোখের অনুশীলন",
  "Smooth Pursuit for one-target visual tracking.":
    "একটি লক্ষ্য ভিজ্যুয়াল ট্র্যাকিংয়ের জন্য মসৃণ অনুসরণ।",
  "Smooth Pursuit helps train steady tracking, moving-target focus, and controlled eye movement across more of your usable range. Predictable paths build rhythm and control. Random paths and hard turns add more visual search and reaction demand.":
    "মসৃণ অনুসরণ স্থির ট্র্যাকিং, চলমান লক্ষ্য ফোকাস এবং ব্যবহারযোগ্য পরিসরের বেশি অংশ জুড়ে নিয়ন্ত্রিত চোখের চলন অনুশীলনে সাহায্য করে। অনুমানযোগ্য পথ ছন্দ ও নিয়ন্ত্রণ গড়ে। এলোমেলো পথ ও hard turns বেশি ভিজ্যুয়াল খোঁজ এবং প্রতিক্রিয়ার চাপ যোগ করে।",
  "Smooth Pursuit is FoveaFlow's moving-target drill. Keep your head still, follow one target with your eyes, and stay smooth instead of jumping ahead.":
    "মসৃণ অনুসরণ হলো FoveaFlow-এর চলমান লক্ষ্য ড্রিল। মাথা স্থির রাখুন, চোখ দিয়ে একটি লক্ষ্য অনুসরণ করুন, এবং আগে লাফিয়ে না গিয়ে মসৃণ থাকুন।",
  "Smooth Pursuit is a drill where you follow one moving target as steadily as you can with your eyes.":
    "মসৃণ অনুসরণ এমন ড্রিল যেখানে চোখ দিয়ে একটি চলমান লক্ষ্য যতটা পারেন স্থিরভাবে অনুসরণ করেন।",
  "Smooth Pursuit is a moving-target eye training drill. Keep your head still and track the ball with your eyes. Predictable paths build steady control; random paths and hard turns add more target-search work.":
    "মসৃণ অনুসরণ চলমান লক্ষ্যভিত্তিক চোখের অনুশীলন ড্রিল। মাথা স্থির রাখুন এবং চোখ দিয়ে বলটি অনুসরণ করুন। অনুমানযোগ্য পথ স্থির নিয়ন্ত্রণ গড়ে; random পথ ও hard turns লক্ষ্য খোঁজার কাজ বাড়ায়।",
  "Smooth Pursuit is for steady moving-target tracking.":
    "মসৃণ অনুসরণ স্থিরভাবে চলমান লক্ষ্য অনুসরণের জন্য।",
  "Smooth Pursuit is the best starting point when your goal is following one moving target as steadily as possible.":
    "একটি চলমান লক্ষ্য যতটা সম্ভব স্থিরভাবে অনুসরণ করাই লক্ষ্য হলে মসৃণ অনুসরণ সবচেয়ে ভালো শুরু।",
  "Smooth Pursuit paths for moving-target tracking.":
    "চলমান লক্ষ্য ট্র্যাকিংয়ের জন্য মসৃণ অনুসরণ পথ।",
  "Smooth Pursuit pattern": "মসৃণ অনুসরণ প্যাটার্ন",
  "Smooth Pursuit pattern routes": "মসৃণ অনুসরণ প্যাটার্ন পথ",
  "Smooth Pursuit, Reaction Jumps, Multiple Distractions, and Lilac Chaser are available as separate modes.":
    "মসৃণ অনুসরণ, দ্রুত ফোকাস বদল, একাধিক বিভ্রান্তি এবং লাইলাক চেজার আলাদা মোড হিসেবে আছে।",
  "Snap to the next target and train fast refocus.":
    "পরের লক্ষ্যে দ্রুত চোখ নিন এবং দ্রুত রিফোকাস অনুশীলন করুন।",
  Source: "সোর্স",
  "Source code": "সোর্স কোড",
  "Spatial allocation of attention during smooth pursuit":
    "মসৃণ অনুসরণ চলার সময় মনোযোগের স্থানিক বণ্টন",
  Speed: "গতি",
  "Speed unit": "গতির একক",
  "Speed units, target size, shape, color, opacity, trails, paths, motion behavior, distractors, letter overlays, viewing distance, screen scale, and mode-specific controls.":
    "গতির একক, লক্ষ্যের সাইজ, আকৃতি, রং, অস্বচ্ছতা, ট্রেইল, পথ, চলনের আচরণ, বিভ্রান্তিকারী, অক্ষর ওভারলে, দেখার দূরত্ব, স্ক্রিন স্কেল এবং মোডভিত্তিক কন্ট্রোল।",
  "Speed wave": "স্পিড ওয়েভ",
  "Speed, size, shape, color, opacity, and trail change the feel of the moving drills. Lilac Chaser has its own ball color and scale controls.":
    "গতি, সাইজ, আকৃতি, রং, অস্বচ্ছতা ও ট্রেইল চলমান ড্রিলের অনুভূতি বদলায়। লাইলাক চেজারের নিজস্ব বলের রং ও স্কেল কন্ট্রোল আছে।",
  "Speed, target size, and path matter most because they change difficulty quickly.":
    "গতি, লক্ষ্যের সাইজ ও পথ সবচেয়ে গুরুত্বপূর্ণ, কারণ এগুলো দ্রুত কঠিনতা বদলায়।",
  Square: "বর্গ",
  "Stair Steps Smooth Pursuit Drill": "সিঁড়ি ধাপ মসৃণ অনুসরণ ড্রিল",
  "Stair Steps Smooth Pursuit Eye Training":
    "সিঁড়ি ধাপ মসৃণ অনুসরণ চোখের অনুশীলন",
  "Stair Steps creates a mechanical route with discrete directional segments that stays easier to predict than Random or Hard Turns.":
    "সিঁড়ি ধাপ আলাদা দিক-ভাগসহ যান্ত্রিক পথ তৈরি করে, যা এলোমেলো বা তীক্ষ্ণ মোড়-এর চেয়ে সহজে অনুমান করা যায়।",
  "Stair steps": "স্টেয়ার স্টেপস",
  "Start Bounce": "বাউন্স শুরু করুন",
  "Start Circle": "বৃত্ত শুরু করুন",
  "Start Clover": "ক্লোভার শুরু করুন",
  "Start Corner tour": "চার কোণ ঘোরা শুরু করুন",
  "Start Diagonal": "তির্যক শুরু করুন",
  "Start Diamond loop": "ডায়মন্ড লুপ শুরু করুন",
  "Start Distractor Tracking": "Distractor Tracking শুরু করুন",
  "Start Down-left sweep": "নিচে-বামে সুইপ শুরু করুন",
  "Start Down-right sweep": "নিচে-ডানে সুইপ শুরু করুন",
  "Start Edge loop": "প্রান্ত লুপ শুরু করুন",
  "Start Ellipse": "উপবৃত্ত শুরু করুন",
  "Start Figure eight": "আটের আকৃতি শুরু করুন",
  "Start Hard turns": "তীক্ষ্ণ মোড় শুরু করুন",
  "Start Horizontal sweep": "অনুভূমিক সুইপ শুরু করুন",
  "Start Hourglass": "বালুঘড়ি শুরু করুন",
  "Start Lilac Chaser": "লাইলাক চেজার শুরু করুন",
  "Start Lissajous": "Lissajous শুরু করুন",
  "Start Random": "এলোমেলো শুরু করুন",
  "Start Reaction Jumps": "দ্রুত ফোকাস বদল শুরু করুন",
  "Start Smooth Pursuit": "মসৃণ অনুসরণ শুরু করুন",
  "Start Stair steps": "সিঁড়ি ধাপ শুরু করুন",
  "Start Vertical sweep": "উল্লম্ব সুইপ শুরু করুন",
  "Start Wave": "ঢেউ শুরু করুন",
  "Start Zigzag": "জিগজ্যাগ শুরু করুন",
  "Start the warmup": "ওয়ার্মআপ শুরু করুন",
  "Start with a comfortable speed and a medium target size, then raise difficulty only when you can stay on the target cleanly.":
    "আরামদায়ক গতি ও মাঝারি লক্ষ্যের সাইজ দিয়ে শুরু করুন, তারপর লক্ষ্য পরিষ্কারভাবে ধরে রাখতে পারলে তবেই কঠিনতা বাড়ান।",
  "Start with a predictable path such as Circle or Ellipse before moving to harder patterns.":
    "কঠিন প্যাটার্নে যাওয়ার আগে বৃত্ত বা উপবৃত্ত-এর মতো অনুমানযোগ্য পথ দিয়ে শুরু করুন।",
  "Start with fewer distractors, a slower speed, and a larger main target.":
    "কম বিভ্রান্তিকারী, ধীর গতি এবং বড় মূল লক্ষ্য দিয়ে শুরু করুন।",
  "Start with fewer distractors, then add more when you can keep the target cleanly.":
    "কম বিভ্রান্তিকারী দিয়ে শুরু করুন, তারপর লক্ষ্য পরিষ্কারভাবে ধরে রাখতে পারলে আরও যোগ করুন।",
  "Steady fixation": "স্থির ফিক্সেশন",
  "Steady speed": "স্থির গতি",
  "Stop the session and rest. Do not push through discomfort.":
    "সেশন থামিয়ে বিশ্রাম নিন। অস্বস্তি সহ্য করে চালিয়ে যাবেন না।",
  "Target acquisition": "লক্ষ্য ধরা",
  "Target appearance, trail display, distractor brightness, letter styling, viewing distance, and screen scale can be tuned.":
    "লক্ষ্যের চেহারা, ট্রেইল দেখা, বিভ্রান্তিকারীর উজ্জ্বলতা, অক্ষরের স্টাইল, দেখার দূরত্ব ও স্ক্রিন স্কেল বদলানো যায়।",
  "Target opacity": "লক্ষ্যের অস্বচ্ছতা",
  "Target size": "লক্ষ্যের সাইজ",
  Targets: "লক্ষ্য",
  Terms: "শর্ত",
  "Terms of Use": "ব্যবহারের শর্ত",
  "Text size": "টেক্সট সাইজ",
  "The goal is steady fixation and better awareness of change away from the center of your gaze.":
    "লক্ষ্য হলো স্থির ফিক্সেশন এবং দৃষ্টির কেন্দ্রের বাইরে বদল আরও ভালোভাবে খেয়াল করা।",
  "The path shape changes how predictable the movement feels and how often the target changes direction.":
    "পথের আকৃতি বদলালে চলন কতটা অনুমানযোগ্য লাগে এবং লক্ষ্য কতবার দিক বদলায় তা বদলায়।",
  "The patterns are simple screen paths and timing drills. They are not a clinical program, and results will vary from person to person.":
    "প্যাটার্নগুলো সহজ স্ক্রিন পথ ও টাইমিং ড্রিল। এগুলো ক্লিনিক্যাল প্রোগ্রাম নয়, এবং ফল ব্যক্তি ভেদে বদলাবে।",
  "The public browser page keeps controls simple: Show Grid, Darkmode, and pattern selection.":
    "পাবলিক ব্রাউজার পেজে কন্ট্রোল সহজ রাখা হয়েছে: Show Grid, Darkmode এবং প্যাটার্ন নির্বাচন।",
  "The site is provided as is. It may change, break, or go offline. To the fullest extent allowed by law, FoveaFlow is provided without warranties of any kind.":
    "সাইটটি যেমন আছে তেমনভাবেই দেওয়া হয়। এটি বদলাতে পারে, নষ্ট হতে পারে বা অফলাইনে যেতে পারে। আইন যতটা অনুমতি দেয়, FoveaFlow কোনো ধরনের ওয়ারেন্টি ছাড়া দেওয়া হয়।",
  "The site runs on Cloudflare. Cloudflare may process request data such as IP address, user agent, requested URL, and timing data to deliver the site, protect it from abuse, and show basic traffic and performance metrics.":
    "সাইটটি Cloudflare-এ চলে। সাইট পৌঁছে দেওয়া, অপব্যবহার থেকে রক্ষা করা এবং মৌলিক ট্রাফিক ও পারফরম্যান্স মেট্রিক দেখানোর জন্য Cloudflare IP address, user agent, requested URL এবং timing data-এর মতো request data প্রসেস করতে পারে।",
  "The source code is public on GitHub under the license in the repository. These terms cover use of the hosted FoveaFlow site.":
    "সোর্স কোড repository-র লাইসেন্স অনুযায়ী GitHub-এ পাবলিক। এই শর্তগুলো হোস্ট করা FoveaFlow সাইট ব্যবহারের জন্য।",
  "These pages cover the common search paths around eye trainer warmups and browser-based alternatives.":
    "এই পেজগুলো eye trainer warmup এবং ব্রাউজার-ভিত্তিক বিকল্প নিয়ে সাধারণ সার্চ পথগুলো ধরে।",
  "These terms may be updated when the app or site changes. The date at the top shows the latest version.":
    "অ্যাপ বা সাইট বদলালে এই শর্তগুলো আপডেট হতে পারে। ওপরে থাকা তারিখটি সর্বশেষ সংস্করণ দেখায়।",
  "This mode is a strong fit when you want to practice selective attention and target identity under visual noise.":
    "ভিজ্যুয়াল শব্দের মধ্যে নির্বাচিত মনোযোগ ও লক্ষ্য চেনা অনুশীলন করতে চাইলে এই মোড ভালো মানায়।",
  "This mode is useful when you want faster target acquisition and cleaner refocusing. Start slower for accuracy, then raise the speed for a more demanding session.":
    "দ্রুত লক্ষ্য ধরা এবং পরিষ্কার রিফোকাস চাইলে এই মোড কাজে লাগে। নির্ভুলতার জন্য ধীরে শুরু করুন, তারপর বেশি চ্যালেঞ্জ চাইলে গতি বাড়ান।",
  "This mode works best when you resist the urge to chase the disappearing gap. Keep your gaze centered, stay relaxed, and let the effect happen on its own.":
    "অদৃশ্য হওয়া ফাঁক অনুসরণ করার তাগিদ সামলাতে পারলে এই মোড সবচেয়ে ভালো কাজ করে। দৃষ্টি মাঝখানে রাখুন, শিথিল থাকুন, প্রভাবটি নিজে থেকে হতে দিন।",
  "This page is specific to this free browser tool.":
    "এই পেজটি এই বিনা খরচের ব্রাউজার টুলের জন্য নির্দিষ্ট।",
  "Those settings stay in your browser unless your browser syncs, backs up, or exports its site data. You can remove them by clearing site data for foveaflow.com.":
    "আপনার ব্রাউজার সাইট ডেটা sync, backup বা export না করলে এই সেটিংগুলো ব্রাউজারেই থাকে। foveaflow.com-এর site data মুছে এগুলো সরাতে পারেন।",
  "Track the ball as smoothly as you can instead of jumping ahead of it.":
    "বলের আগে লাফিয়ে না গিয়ে যতটা পারেন মসৃণভাবে বলটি অনুসরণ করুন।",
  "Train peripheral awareness by holding focus on the center cross.":
    "মাঝের ক্রসে ফোকাস ধরে রেখে পারিপার্শ্বিক সচেতনতা অনুশীলন করুন।",
  "Train quick refocus by snapping your eyes to each new target position.":
    "প্রতিটি নতুন লক্ষ্য অবস্থানে দ্রুত চোখ নিয়ে দ্রুত রিফোকাস অনুশীলন করুন।",
  "Train smooth visual tracking by following one moving target.":
    "একটি চলমান লক্ষ্য অনুসরণ করে মসৃণ ভিজ্যুয়াল ট্র্যাকিং অনুশীলন করুন।",
  "Train visual tracking, quick refocus, peripheral awareness, and focus under distraction in your browser.":
    "ব্রাউজারেই ভিজ্যুয়াল ট্র্যাকিং, দ্রুত রিফোকাস, পারিপার্শ্বিক সচেতনতা এবং বিভ্রান্তির মধ্যে ফোকাস অনুশীলন করুন।",
  Triangle: "ত্রিভুজ",
  "Try FoveaFlow": "FoveaFlow ব্যবহার করে দেখুন",
  "Try Smooth Pursuit": "মসৃণ অনুসরণ ব্যবহার করে দেখুন",
  "Try Smooth Pursuit first": "আগে মসৃণ অনুসরণ চেষ্টা করুন",
  "Try another drill": "আরেকটি ড্রিল চেষ্টা করুন",
  "Tune the session before you start": "শুরু করার আগে সেশন ঠিক করুন",
  Unit: "একক",
  Unpredictive: "অনুমান করা কঠিন",
  "Updated April 28, 2026": "আপডেট: ২৮ এপ্রিল ২০২৬",
  "Updated May 14, 2026": "আপডেট: ১৪ মে ২০২৬",
  "Updated May 14, 2026. FoveaFlow is practice software, not medical care. Stop if a session causes strain, dizziness, headache, nausea, or any other discomfort.":
    "আপডেট: ১৪ মে ২০২৬। FoveaFlow অনুশীলনের সফটওয়্যার, চিকিৎসা নয়। কোনো সেশনে চাপ, মাথা ঘোরা, মাথাব্যথা, বমিভাব বা অন্য অস্বস্তি হলে থামুন।",
  "Updated May 14, 2026. If the visual effect feels strange or uncomfortable, stop the session and rest.":
    "আপডেট: ১৪ মে ২০২৬। ভিজ্যুয়াল প্রভাব অদ্ভুত বা অস্বস্তিকর লাগলে সেশন থামিয়ে বিশ্রাম নিন।",
  "Updated May 14, 2026. Keep sessions brief and controlled. If the drill causes strain or discomfort, stop.":
    "আপডেট: ১৪ মে ২০২৬। সেশন ছোট ও নিয়ন্ত্রিত রাখুন। ড্রিলে চাপ বা অস্বস্তি হলে থামুন।",
  "Updated May 14, 2026. Start with fewer distractors or a bigger target, and stop if the session becomes uncomfortable.":
    "আপডেট: ১৪ মে ২০২৬। কম বিভ্রান্তিকারী বা বড় লক্ষ্য দিয়ে শুরু করুন, এবং সেশন অস্বস্তিকর হলে থামুন।",
  "Updated May 14, 2026. This is a browser-based practice drill and not medical therapy.":
    "আপডেট: ১৪ মে ২০২৬। এটি ব্রাউজার-ভিত্তিক অনুশীলন ড্রিল, চিকিৎসা থেরাপি নয়।",
  "Updated May 14, 2026. This is practice software, not medical care, so stop if the session feels uncomfortable.":
    "আপডেট: ১৪ মে ২০২৬। এটি অনুশীলনের সফটওয়্যার, চিকিৎসা নয়, তাই সেশন অস্বস্তিকর লাগলে থামুন।",
  "Use FoveaFlow as a free FPS eye training warmup for visual tracking, quick refocus, target switching, and focus under distraction.":
    "ভিজ্যুয়াল ট্র্যাকিং, দ্রুত রিফোকাস, লক্ষ্য বদল এবং বিভ্রান্তির মধ্যে ফোকাসের জন্য FoveaFlow বিনা খরচের FPS চোখের ওয়ার্মআপ হিসেবে ব্যবহার করুন।",
  "Use FoveaFlow as a short FPS warmup, an active screen break, or a focused visual practice session.":
    "FoveaFlow ছোট FPS ওয়ার্মআপ, সক্রিয় স্ক্রিন বিরতি বা ফোকাসড ভিজ্যুয়াল অনুশীলন সেশন হিসেবে ব্যবহার করুন।",
  "Use FoveaFlow as practice software, not medical care. Stop if a session causes eye strain, dizziness, headache, nausea, or discomfort.":
    "FoveaFlow অনুশীলনের সফটওয়্যার হিসেবে ব্যবহার করুন, চিকিৎসা হিসেবে নয়। কোনো সেশনে চোখে চাপ, মাথা ঘোরা, মাথাব্যথা, বমিভাব বা অস্বস্তি হলে থামুন।",
  "Use Lilac Chaser for a short fixation drill, a perceptual reset, or a quick change of pace between more active modes.":
    "ছোট ফিক্সেশন ড্রিল, উপলব্ধির রিসেট, বা বেশি সক্রিয় মোডের মাঝে দ্রুত গতি বদলের জন্য লাইলাক চেজার ব্যবহার করুন।",
  "Use Smooth Pursuit for short browser-based practice, a pre-game warmup, or a focused reset after dense screen work.":
    "ছোট ব্রাউজার-ভিত্তিক অনুশীলন, গেমের আগে ওয়ার্মআপ, বা ঘন স্ক্রিন কাজের পরে ফোকাসড রিসেটের জন্য মসৃণ অনুসরণ ব্যবহার করুন।",
  "Use Smooth Pursuit to follow one moving target, Reaction Jumps to snap focus to new target positions, Multiple Distractions to track the right target through visual clutter, and Lilac Chaser to hold fixation while noticing peripheral change.":
    "একটি চলমান লক্ষ্য অনুসরণে মসৃণ অনুসরণ, নতুন লক্ষ্য অবস্থানে দ্রুত ফোকাসে দ্রুত ফোকাস বদল, ভিজ্যুয়াল ভিড়ের মধ্যে ঠিক লক্ষ্য ট্র্যাক করতে একাধিক বিভ্রান্তি, এবং পারিপার্শ্বিক বদল খেয়াল করতে করতে ফিক্সেশন ধরে রাখতে লাইলাক চেজার ব্যবহার করুন।",
  "Use dark theme": "ডার্ক থিম ব্যবহার করুন",
  "Use forward motion direction": "সামনের দিকের চলন ব্যবহার করুন",
  "Use it as a quick visual warmup or active screen break, not as medical care.":
    "এটি দ্রুত ভিজ্যুয়াল ওয়ার্মআপ বা সক্রিয় স্ক্রিন বিরতি হিসেবে ব্যবহার করুন, চিকিৎসা হিসেবে নয়।",
  "Use it for short practice sessions. Stop if you feel eye strain, dizziness, headache, nausea, or discomfort.":
    "ছোট অনুশীলন সেশনের জন্য এটি ব্যবহার করুন। চোখে চাপ, মাথা ঘোরা, মাথাব্যথা, বমিভাব বা অস্বস্তি হলে থামুন।",
  "Use it when you want a busier, harder tracking task than Smooth Pursuit.":
    "মসৃণ অনুসরণের চেয়ে বেশি ব্যস্ত ও কঠিন ট্র্যাকিং কাজ চাইলে এটি ব্যবহার করুন।",
  "Use it when you want a fixation-focused drill rather than a moving-target tracking session.":
    "চলমান লক্ষ্য ট্র্যাকিং সেশনের বদলে ফিক্সেশন-কেন্দ্রিক ড্রিল চাইলে এটি ব্যবহার করুন।",
  "Use it when you want steady moving-target practice without the extra clutter of other drills.":
    "অন্য ড্রিলের বাড়তি ভিড় ছাড়া স্থিরভাবে চলমান লক্ষ্য অনুশীলন চাইলে এটি ব্যবহার করুন।",
  "Use predictable paths for steady tracking. Use random paths or hard turns when you want more target-search work.":
    "স্থির ট্র্যাকিংয়ের জন্য অনুমানযোগ্য পথ ব্যবহার করুন। বেশি লক্ষ্য খোঁজার কাজ চাইলে random পথ বা hard turns ব্যবহার করুন।",
  "Use safely": "নিরাপদে ব্যবহার করুন",
  "Use slower speeds for clean refocusing. Raise the speed when you want a sharper reaction drill.":
    "পরিষ্কার রিফোকাসের জন্য কম গতি ব্যবহার করুন। বেশি তীক্ষ্ণ প্রতিক্রিয়া ড্রিল চাইলে গতি বাড়ান।",
  "Use the Bounce path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য বাউন্স পথ ব্যবহার করুন।",
  "Use the Circle path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য বৃত্ত পথ ব্যবহার করুন।",
  "Use the Clover path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য ক্লোভার পথ ব্যবহার করুন।",
  "Use the Corner tour path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য চার কোণ ঘোরা পথ ব্যবহার করুন।",
  "Use the Diagonal path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য তির্যক পথ ব্যবহার করুন।",
  "Use the Diamond loop path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য ডায়মন্ড লুপ পথ ব্যবহার করুন।",
  "Use the Down-left sweep path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য নিচে-বামে সুইপ পথ ব্যবহার করুন।",
  "Use the Down-right sweep path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য নিচে-ডানে সুইপ পথ ব্যবহার করুন।",
  "Use the Edge loop path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য প্রান্ত লুপ পথ ব্যবহার করুন।",
  "Use the Ellipse path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য উপবৃত্ত পথ ব্যবহার করুন।",
  "Use the Figure eight path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য আটের আকৃতি পথ ব্যবহার করুন।",
  "Use the Hard turns path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য তীক্ষ্ণ মোড় পথ ব্যবহার করুন।",
  "Use the Horizontal sweep path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য অনুভূমিক সুইপ পথ ব্যবহার করুন।",
  "Use the Hourglass path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য বালুঘড়ি পথ ব্যবহার করুন।",
  "Use the Lissajous path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য Lissajous পথ ব্যবহার করুন।",
  "Use the Random path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য এলোমেলো পথ ব্যবহার করুন।",
  "Use the Stair steps path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য সিঁড়ি ধাপ পথ ব্যবহার করুন।",
  "Use the Vertical sweep path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য উল্লম্ব সুইপ পথ ব্যবহার করুন।",
  "Use the Wave path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য ঢেউ পথ ব্যবহার করুন।",
  "Use the Zigzag path for short smooth pursuit practice.":
    "ছোট মসৃণ অনুসরণ অনুশীলনের জন্য জিগজ্যাগ পথ ব্যবহার করুন।",
  "Use the app in a safe place where you can stop easily. Do not use it while driving, walking around, operating equipment, or doing anything that needs your full attention.":
    "অ্যাপটি এমন নিরাপদ জায়গায় ব্যবহার করুন, যেখানে সহজে থামতে পারবেন। গাড়ি চালানোর সময়, হাঁটাহাঁটির সময়, যন্ত্র চালানোর সময়, বা পুরো মনোযোগ লাগে এমন কোনো কাজের সময় এটি ব্যবহার করবেন না।",
  "Use this drill when Smooth Pursuit feels too clean and you want a more realistic visual-attention challenge.":
    "মসৃণ অনুসরণ খুব পরিষ্কার বা সহজ লাগলে এবং আরও বাস্তবসম্মত ভিজ্যুয়াল মনোযোগের চ্যালেঞ্জ চাইলে এই ড্রিল ব্যবহার করুন।",
  "Use this guide to choose the right FoveaFlow eye trainer drill for visual tracking, quick refocus, peripheral awareness, FPS warmups, or focus under distraction.":
    "ভিজ্যুয়াল ট্র্যাকিং, দ্রুত রিফোকাস, পারিপার্শ্বিক সচেতনতা, FPS ওয়ার্মআপ বা বিভ্রান্তির মধ্যে ফোকাসের জন্য ঠিক FoveaFlow চোখের অনুশীলন ড্রিল বেছে নিতে এই গাইড ব্যবহার করুন।",
  "Vertical Sweep Smooth Pursuit Drill": "উল্লম্ব সুইপ মসৃণ অনুসরণ ড্রিল",
  "Vertical Sweep Smooth Pursuit Eye Training":
    "উল্লম্ব সুইপ মসৃণ অনুসরণ চোখের অনুশীলন",
  "Vertical Sweep mirrors the simplicity of Horizontal Sweep but changes the direction of travel for straightforward up-down tracking.":
    "উল্লম্ব সুইপ, অনুভূমিক সুইপ-এর সরলতা ধরে রাখে, তবে সরাসরি ওপর-নিচ ট্র্যাকিংয়ের জন্য চলনের দিক বদলায়।",
  "Vertical sweep": "উল্লম্ব সুইপ",
  "Viewing distance": "দেখার দূরত্ব",
  "Viewing distance and CSS pixels/cm help speed settings match your display setup more closely.":
    "দেখার দূরত্ব এবং CSS pixels/cm গতি সেটিংকে আপনার ডিসপ্লে সেটআপের সঙ্গে বেশি মিলাতে সাহায্য করে।",
  "Viewing distance and screen scale controls help match motion to your setup.":
    "দেখার দূরত্ব ও স্ক্রিন স্কেল কন্ট্রোল চলনকে আপনার সেটআপের সঙ্গে মিলাতে সাহায্য করে।",
  "Visit BlinkCamp": "BlinkCamp দেখুন",
  "Visit EyeTrainer.gg": "EyeTrainer.gg দেখুন",
  "Visual clutter": "ভিজ্যুয়াল ভিড়",
  "Visual guidance of smooth pursuit eye movements":
    "মসৃণ অনুসরণ চোখের চলনের ভিজ্যুয়াল দিকনির্দেশ",
  "Visual learning in multiple-object tracking":
    "একাধিক বস্তু ট্র্যাকিংয়ে ভিজ্যুয়াল শেখা",
  "Visual practice": "ভিজ্যুয়াল অনুশীলন",
  "Visual tracking": "ভিজ্যুয়াল ট্র্যাকিং",
  Wave: "ওয়েভ",
  "Wave Smooth Pursuit Drill": "ঢেউ মসৃণ অনুসরণ ড্রিল",
  "Wave Smooth Pursuit Eye Training": "ঢেউ মসৃণ অনুসরণ চোখের অনুশীলন",
  "Wave introduces a repeating up-and-down rhythm on top of horizontal movement. It stays readable and smooth without the abrupt feel of hard corners.":
    "ঢেউ আড়াআড়ি চলনের ওপর পুনরাবৃত্ত ওপর-নিচ ছন্দ যোগ করে। কঠিন কোণের হঠাৎ অনুভূতি ছাড়াই এটি বোঝা যায় এবং মসৃণ থাকে।",
  Weight: "ওজন",
  "What FoveaFlow includes": "FoveaFlow-এ কী আছে",
  "What does Multiple Distractions train?":
    "একাধিক বিভ্রান্তি কী অনুশীলন করায়?",
  "What does Reaction Jumps train?": "দ্রুত ফোকাস বদল কী অনুশীলন করায়?",
  "What if the effect feels strange or uncomfortable?":
    "প্রভাবটি অদ্ভুত বা অস্বস্তিকর লাগলে কী করব?",
  "What is FoveaFlow?": "FoveaFlow কী?",
  "What is Lilac Chaser mode?": "লাইলাক চেজার মোড কী?",
  "What is Lilac Chaser?": "লাইলাক চেজার কী?",
  "What is Multiple Distractions mode?": "একাধিক বিভ্রান্তি মোড কী?",
  "What is Reaction Jumps mode?": "দ্রুত ফোকাস বদল মোড কী?",
  "What is Smooth Pursuit mode?": "মসৃণ অনুসরণ মোড কী?",
  "What is Smooth Pursuit?": "মসৃণ অনুসরণ কী?",
  "What is distractor tracking?": "Distractor tracking কী?",
  "What is the Bounce drill?": "বাউন্স ড্রিল কী?",
  "What is the Circle drill?": "বৃত্ত ড্রিল কী?",
  "What is the Clover drill?": "ক্লোভার ড্রিল কী?",
  "What is the Corner tour drill?": "চার কোণ ঘোরা ড্রিল কী?",
  "What is the Diagonal drill?": "তির্যক ড্রিল কী?",
  "What is the Diamond loop drill?": "ডায়মন্ড লুপ ড্রিল কী?",
  "What is the Down-left sweep drill?": "নিচে-বামে সুইপ ড্রিল কী?",
  "What is the Down-right sweep drill?": "নিচে-ডানে সুইপ ড্রিল কী?",
  "What is the Edge loop drill?": "প্রান্ত লুপ ড্রিল কী?",
  "What is the Ellipse drill?": "উপবৃত্ত ড্রিল কী?",
  "What is the Figure eight drill?": "আটের আকৃতি ড্রিল কী?",
  "What is the Hard turns drill?": "তীক্ষ্ণ মোড় ড্রিল কী?",
  "What is the Horizontal sweep drill?": "অনুভূমিক সুইপ ড্রিল কী?",
  "What is the Hourglass drill?": "বালুঘড়ি ড্রিল কী?",
  "What is the Lissajous drill?": "Lissajous ড্রিল কী?",
  "What is the Random drill?": "এলোমেলো ড্রিল কী?",
  "What is the Stair steps drill?": "সিঁড়ি ধাপ ড্রিল কী?",
  "What is the Vertical sweep drill?": "উল্লম্ব সুইপ ড্রিল কী?",
  "What is the Wave drill?": "ঢেউ ড্রিল কী?",
  "What is the Zigzag drill?": "জিগজ্যাগ ড্রিল কী?",
  "What is the goal of this mode?": "এই মোডের লক্ষ্য কী?",
  "What it trains:": "এটি যা অনুশীলন করায়:",
  "What makes the Bounce path different?": "বাউন্স পথ আলাদা কেন?",
  "What makes the Circle path different?": "বৃত্ত পথ আলাদা কেন?",
  "What makes the Clover path different?": "ক্লোভার পথ আলাদা কেন?",
  "What makes the Corner tour path different?": "চার কোণ ঘোরা পথ আলাদা কেন?",
  "What makes the Diagonal path different?": "তির্যক পথ আলাদা কেন?",
  "What makes the Diamond loop path different?": "ডায়মন্ড লুপ পথ আলাদা কেন?",
  "What makes the Down-left sweep path different?":
    "নিচে-বামে সুইপ পথ আলাদা কেন?",
  "What makes the Down-right sweep path different?":
    "নিচে-ডানে সুইপ পথ আলাদা কেন?",
  "What makes the Edge loop path different?": "প্রান্ত লুপ পথ আলাদা কেন?",
  "What makes the Ellipse path different?": "উপবৃত্ত পথ আলাদা কেন?",
  "What makes the Figure eight path different?": "আটের আকৃতি পথ আলাদা কেন?",
  "What makes the Hard turns path different?": "তীক্ষ্ণ মোড় পথ আলাদা কেন?",
  "What makes the Horizontal sweep path different?":
    "অনুভূমিক সুইপ পথ আলাদা কেন?",
  "What makes the Hourglass path different?": "বালুঘড়ি পথ আলাদা কেন?",
  "What makes the Lissajous path different?": "Lissajous পথ আলাদা কেন?",
  "What makes the Random path different?": "এলোমেলো পথ আলাদা কেন?",
  "What makes the Stair steps path different?": "সিঁড়ি ধাপ পথ আলাদা কেন?",
  "What makes the Vertical sweep path different?": "উল্লম্ব সুইপ পথ আলাদা কেন?",
  "What makes the Wave path different?": "ঢেউ পথ আলাদা কেন?",
  "What makes the Zigzag path different?": "জিগজ্যাগ পথ আলাদা কেন?",
  "What settings can I change?": "কোন সেটিং বদলাতে পারি?",
  "What the app is": "অ্যাপটি কী",
  "When it jumps, find the new location and actually focus on it before the next jump.":
    "এটি লাফ দিলে নতুন জায়গাটি খুঁজুন এবং পরের লাফের আগে সত্যি ফোকাস করুন।",
  "When should I choose this over Smooth Pursuit?":
    "মসৃণ অনুসরণের বদলে কখন এটি বেছে নেব?",
  "When should I use Smooth Pursuit?": "মসৃণ অনুসরণ কখন ব্যবহার করব?",
  "When should I use this mode?": "এই মোড কখন ব্যবহার করব?",
  "Which drill is best for fixation and edge-of-vision awareness?":
    "ফিক্সেশন ও দৃষ্টির প্রান্তের সচেতনতার জন্য কোন ড্রিল ভালো?",
  "Which drill is best for quick refocus?":
    "দ্রুত রিফোকাসের জন্য কোন ড্রিল ভালো?",
  "Which drill is best for steady tracking?":
    "স্থির ট্র্যাকিংয়ের জন্য কোন ড্রিল ভালো?",
  "Which drill is best when the screen feels busy?":
    "স্ক্রিন ব্যস্ত লাগলে কোন ড্রিল ভালো?",
  "Which drill set is broader?": "কোন ড্রিল সেট বেশি বিস্তৃত?",
  "Which drill to use": "কোন ড্রিল ব্যবহার করবেন",
  "Which drills are available?": "কোন ড্রিল আছে?",
  "Which path is best for beginners?": "নতুনদের জন্য কোন পথ ভালো?",
  "Which settings matter most?": "কোন সেটিং সবচেয়ে গুরুত্বপূর্ণ?",
  "Which settings should I change first?": "প্রথমে কোন সেটিং বদলাব?",
  Yes: "হ্যাঁ",
  "Yes, but a larger screen gives the moving target more room. A desktop, laptop, or tablet usually feels better for longer paths.":
    "হ্যাঁ, তবে বড় স্ক্রিনে চলমান লক্ষ্য বেশি জায়গা পায়। লম্বা পথের জন্য ডেস্কটপ, ল্যাপটপ বা ট্যাবলেট সাধারণত ভালো লাগে।",
  "Yes. BlinkCamp also links to GitHub.":
    "হ্যাঁ। BlinkCamp-ও GitHub-এ লিংক দেয়।",
  "Yes. BlinkCamp exposes simple speed and size controls.":
    "হ্যাঁ। BlinkCamp সহজ গতি ও সাইজ কন্ট্রোল দেখায়।",
  "Yes. BlinkCamp is also a free browser-based eye training tool.":
    "হ্যাঁ। BlinkCamp-ও বিনা খরচে ব্রাউজার-ভিত্তিক চোখের অনুশীলন টুল।",
  "Yes. EyeTrainer.gg is also positioned around FPS eye training.":
    "হ্যাঁ। EyeTrainer.gg-ও FPS চোখের অনুশীলন ঘিরে তৈরি।",
  "Yes. FoveaFlow includes FPS warmup-friendly drills for tracking, refocus, and focus under clutter.":
    "হ্যাঁ। FoveaFlow-এ ট্র্যাকিং, রিফোকাস এবং ভিড়ের মধ্যে ফোকাসের জন্য FPS ওয়ার্মআপ-উপযোগী ড্রিল আছে।",
  "Yes. FoveaFlow is free to use, with no account, subscription, or paid plan.":
    "হ্যাঁ। FoveaFlow ব্যবহার করতে খরচ নেই; অ্যাকাউন্ট, সাবস্ক্রিপশন বা পেইড প্ল্যান নেই।",
  "Yes. It can be a short warmup for games that reward fast visual reactions.":
    "হ্যাঁ। দ্রুত ভিজ্যুয়াল প্রতিক্রিয়া কাজে লাগে এমন গেমের জন্য এটি ছোট ওয়ার্মআপ হতে পারে।",
  "Yes. It can be useful as a short warmup for games where visual clutter matters.":
    "হ্যাঁ। ভিজ্যুয়াল ভিড় গুরুত্বপূর্ণ এমন গেমের জন্য ছোট ওয়ার্মআপ হিসেবে এটি কাজে লাগতে পারে।",
  "Yes. It gives developers, sysadmins, and support teams a short visual reset between code, logs, dashboards, terminals, tickets, and multiple windows.":
    "হ্যাঁ। কোড, লগ, ড্যাশবোর্ড, টার্মিনাল, টিকিট ও একাধিক উইন্ডোর মাঝে ডেভেলপার, sysadmin এবং support team-কে এটি ছোট ভিজ্যুয়াল রিসেট দেয়।",
  "Yes. It works well as a short visual warmup before games or demanding screen tasks.":
    "হ্যাঁ। গেম বা মনোযোগী স্ক্রিন কাজের আগে ছোট ভিজ্যুয়াল ওয়ার্মআপ হিসেবে এটি ভালো কাজ করে।",
  "Yes. Main modes and Smooth Pursuit patterns have direct URLs.":
    "হ্যাঁ। মূল মোড এবং মসৃণ অনুসরণ প্যাটার্নের সরাসরি URL আছে।",
  "Yes. Speed can be tuned in deg/s, cm/s, or screen/s, and target size can be changed per session.":
    "হ্যাঁ। গতি deg/s, cm/s বা screen/s-এ বদলানো যায়, এবং প্রতিটি সেশনে লক্ষ্যের সাইজ বদলানো যায়।",
  "Yes. The GitHub repository is linked from the app.":
    "হ্যাঁ। অ্যাপ থেকে GitHub repository-র লিংক দেওয়া আছে।",
  "Yes. The app runs in the browser with no account or install.":
    "হ্যাঁ। অ্যাপটি অ্যাকাউন্ট বা ইনস্টল ছাড়া ব্রাউজারে চলে।",
  "Yes. The public page includes simple browser patterns and also promotes a Steam wishlist.":
    "হ্যাঁ। পাবলিক পেজে সহজ ব্রাউজার প্যাটার্ন আছে এবং একটি Steam wishlist-ও প্রচার করা হয়।",
  "Yes. Use FoveaFlow as a quick visual warmup before FPS games or any game where tracking targets and reading movement matters.":
    "হ্যাঁ। FPS গেম বা লক্ষ্য ট্র্যাকিং ও চলন পড়া দরকার এমন যেকোনো গেমের আগে FoveaFlow দ্রুত ভিজ্যুয়াল ওয়ার্মআপ হিসেবে ব্যবহার করুন।",
  "You can adjust the mode, motion path, target size, speed, shape, color, opacity, trail, distractor count, viewing distance, screen scale, and Lilac Chaser size and color.":
    "আপনি মোড, চলনের পথ, লক্ষ্যের সাইজ, গতি, আকৃতি, রং, অস্বচ্ছতা, ট্রেইল, বিভ্রান্তিকারীর সংখ্যা, দেখার দূরত্ব, স্ক্রিন স্কেল এবং লাইলাক চেজারের সাইজ ও রং বদলাতে পারেন।",
  "You can clear saved FoveaFlow settings from your browser's site data controls. You can also use browser or extension settings to block optional analytics scripts.":
    "ব্রাউজারের site data controls থেকে সংরক্ষিত FoveaFlow সেটিং মুছতে পারেন। চাইলে ব্রাউজার বা extension settings দিয়ে ঐচ্ছিক analytics script ব্লক করতে পারেন।",
  "You choose the settings and session length. Keep sessions short if you are unsure, and take breaks.":
    "সেটিং ও সেশনের সময় আপনি বেছে নেন। নিশ্চিত না হলে সেশন ছোট রাখুন এবং বিরতি নিন।",
  "You do not need to create an account to use FoveaFlow. The app does not ask for your name, email address, payment details, or health records.":
    "FoveaFlow ব্যবহার করতে অ্যাকাউন্ট খুলতে হয় না। অ্যাপ আপনার নাম, ইমেল ঠিকানা, পেমেন্ট তথ্য বা স্বাস্থ্য নথি চায় না।",
  "Your choices": "আপনার পছন্দ",
  "Your practice choices are not uploaded to a FoveaFlow account because there are no accounts.":
    "অ্যাকাউন্ট নেই বলে আপনার অনুশীলনের পছন্দ কোনো FoveaFlow অ্যাকাউন্টে আপলোড হয় না।",
  Zigzag: "জিগজ্যাগ",
  "Zigzag Smooth Pursuit Drill": "জিগজ্যাগ মসৃণ অনুসরণ ড্রিল",
  "Zigzag Smooth Pursuit Eye Training": "জিগজ্যাগ মসৃণ অনুসরণ চোখের অনুশীলন",
  "Zigzag adds frequent directional switching, so it feels more aggressive than Wave or Diagonal.":
    "জিগজ্যাগ বারবার দিক বদল যোগ করে, তাই এটি ঢেউ বা তির্যক-এর চেয়ে বেশি আক্রমণাত্মক লাগে।",
  constant: "ধ্রুব",
  forward: "সামনে",
  guide: "গাইড",
  overview: "সারাংশ",
  paused: "বিরত",
  playing: "চলছে",
  reverse: "উল্টো",
} satisfies Record<keyof typeof en, string>;
