import type { en } from "./en";

export const hi = {
  "1 minute Lilac Chaser: hold center focus and notice peripheral change.":
    "1 मिनट लाइलैक चेजर: बीच पर नजर टिकाए रखें और किनारों में बदलाव देखें.",
  "1 minute Multiple Distractions: stay locked on the brightest target.":
    "1 मिनट कई विचलन: सबसे चमकीले लक्ष्य पर नजर बनाए रखें.",
  "2 minutes Reaction Jumps: find each new target position quickly.":
    "2 मिनट रिएक्शन जंप्स: हर नई जगह पर लक्ष्य जल्दी खोजें.",
  "3 minutes Smooth Pursuit: follow the target smoothly.":
    "3 मिनट स्मूद परसूट: लक्ष्य को सहजता से फॉलो करें.",
  "7-minute FPS eye warmup": "7 मिनट का FPS आंख वॉर्मअप",
  "About FoveaFlow": "FoveaFlow के बारे में",
  "About FoveaFlow eye trainer": "FoveaFlow आई ट्रेनर के बारे में",
  "Acceptable use": "स्वीकार्य उपयोग",
  "Adjust the settings without guesswork": "अनुमान लगाए बिना सेटिंग बदलें",
  Agreement: "सहमति",
  "Alternating pace": "बदलती गति",
  Alternative: "विकल्प",
  App: "ऐप",
  "App actions": "ऐप की कार्रवाइयां",
  "Availability and warranty": "उपलब्धता और वारंटी",
  "Ball color": "गेंद का रंग",
  "Best fit": "सबसे सही विकल्प",
  "Best uses for": "सबसे अच्छा उपयोग",
  BlinkCamp: "BlinkCamp",
  "BlinkCamp has its own exercise set and a simpler public workflow.":
    "BlinkCamp का अपना अभ्यास सेट और सरल सार्वजनिक तरीका है.",
  "BlinkCamp is more focused on straightforward eye-training routines than distractor and letter-overlay customization.":
    "BlinkCamp में भटकाने वाली वस्तुओं और अक्षर ओवरले की कस्टम सेटिंग से ज्यादा सीधे आंखों के अभ्यास पर ध्यान है.",
  "BlinkCamp is organized around its own routine interface.":
    "BlinkCamp अपने रूटीन इंटरफेस के हिसाब से बना है.",
  "BlinkCamp keeps setup lighter and does not expose the same viewing-distance and screen-scale controls.":
    "BlinkCamp सेटअप को हल्का रखता है और वही देखने की दूरी या स्क्रीन स्केल के कंट्रोल नहीं देता.",
  "BlinkCamp keeps the public controls simpler, with the main visible controls focused on speed and size.":
    "BlinkCamp सार्वजनिक कंट्रोल सरल रखता है. मुख्य कंट्रोल गति और आकार पर हैं.",
  "BlinkCamp uses a simpler routine-based setup rather than exposing the same path and motion-behavior controls.":
    "BlinkCamp वही रास्ता और गति व्यवहार कंट्रोल देने के बजाय सरल रूटीन आधारित सेटअप इस्तेमाल करता है.",
  Blue: "नीला",
  Bold: "बोल्ड",
  Bounce: "बाउंस",
  "Bounce Smooth Pursuit Drill": "बाउंस स्मूद परसूट अभ्यास",
  "Bounce Smooth Pursuit Eye Training": "बाउंस स्मूद परसूट आंख अभ्यास",
  "Bounce adds repeated reversals at the edges. It is useful when you want more direction changes and less continuous flow than Circle or Wave.":
    "बाउंस किनारों पर बार-बार दिशा बदलता है. सर्कल या वेव से ज्यादा दिशा बदलाव और कम लगातार बहाव चाहिए तो यह काम आता है.",
  "Browser only, no account": "सिर्फ ब्राउजर, कोई खाता नहीं",
  "Build and reset": "बनाएं और रीसेट करें",
  "By using FoveaFlow, you agree to these terms. If you do not agree, do not use the site.":
    "FoveaFlow इस्तेमाल करके आप इन शर्तों से सहमत होते हैं. सहमत नहीं हैं तो साइट इस्तेमाल न करें.",
  "CSS pixels/cm": "CSS पिक्सल/cm",
  "Can FoveaFlow help with tired eyes from screen work?":
    "क्या FoveaFlow स्क्रीन पर काम करने से थकी आंखों में मदद कर सकता है?",
  "Can FoveaFlow improve eyesight or reaction time?":
    "क्या FoveaFlow नजर या प्रतिक्रिया समय सुधार सकता है?",
  "Can I adjust speed and target size?":
    "क्या मैं गति और लक्ष्य का आकार बदल सकता हूं?",
  "Can I calibrate the session to my setup?":
    "क्या मैं सेशन को अपने सेटअप के हिसाब से कैलिब्रेट कर सकता हूं?",
  "Can I change how the target looks?": "क्या मैं लक्ष्य का रूप बदल सकता हूं?",
  "Can I change paths and motion behavior?":
    "क्या मैं रास्ते और गति का व्यवहार बदल सकता हूं?",
  "Can I link straight to a drill?": "क्या मैं सीधे किसी अभ्यास का लिंक दे सकता हूं?",
  "Can I start in the browser?": "क्या मैं ब्राउजर में शुरू कर सकता हूं?",
  "Can I train with distractors or letters?":
    "क्या मैं भटकाने वाली वस्तुओं या अक्षरों के साथ अभ्यास कर सकता हूं?",
  "Can I tune target appearance and calibration?":
    "क्या मैं लक्ष्य का रूप और कैलिब्रेशन बदल सकता हूं?",
  "Can I use FoveaFlow on a phone?": "क्या मैं FoveaFlow फोन पर इस्तेमाल कर सकता हूं?",
  "Can I use it free in the browser?":
    "क्या मैं इसे ब्राउजर में मुफ्त इस्तेमाल कर सकता हूं?",
  "Change language": "भाषा बदलें",
  "Change speed and target size first. They usually have the biggest effect on difficulty and control.":
    "पहले गति और लक्ष्य का आकार बदलें. कठिनाई और कंट्रोल पर आम तौर पर इन्हीं का सबसे ज्यादा असर होता है.",
  "Change your saved FoveaFlow settings.": "अपनी सेव की गई FoveaFlow सेटिंग बदलें.",
  "Changes to these terms": "इन शर्तों में बदलाव",
  Children: "बच्चे",
  "Choose BlinkCamp if you want a simple browser tool centered on basic eye-training routines and speed-and-size adjustment.":
    "अगर आपको बुनियादी आंख अभ्यास, गति और आकार सेटिंग वाला सरल ब्राउजर टूल चाहिए तो BlinkCamp चुनें.",
  "Choose FoveaFlow if you want a lightweight web app with direct access to visual tracking, reaction, distractor, and fixation drills.":
    "अगर आपको विजुअल ट्रैकिंग, प्रतिक्रिया, विचलन और नजर टिकाने वाले अभ्यास सीधे चाहिए तो FoveaFlow चुनें.",
  "Choose FoveaFlow when you want direct drill links, adjustable target behavior, and a clean full-screen canvas for short visual tracking sessions.":
    "सीधे अभ्यास लिंक, बदल सकने वाला लक्ष्य व्यवहार और छोटे विजुअल ट्रैकिंग सेशन के लिए साफ फुल स्क्रीन कैनवास चाहिए तो FoveaFlow चुनें.",
  "Choose Reaction Jumps when you want discrete target changes rather than continuous motion.":
    "लगातार गति के बजाय अलग-अलग लक्ष्य बदलाव चाहिए तो रिएक्शन जंप्स चुनें.",
  "Choose a drill by the result you want":
    "जो नतीजा चाहिए, उसके हिसाब से अभ्यास चुनें",
  "Choose a path, set the speed and target style, then use it for a short visual tracking session.":
    "रास्ता चुनें, गति और लक्ष्य शैली सेट करें, फिर छोटे विजुअल ट्रैकिंग सेशन में इस्तेमाल करें.",
  "Choose an easy path when you want rhythm and control. Choose a harder path when you want more direction changes and more target-search demand.":
    "लय और कंट्रोल चाहिए तो आसान रास्ता चुनें. ज्यादा दिशा बदलाव और लक्ष्य खोजने की मेहनत चाहिए तो कठिन रास्ता चुनें.",
  Circle: "सर्कल",
  "Circle Smooth Pursuit Drill": "सर्कल स्मूद परसूट अभ्यास",
  "Circle Smooth Pursuit Eye Training": "सर्कल स्मूद परसूट आंख अभ्यास",
  "Circle is the easiest repeating Smooth Pursuit pattern and the best starting point for most users. The loop is predictable, which makes it useful for warmups and speed changes.":
    "सर्कल सबसे आसान दोहराया जाने वाला स्मूद परसूट पैटर्न है और ज्यादातर लोगों के लिए अच्छी शुरुआत है. लूप का अंदाजा पहले से लग जाता है, इसलिए यह वॉर्मअप और गति बदलने के लिए अच्छा है.",
  Close: "बंद करें",
  "Cloudflare Web Analytics": "Cloudflare Web Analytics",
  "Cloudflare Web Analytics may load a small beacon script. Cloudflare says Web Analytics measures page views and performance without collecting or using visitor personal data.":
    "Cloudflare Web Analytics एक छोटा बीकन स्क्रिप्ट लोड कर सकता है. Cloudflare के अनुसार Web Analytics विजिटर का निजी डेटा इकट्ठा या इस्तेमाल किए बिना पेज व्यू और प्रदर्शन मापता है.",
  "Cloudflare cookie reference": "Cloudflare कुकी संदर्भ",
  "Cloudflare data collection docs": "Cloudflare डेटा संग्रह दस्तावेज",
  "Cloudflare hosting and analytics": "Cloudflare होस्टिंग और एनालिटिक्स",
  Clover: "क्लोवर",
  "Clover Smooth Pursuit Drill": "क्लोवर स्मूद परसूट अभ्यास",
  "Clover Smooth Pursuit Eye Training": "क्लोवर स्मूद परसूट आंख अभ्यास",
  "Clover creates repeated looping lobes for continuous motion with more shape variation than Circle.":
    "क्लोवर लगातार गति के लिए दोहराए जाने वाले लूप बनाता है. इसमें सर्कल से ज्यादा आकार बदलाव होता है.",
  "Compare FoveaFlow and BlinkCamp for free browser-based eye training, visual tracking, FPS warmups, reaction jumps, and distractor tracking.":
    "मुफ्त ब्राउजर आधारित आंख अभ्यास, विजुअल ट्रैकिंग, FPS वॉर्मअप, रिएक्शन जंप्स और विचलन ट्रैकिंग के लिए FoveaFlow और BlinkCamp की तुलना करें.",
  "Compare FoveaFlow and EyeTrainer.gg for FPS eye training, visual tracking, reaction drills, distraction control, and browser-based warmups.":
    "FPS आंख अभ्यास, विजुअल ट्रैकिंग, प्रतिक्रिया अभ्यास, विचलन कंट्रोल और ब्राउजर आधारित वॉर्मअप के लिए FoveaFlow और EyeTrainer.gg की तुलना करें.",
  "Compare all drills": "सभी अभ्यासों की तुलना करें",
  "Compare drills": "अभ्यासों की तुलना करें",
  Comparison: "तुलना",
  Contact: "संपर्क",
  "Control sections": "कंट्रोल सेक्शन",
  Controls: "कंट्रोल",
  "Controls for speed units, target size, shape, color, opacity, trail length, motion path, motion behavior, distractor count, distractor brightness, letter overlays, viewing distance, and screen scale.":
    "गति इकाई, लक्ष्य आकार, आकार प्रकार, रंग, अपारदर्शिता, ट्रेल लंबाई, गति रास्ता, गति व्यवहार, विचलन संख्या, विचलन चमक, अक्षर ओवरले, देखने की दूरी और स्क्रीन स्केल के कंट्रोल.",
  "Controls for speed units, target size, shape, color, opacity, trail length, motion path, motion behavior, distractors, letter overlays, viewing distance, screen scale, and mode-specific options such as Lilac Chaser ball scale.":
    "गति इकाई, लक्ष्य आकार, आकार प्रकार, रंग, अपारदर्शिता, ट्रेल लंबाई, गति रास्ता, गति व्यवहार, विचलन, अक्षर ओवरले, देखने की दूरी, स्क्रीन स्केल और लाइलैक चेजर गेंद स्केल जैसे मोड विकल्पों के कंट्रोल.",
  Cookies: "कुकी",
  "Corner Tour Smooth Pursuit Drill": "कॉर्नर टूर स्मूद परसूट अभ्यास",
  "Corner Tour Smooth Pursuit Eye Training": "कॉर्नर टूर स्मूद परसूट आंख अभ्यास",
  "Corner Tour gives each corner of the display a deliberate role, making the route spacious and structured.":
    "कॉर्नर टूर स्क्रीन के हर कोने को साफ भूमिका देता है. रास्ता खुला और व्यवस्थित लगता है.",
  "Corner tour": "कॉर्नर टूर",
  Cross: "क्रॉस",
  Cyan: "सियान",
  "Dark mode": "डार्क मोड",
  "Data is used to run the site, keep it secure, understand whether pages load correctly, and see which public pages people use. FoveaFlow does not sell visitor data.":
    "डेटा साइट चलाने, उसे सुरक्षित रखने, पेज सही लोड हो रहे हैं या नहीं समझने और लोग कौन से सार्वजनिक पेज इस्तेमाल करते हैं यह देखने के लिए इस्तेमाल होता है. FoveaFlow विजिटर डेटा नहीं बेचता.",
  "Data we do not collect": "जो डेटा हम इकट्ठा नहीं करते",
  Defaults: "डिफॉल्ट",
  Details: "विवरण",
  Diagonal: "तिरछा",
  "Diagonal Smooth Pursuit Drill": "तिरछा स्मूद परसूट अभ्यास",
  "Diagonal Smooth Pursuit Eye Training": "तिरछा स्मूद परसूट आंख अभ्यास",
  "Diagonal uses longer corner-to-corner motion, so it emphasizes broader screen coverage and clean tracking over distance.":
    "तिरछा लंबे कोने से कोने तक चलने वाला रास्ता इस्तेमाल करता है. इससे ज्यादा स्क्रीन कवरेज और दूरी पर साफ ट्रैकिंग पर ध्यान जाता है.",
  Diamond: "डायमंड",
  "Diamond Loop Smooth Pursuit Drill": "डायमंड लूप स्मूद परसूट अभ्यास",
  "Diamond Loop Smooth Pursuit Eye Training": "डायमंड लूप स्मूद परसूट आंख अभ्यास",
  "Diamond Loop combines a simple repeating route with clear corner transitions and sharper shifts than Circle.":
    "डायमंड लूप सरल दोहराए जाने वाले रास्ते को साफ कोना बदलावों से जोड़ता है. इसमें सर्कल से तेज बदलाव होते हैं.",
  "Diamond loop": "डायमंड लूप",
  "Direct routes": "सीधे रास्ते",
  Direction: "दिशा",
  "Distractor Tracking Eye Training": "विचलन ट्रैकिंग आंख अभ्यास",
  "Distractor color": "विचलन का रंग",
  "Distractor color brightness": "विचलन रंग की चमक",
  Distractors: "विचलन",
  "Do I need an account or app install?": "क्या मुझे खाता या ऐप इंस्टॉल चाहिए?",
  "Do I need an account or install?": "क्या मुझे खाता या इंस्टॉल चाहिए?",
  "Do not attack, overload, scrape aggressively, or try to gain unauthorized access to the site or its infrastructure.":
    "साइट या उसके इंफ्रास्ट्रक्चर पर हमला, ओवरलोड, तेज स्क्रैपिंग या बिना अनुमति एक्सेस की कोशिश न करें.",
  "Do not follow the balls with your eyes.": "गेंदों को आंखों से फॉलो न करें.",
  "Down-left Sweep Smooth Pursuit Drill": "नीचे-बाएं स्वीप स्मूद परसूट अभ्यास",
  "Down-left Sweep Smooth Pursuit Eye Training":
    "नीचे-बाएं स्वीप स्मूद परसूट आंख अभ्यास",
  "Down-left Sweep moves from the top-right corner toward the bottom-left corner on a simple diagonal line.":
    "नीचे-बाएं स्वीप सरल तिरछी रेखा पर ऊपर-दाएं कोने से नीचे-बाएं कोने की ओर चलता है.",
  "Down-left sweep": "नीचे-बाएं स्वीप",
  "Down-right Sweep Smooth Pursuit Drill": "नीचे-दाएं स्वीप स्मूद परसूट अभ्यास",
  "Down-right Sweep Smooth Pursuit Eye Training":
    "नीचे-दाएं स्वीप स्मूद परसूट आंख अभ्यास",
  "Down-right Sweep moves from the top-left corner toward the bottom-right corner on a simple diagonal line.":
    "नीचे-दाएं स्वीप सरल तिरछी रेखा पर ऊपर-बाएं कोने से नीचे-दाएं कोने की ओर चलता है.",
  "Down-right sweep": "नीचे-दाएं स्वीप",
  Drill: "अभ्यास",
  Drills: "अभ्यास",
  "Edge Loop Smooth Pursuit Drill": "किनारा लूप स्मूद परसूट अभ्यास",
  "Edge Loop Smooth Pursuit Eye Training": "किनारा लूप स्मूद परसूट आंख अभ्यास",
  "Edge Loop pushes the target around the perimeter, making the drill more spacious and edge-focused than center-heavy loops.":
    "किनारा लूप लक्ष्य को किनारों के आसपास चलाता है. यह केंद्र वाले लूप से ज्यादा खुला और किनारा केंद्रित लगता है.",
  "Edge loop": "किनारा लूप",
  Ellipse: "दीर्घवृत्त",
  "Ellipse Smooth Pursuit Drill": "दीर्घवृत्त स्मूद परसूट अभ्यास",
  "Ellipse Smooth Pursuit Eye Training": "दीर्घवृत्त स्मूद परसूट आंख अभ्यास",
  "Ellipse keeps the calm rhythm of Circle but changes the width and height of the motion. It is a good bridge pattern when you want a familiar loop with more range.":
    "दीर्घवृत्त में सर्कल जैसी शांत लय रहती है, लेकिन गति की चौड़ाई और ऊंचाई बदलती है. जाना-पहचाना लूप और ज्यादा दायरा चाहिए तो यह अच्छा बीच का पैटर्न है.",
  "EyeTrainer.gg": "EyeTrainer.gg",
  "EyeTrainer.gg also offers simple browser patterns and promotes its upcoming Steam version from the public page.":
    "EyeTrainer.gg भी सरल ब्राउजर पैटर्न देता है और सार्वजनिक पेज से अपने आने वाले Steam संस्करण को दिखाता है.",
  "EyeTrainer.gg focuses the public browser tool on simple pattern practice rather than detailed target and display calibration.":
    "EyeTrainer.gg का सार्वजनिक ब्राउजर टूल विस्तार वाली लक्ष्य और डिस्प्ले कैलिब्रेशन के बजाय सरल पैटर्न अभ्यास पर केंद्रित है.",
  FAQ: "सवाल-जवाब",
  "FPS Eye Training Warmup": "FPS आंख अभ्यास वॉर्मअप",
  "FPS Eye Training Warmup (HIGH FPS)": "FPS आंख अभ्यास वॉर्मअप (हाई FPS)",
  "FPS warmup": "FPS वॉर्मअप",
  "Feature comparison": "फीचर तुलना",
  "Figure Eight Smooth Pursuit Drill": "फिगर आठ स्मूद परसूट अभ्यास",
  "Figure Eight Smooth Pursuit Eye Training": "फिगर आठ स्मूद परसूट आंख अभ्यास",
  "Figure Eight adds a crossover point, which means the target passes through the center and changes direction more often than a simple loop.":
    "फिगर आठ में एक क्रॉसओवर बिंदु होता है. लक्ष्य बीच से गुजरता है और सरल लूप से ज्यादा बार दिशा बदलता है.",
  "Figure eight": "फिगर आठ",
  "Focused guides for FPS and alternatives": "FPS और विकल्पों के लिए केंद्रित गाइड",
  "Follow it like Smooth Pursuit, but do not let the darker balls pull your eyes away.":
    "इसे स्मूद परसूट की तरह फॉलो करें, लेकिन गहरी गेंदों को आपकी नजर भटकाने न दें.",
  "Follow one moving target and train steady visual tracking.":
    "एक चलते लक्ष्य को फॉलो करें और स्थिर विजुअल ट्रैकिंग का अभ्यास करें.",
  "For project questions, use the GitHub repository. Do not post private information in a public issue.":
    "प्रोजेक्ट से जुड़े सवालों के लिए GitHub रिपॉजिटरी इस्तेमाल करें. सार्वजनिक इश्यू में निजी जानकारी न डालें.",
  "FoveaFlow Guide": "FoveaFlow गाइड",
  "FoveaFlow and BlinkCamp are both free browser-based eye training tools. FoveaFlow is the stronger fit when you want direct drill links, FPS warmup use, reaction jumps, distractor tracking, and deeper control over how the target moves and appears.":
    "FoveaFlow और BlinkCamp दोनों मुफ्त ब्राउजर आधारित आंख अभ्यास टूल हैं. सीधे अभ्यास लिंक, FPS वॉर्मअप, रिएक्शन जंप्स, विचलन ट्रैकिंग और लक्ष्य कैसे चले या दिखे इस पर ज्यादा कंट्रोल चाहिए तो FoveaFlow बेहतर फिट है.",
  "FoveaFlow can be a short active break during long screen sessions. If screen use causes pain, dizziness, headaches, or ongoing symptoms, stop and get professional advice.":
    "लंबे स्क्रीन सेशन के बीच FoveaFlow छोटा सक्रिय ब्रेक हो सकता है. स्क्रीन इस्तेमाल से दर्द, चक्कर, सिरदर्द या लगातार लक्षण हों तो रुकें और विशेषज्ञ सलाह लें.",
  "FoveaFlow can be used without sending personal details. It is not built to collect personal information from children.":
    "FoveaFlow निजी जानकारी भेजे बिना इस्तेमाल किया जा सकता है. यह बच्चों की निजी जानकारी इकट्ठा करने के लिए नहीं बना है.",
  "FoveaFlow does not set advertising cookies. Cloudflare may set security cookies when it needs them to keep the site available and safe.":
    "FoveaFlow विज्ञापन कुकी सेट नहीं करता. साइट उपलब्ध और सुरक्षित रखने के लिए जरूरत पड़ने पर Cloudflare सुरक्षा कुकी सेट कर सकता है.",
  "FoveaFlow eye trainer animation for visual tracking practice":
    "विजुअल ट्रैकिंग अभ्यास के लिए FoveaFlow आई ट्रेनर एनिमेशन",
  "FoveaFlow eye trainer animation for visual tracking practice. Use Pause motion to stop target movement before changing controls.":
    "विजुअल ट्रैकिंग अभ्यास के लिए FoveaFlow आई ट्रेनर एनिमेशन. कंट्रोल बदलने से पहले लक्ष्य की गति रोकने के लिए गति रोकें इस्तेमाल करें.",
  "FoveaFlow eye trainer app": "FoveaFlow आई ट्रेनर ऐप",
  "FoveaFlow guide": "FoveaFlow गाइड",
  "FoveaFlow home": "FoveaFlow होम",
  "FoveaFlow is a free browser tool for visual tracking practice. It shows moving targets, reaction jumps, Lilac Chaser fixation practice, and distractor tracking patterns on a screen.":
    "FoveaFlow विजुअल ट्रैकिंग अभ्यास के लिए मुफ्त ब्राउजर टूल है. यह स्क्रीन पर चलते लक्ष्य, रिएक्शन जंप्स, लाइलैक चेजर नजर टिकाने का अभ्यास और विचलन ट्रैकिंग पैटर्न दिखाता है.",
  "FoveaFlow is a free browser tool. Use it safely, stop if it feels bad, and do not treat it as medical care.":
    "FoveaFlow मुफ्त ब्राउजर टूल है. इसे सुरक्षित तरीके से इस्तेमाल करें, असहज लगे तो रुकें और इसे चिकित्सा देखभाल न मानें.",
  "FoveaFlow is a free browser-based FPS eye training warmup. Use Smooth Pursuit for moving-target tracking, Reaction Jumps for quick refocus, and Multiple Distractions for staying locked on the right target through visual clutter.":
    "FoveaFlow मुफ्त ब्राउजर आधारित FPS आंख अभ्यास वॉर्मअप है. चलते लक्ष्य की ट्रैकिंग के लिए स्मूद परसूट, जल्दी दोबारा फोकस के लिए रिएक्शन जंप्स और दृश्य भीड़ में सही लक्ष्य पर टिके रहने के लिए कई विचलन इस्तेमाल करें.",
  "FoveaFlow is a free browser-based eye trainer for FPS warmups, visual tracking, reaction jumps, distractor tracking, and peripheral awareness. It is a practical alternative if you want to start in the browser with no account or install.":
    "FoveaFlow FPS वॉर्मअप, विजुअल ट्रैकिंग, रिएक्शन जंप्स, विचलन ट्रैकिंग और किनारी जागरूकता के लिए मुफ्त ब्राउजर आधारित आई ट्रेनर है. बिना खाता या इंस्टॉल ब्राउजर में शुरू करना हो तो यह व्यावहारिक विकल्प है.",
  "FoveaFlow is a free online eye trainer for visual tracking, quick refocus, peripheral awareness, and focus under distraction. It runs in the browser with no account or install.":
    "FoveaFlow विजुअल ट्रैकिंग, जल्दी दोबारा फोकस, किनारी जागरूकता और विचलन के बीच फोकस के लिए मुफ्त ऑनलाइन आई ट्रेनर है. यह बिना खाता या इंस्टॉल ब्राउजर में चलता है.",
  "FoveaFlow is a free online eye training app for visual tracking, focus, reaction speed, and peripheral awareness. It includes Smooth Pursuit, Reaction Jumps, Lilac Chaser, and distractor tracking with no account or install.":
    "FoveaFlow विजुअल ट्रैकिंग, फोकस, प्रतिक्रिया गति और किनारी जागरूकता के लिए मुफ्त ऑनलाइन आंख अभ्यास ऐप है. इसमें बिना खाता या इंस्टॉल स्मूद परसूट, रिएक्शन जंप्स, लाइलैक चेजर और विचलन ट्रैकिंग शामिल हैं.",
  "FoveaFlow is built to work without an account. The app keeps your settings in your browser and uses Cloudflare to serve the site.":
    "FoveaFlow बिना खाते के चलने के लिए बना है. ऐप आपकी सेटिंग ब्राउजर में रखता है और साइट देने के लिए Cloudflare इस्तेमाल करता है.",
  "FoveaFlow is free to use, requires no account or install, and stores settings locally in your browser.":
    "FoveaFlow मुफ्त है, खाते या इंस्टॉल की जरूरत नहीं, और सेटिंग आपके ब्राउजर में स्थानीय रूप से सेव रहती हैं.",
  "FoveaFlow is free to use. There is no account, paid plan, subscription, or in-app purchase.":
    "FoveaFlow मुफ्त है. कोई खाता, पेड प्लान, सब्सक्रिप्शन या इन-ऐप खरीद नहीं है.",
  "FoveaFlow is free. No account, no install.":
    "FoveaFlow मुफ्त है. खाता नहीं, इंस्टॉल नहीं.",
  "FoveaFlow is free. No account, no paid plan.":
    "FoveaFlow मुफ्त है. खाता नहीं, पेड प्लान नहीं.",
  "FoveaFlow is not medical advice, diagnosis, treatment, vision therapy, or a medical device. It does not replace an optometrist, ophthalmologist, doctor, therapist, or other qualified professional.":
    "FoveaFlow चिकित्सा सलाह, निदान, उपचार, विजन थेरेपी या चिकित्सा उपकरण नहीं है. यह ऑप्टोमेट्रिस्ट, नेत्र रोग विशेषज्ञ, डॉक्टर, थेरेपिस्ट या किसी योग्य पेशेवर की जगह नहीं लेता.",
  "FoveaFlow may help you train visual skills like tracking, refocusing, peripheral awareness, processing speed, and reaction timing. Results vary, and it is not a replacement for professional care if you have an eye condition or ongoing symptoms.":
    "FoveaFlow ट्रैकिंग, दोबारा फोकस, किनारी जागरूकता, प्रोसेसिंग गति और प्रतिक्रिया समय जैसे विजुअल कौशल का अभ्यास कराने में मदद कर सकता है. नतीजे अलग-अलग होंगे, और आंख की समस्या या लगातार लक्षण हों तो यह पेशेवर देखभाल की जगह नहीं है.",
  "FoveaFlow on GitHub": "GitHub पर FoveaFlow",
  "FoveaFlow overview": "FoveaFlow ओवरव्यू",
  "FoveaFlow saves local controls for speed, size, shape, color, opacity, trails, paths, distractors, letters, and display scale.":
    "FoveaFlow गति, आकार, आकार प्रकार, रंग, अपारदर्शिता, ट्रेल, रास्ते, विचलन, अक्षर और डिस्प्ले स्केल के स्थानीय कंट्रोल सेव करता है.",
  "FoveaFlow stores settings locally in your browser so the app can remember them on the current device. That can include the selected language, mode, motion pattern, speed, target size, color, opacity, trail setting, viewing distance, screen scale, and theme.":
    "FoveaFlow सेटिंग आपके ब्राउजर में स्थानीय रूप से रखता है ताकि ऐप उन्हें इसी डिवाइस पर याद रख सके. इनमें चुनी हुई भाषा, मोड, गति पैटर्न, गति, लक्ष्य आकार, रंग, अपारदर्शिता, ट्रेल सेटिंग, देखने की दूरी, स्क्रीन स्केल और थीम शामिल हो सकते हैं.",
  "FoveaFlow stores your language preference in local browser storage and in a same-site preference cookie so the correct language can be selected before the app starts. The cookie can last up to 400 days, uses SameSite=Lax, and is marked Secure on HTTPS.":
    "FoveaFlow आपकी भाषा की पसंद को ब्राउज़र के स्थानीय स्टोरेज और उसी साइट की preference cookie में रखता है, ताकि ऐप शुरू होने से पहले सही भाषा चुनी जा सके. कुकी 400 दिनों तक रह सकती है, SameSite=Lax का उपयोग करती है और HTTPS पर Secure के रूप में चिह्नित होती है.",
  "FoveaFlow vs BlinkCamp": "FoveaFlow बनाम BlinkCamp",
  "FoveaFlow vs EyeTrainer.gg": "FoveaFlow बनाम EyeTrainer.gg",
  "Free access": "मुफ्त पहुंच",
  "Free browser tool": "मुफ्त ब्राउजर टूल",
  "Free online eye trainer": "मुफ्त ऑनलाइन आई ट्रेनर",
  "Free online eye trainer for quick refocus, target acquisition, and fast visual reaction practice. Browser-based with no account or install.":
    "जल्दी दोबारा फोकस, लक्ष्य पकड़ने और तेज विजुअल प्रतिक्रिया अभ्यास के लिए मुफ्त ऑनलाइन आई ट्रेनर. बिना खाता या इंस्टॉल, ब्राउजर आधारित.",
  "Free online eye trainer for selective attention, distractor tracking, and focus under visual clutter. Browser-based with no account or install.":
    "चयनात्मक ध्यान, विचलन ट्रैकिंग और दृश्य भीड़ में फोकस के लिए मुफ्त ऑनलाइन आई ट्रेनर. बिना खाता या इंस्टॉल, ब्राउजर आधारित.",
  "Free online eye trainer for smooth pursuit, visual tracking, and steady moving-target practice. Browser-based with no account or install.":
    "स्मूद परसूट, विजुअल ट्रैकिंग और स्थिर चलते लक्ष्य अभ्यास के लिए मुफ्त ऑनलाइन आई ट्रेनर. बिना खाता या इंस्टॉल, ब्राउजर आधारित.",
  "Free online eye trainer for steady fixation, peripheral awareness, and noticing change outside center focus. Browser-based with no account or install.":
    "स्थिर नजर, किनारी जागरूकता और केंद्र से बाहर बदलाव देखने के लिए मुफ्त ऑनलाइन आई ट्रेनर. बिना खाता या इंस्टॉल, ब्राउजर आधारित.",
  "Game awareness": "गेम जागरूकता",
  "Gamer warm-up": "गेमर वॉर्मअप",
  Gamers: "गेमर",
  "Give tired screen eyes a quick active break after reading, meetings, or too many tabs.":
    "रीडिंग, मीटिंग या बहुत सारे टैब के बाद थकी आंखों को छोटा सक्रिय ब्रेक दें.",
  Gold: "सुनहरा",
  "Guide FAQ": "गाइड सवाल-जवाब",
  "Guide navigation": "गाइड नेविगेशन",
  "Hard Turns Smooth Pursuit Drill": "हार्ड टर्न्स स्मूद परसूट अभ्यास",
  "Hard Turns Smooth Pursuit Eye Training": "हार्ड टर्न्स स्मूद परसूट आंख अभ्यास",
  "Hard Turns is one of the most demanding Smooth Pursuit patterns because the target changes direction abruptly.":
    "हार्ड टर्न्स सबसे कठिन स्मूद परसूट पैटर्न में से है, क्योंकि लक्ष्य अचानक दिशा बदलता है.",
  "Hard turns": "हार्ड टर्न्स",
  "Header target size": "हेडर लक्ष्य आकार",
  "Header target speed": "हेडर लक्ष्य गति",
  Heavy: "गाढ़ा",
  "Hold steady at the center and notice change around it.":
    "बीच में नजर टिकाए रखें और आसपास बदलाव देखें.",
  "Hold the right target even when the screen gets busy.":
    "स्क्रीन व्यस्त लगे तब भी सही लक्ष्य पर टिके रहें.",
  "Horizontal Sweep Smooth Pursuit Drill": "क्षैतिज स्वीप स्मूद परसूट अभ्यास",
  "Horizontal Sweep Smooth Pursuit Eye Training":
    "क्षैतिज स्वीप स्मूद परसूट आंख अभ्यास",
  "Horizontal Sweep keeps the motion simple and broad for left-right tracking across a larger visual range.":
    "क्षैतिज स्वीप गति को सरल और चौड़ा रखता है, ताकि बड़े दृश्य दायरे में बाएं-दाएं ट्रैकिंग हो सके.",
  "Horizontal sweep": "क्षैतिज स्वीप",
  Hourglass: "आवरग्लास",
  "Hourglass Smooth Pursuit Drill": "आवरग्लास स्मूद परसूट अभ्यास",
  "Hourglass Smooth Pursuit Eye Training": "आवरग्लास स्मूद परसूट आंख अभ्यास",
  "Hourglass narrows through the middle and opens back out, creating repeated crossing behavior with a constrained shape.":
    "आवरग्लास बीच में संकरा होता है और फिर खुलता है. इससे सीमित आकार में बार-बार क्रॉसिंग बनती है.",
  "How data is used": "डेटा कैसे इस्तेमाल होता है",
  "How do I make it easier?": "इसे आसान कैसे करूं?",
  "How do I make it harder?": "इसे कठिन कैसे करूं?",
  "How do I make this pattern easier?": "इस पैटर्न को आसान कैसे करूं?",
  "How do I make this pattern harder?": "इस पैटर्न को कठिन कैसे करूं?",
  "How each drill works": "हर अभ्यास कैसे काम करता है",
  "How long should a session be?": "सेशन कितना लंबा होना चाहिए?",
  "How much can I customize?": "मैं कितना कस्टमाइज कर सकता हूं?",
  "How should beginners start?": "शुरुआती लोग कैसे शुरू करें?",
  "How to use": "कैसे इस्तेमाल करें",
  "IT professionals": "IT पेशेवर",
  "If JavaScript is turned off, the moving target app will not run. The guide and policy pages still work as normal pages.":
    "JavaScript बंद हो तो चलता लक्ष्य वाला ऐप नहीं चलेगा. गाइड और नीति पेज सामान्य पेज की तरह चलते रहेंगे.",
  "If you have a vision condition, recent eye injury, surgery, neurological symptoms, or any concern about using moving visual targets, ask a qualified professional before using the app.":
    "अगर आंखों की समस्या, हाल की आंख चोट, सर्जरी, न्यूरोलॉजिकल लक्षण या चलते विजुअल लक्ष्य इस्तेमाल करने को लेकर कोई चिंता है, तो ऐप इस्तेमाल करने से पहले योग्य पेशेवर से पूछें.",
  "Is FoveaFlow free?": "क्या FoveaFlow मुफ्त है?",
  "Is FoveaFlow good for gamers?": "क्या FoveaFlow गेमरों के लिए अच्छा है?",
  "Is FoveaFlow useful for IT professionals?":
    "क्या FoveaFlow IT पेशेवरों के लिए उपयोगी है?",
  "Is Reaction Jumps useful before games?": "क्या रिएक्शन जंप्स गेम से पहले उपयोगी है?",
  "Is Smooth Pursuit good as a warmup?": "क्या स्मूद परसूट वॉर्मअप के लिए अच्छा है?",
  "Is it aimed at FPS eye training?": "क्या यह FPS आंख अभ्यास के लिए है?",
  "Is there a public source link?": "क्या सार्वजनिक सोर्स लिंक है?",
  "Is this a medical tool?": "क्या यह चिकित्सा टूल है?",
  "Is this good for gamers?": "क्या यह गेमरों के लिए अच्छा है?",
  "Is this meant to improve eyesight?": "क्या यह नजर सुधारने के लिए है?",
  "Is this medical treatment?": "क्या यह चिकित्सा उपचार है?",
  "Is this the same as medical peripheral-vision therapy?":
    "क्या यह मेडिकल पेरिफेरल विजन थेरेपी जैसा है?",
  "It is a Smooth Pursuit pattern page that loads the matching path so you can start that style of moving-target practice immediately.":
    "यह स्मूद परसूट पैटर्न पेज है जो मेल खाता रास्ता लोड करता है, ताकि आप तुरंत उसी शैली का चलते लक्ष्य अभ्यास शुरू कर सकें.",
  "It is the task of following the correct target while similar moving objects compete for your attention.":
    "इसमें मिलती-जुलती चलती वस्तुओं के बीच सही लक्ष्य को फॉलो करना होता है.",
  "It trains selective attention, target identity, and steady tracking under clutter.":
    "यह दृश्य भीड़ में चयनात्मक ध्यान, लक्ष्य पहचान और स्थिर ट्रैकिंग का अभ्यास कराता है.",
  "Keep it short": "इसे छोटा रखें",
  "Keep sessions short and deliberate. The goal is focused practice, not pushing through discomfort.":
    "सेशन छोटे और सोच-समझकर रखें. लक्ष्य केंद्रित अभ्यास है, असुविधा सहते रहना नहीं.",
  "Keep the safety line clear": "सुरक्षा सीमा साफ रखें",
  "Keep your head still and let your eyes do the work.":
    "सिर स्थिर रखें और आंखों को काम करने दें.",
  "Keep your head still and lock onto the main, brightest ball.":
    "सिर स्थिर रखें और मुख्य, सबसे चमकीली गेंद पर टिके रहें.",
  "Keep your head still and start with your eyes on the ball.":
    "सिर स्थिर रखें और नजर गेंद पर रखकर शुरू करें.",
  Language: "भाषा",
  "Legal pages": "कानूनी पेज",
  "Let the disappearing gap move around the fixed circle. With steady focus, the colored balls may fade and the missing spot can look like a moving green afterimage.":
    "गायब होती खाली जगह को स्थिर गोल घेरे के आसपास चलने दें. नजर स्थिर रहे तो रंगीन गेंदें फीकी लग सकती हैं और खाली जगह चलती हुई हरी आफ्टरइमेज जैसी दिख सकती है.",
  Letter: "अक्षर",
  "Letter color": "अक्षर का रंग",
  "Letter text size": "अक्षर का आकार",
  "Letter weight": "अक्षर की मोटाई",
  "Lilac Chaser": "लाइलैक चेज़र",
  "Lilac Chaser Fixation and Peripheral Awareness":
    "लाइलैक चेजर नजर टिकाना और किनारी जागरूकता",
  "Lilac Chaser ball color": "लाइलैक चेजर गेंद का रंग",
  "Lilac Chaser for fixation and peripheral awareness.":
    "नजर टिकाने और किनारी जागरूकता के लिए लाइलैक चेजर.",
  "Lilac Chaser is a fixation drill where you keep your gaze on a central cross while the outer pattern changes.":
    "लाइलैक चेजर नजर टिकाने का अभ्यास है. इसमें बाहरी पैटर्न बदलता रहता है और आपकी नजर बीच के क्रॉस पर रहती है.",
  "Lilac Chaser is a peripheral vision and focus drill. Keep your eyes on the center cross while one ball disappears at a time around a fixed circle. With steady fixation, many people perceive a moving green afterimage where the missing ball is.":
    "लाइलैक चेजर किनारी दृष्टि और फोकस का अभ्यास है. स्थिर गोले में एक-एक गेंद गायब होती है और आपकी नजर बीच के क्रॉस पर रहती है. नजर स्थिर रहे तो कई लोगों को गायब गेंद की जगह चलती हुई हरी आफ्टरइमेज दिखती है.",
  "Lilac Chaser is different from the moving-target drills. Instead of following an object, you keep your eyes on the center cross while the outer ring changes.":
    "लाइलैक चेजर चलते लक्ष्य वाले अभ्यासों से अलग है. किसी वस्तु को फॉलो करने के बजाय आप बाहरी रिंग बदलते समय नजर बीच के क्रॉस पर रखते हैं.",
  "Lilac Chaser is for fixation and peripheral awareness.":
    "लाइलैक चेजर नजर टिकाने और किनारी जागरूकता के लिए है.",
  "Lilac Chaser is the best choice when you want to hold your gaze on the center and notice change away from it.":
    "बीच पर नजर टिकाकर उससे दूर बदलाव देखना हो तो लाइलैक चेजर सबसे सही विकल्प है.",
  "Lilac Chaser scale": "लाइलैक चेजर स्केल",
  "Lilac Chaser trains fixation, peripheral awareness, visual attention, and noticing change away from the point you are looking at. For gaming, it can be a short warmup for catching movement near the edge of your vision without constantly shifting your gaze.":
    "लाइलैक चेजर नजर टिकाना, किनारी जागरूकता, विजुअल ध्यान और जिस बिंदु को आप देख रहे हैं उससे दूर बदलाव पहचानना सिखाता है. गेमिंग में यह नजर बार-बार हटाए बिना किनारों के पास हरकत पकड़ने का छोटा वॉर्मअप हो सकता है.",
  "Lilac chaser illusion": "लाइलैक चेजर भ्रम",
  Lissajous: "लिसाजू",
  "Lissajous Smooth Pursuit Drill": "लिसाजू स्मूद परसूट अभ्यास",
  "Lissajous Smooth Pursuit Eye Training": "लिसाजू स्मूद परसूट आंख अभ्यास",
  "Lissajous is one of the more complex flowing patterns because the target moves through a looping path that changes its relationship to the center over time.":
    "लिसाजू ज्यादा जटिल बहते पैटर्न में से एक है. लक्ष्य ऐसे लूप वाले रास्ते से चलता है जिसका केंद्र से रिश्ता समय के साथ बदलता है.",
  "Local settings with no account or install.":
    "बिना खाता या इंस्टॉल, स्थानीय सेटिंग.",
  "Look only at the black cross in the middle.": "सिर्फ बीच के काले क्रॉस को देखें.",
  "Lower the speed and increase target size so you have more time to settle on each jump.":
    "गति कम करें और लक्ष्य का आकार बढ़ाएं ताकि हर जंप पर नजर जमाने के लिए ज्यादा समय मिले.",
  "Lower the speed, increase target size, and keep the trail visible until you can stay on target comfortably.":
    "गति कम करें, लक्ष्य का आकार बढ़ाएं और जब तक लक्ष्य पर आराम से टिकना न हो जाए, ट्रेल दिखती रहने दें.",
  Magenta: "मैजेंटा",
  Medium: "मध्यम",
  "Mode guide": "मोड गाइड",
  "More pages": "और पेज",
  Motion: "गति",
  "Motion and target": "गति और लक्ष्य",
  "Motion feel": "गति का एहसास",
  "Motion path": "गति रास्ता",
  "Multiple Distractions": "कई व्यवधान",
  "Multiple Distractions for focus under visual clutter.":
    "दृश्य भीड़ में फोकस के लिए कई विचलन.",
  "Multiple Distractions for staying with a target through clutter.":
    "भीड़ के बीच लक्ष्य पर टिके रहने के लिए कई विचलन.",
  "Multiple Distractions includes distractor count and brightness controls. Letter overlays include letter color, weight, and scale controls.":
    "कई विचलन में विचलन संख्या और चमक के कंट्रोल हैं. अक्षर ओवरले में अक्षर रंग, मोटाई और स्केल के कंट्रोल हैं.",
  "Multiple Distractions is FoveaFlow's clutter drill. One target matters most, but other moving objects share the screen and try to pull your attention away.":
    "कई विचलन, FoveaFlow का दृश्य भीड़ अभ्यास है. एक लक्ष्य सबसे अहम होता है, लेकिन बाकी चलती वस्तुएं स्क्रीन पर रहती हैं और आपका ध्यान खींचती हैं.",
  "Multiple Distractions is focus training under visual clutter. Follow the brightest ball while darker balls move through the same space and compete for your attention.":
    "कई विचलन दृश्य भीड़ में फोकस अभ्यास है. सबसे चमकीली गेंद को फॉलो करें, जबकि गहरी गेंदें उसी जगह चलती हैं और आपका ध्यान बांटती हैं.",
  "Multiple Distractions is for staying with the right target when the screen is busy.":
    "स्क्रीन व्यस्त हो तब सही लक्ष्य पर टिके रहने के लिए कई विचलन है.",
  "Multiple Distractions is the best choice for practicing selective attention under visual clutter.":
    "दृश्य भीड़ में चयनात्मक ध्यान का अभ्यास करना हो तो कई विचलन सबसे अच्छा विकल्प है.",
  "Multiple Distractions trains selective attention, visual tracking under clutter, and target identity. The job is not just following motion. You also have to keep choosing the right object when similar objects compete for attention.":
    "कई विचलन चयनात्मक ध्यान, भीड़ में विजुअल ट्रैकिंग और लक्ष्य पहचान का अभ्यास कराता है. इसमें गति पर नज़र रखते हुए, मिलती-जुलती वस्तुओं के बीच सही लक्ष्य बार-बार चुनना होता है.",
  "Multiple object tracking": "कई वस्तुओं की ट्रैकिंग",
  "No account is needed for the simple browser patterns shown publicly. Steam is promoted for the upcoming app.":
    "सार्वजनिक रूप से दिखाए गए सरल ब्राउजर पैटर्न के लिए खाते की जरूरत नहीं. आने वाले ऐप के लिए Steam को दिखाया गया है.",
  "No. FoveaFlow is practice software and should not replace professional care.":
    "नहीं. FoveaFlow अभ्यास सॉफ्टवेयर है और पेशेवर देखभाल की जगह नहीं लेना चाहिए.",
  "No. FoveaFlow runs in the browser and stores settings locally.":
    "नहीं. FoveaFlow ब्राउजर में चलता है और सेटिंग स्थानीय रूप से सेव करता है.",
  "No. It is a browser-based practice drill and not medical treatment.":
    "नहीं. यह ब्राउजर आधारित अभ्यास है, चिकित्सा उपचार नहीं.",
  "No. It is designed for practice, not to promise eyesight improvement.":
    "नहीं. यह अभ्यास के लिए बना है, नजर सुधारने का वादा करने के लिए नहीं.",
  "No. It is practice software, not a treatment or diagnostic tool.":
    "नहीं. यह अभ्यास सॉफ्टवेयर है, उपचार या निदान टूल नहीं.",
  "No. Keep your eyes on the center cross and let the visual effect happen in the periphery.":
    "नहीं. नजर बीच के क्रॉस पर रखें और विजुअल असर किनारों में होने दें.",
  "No. The tool runs in a modern browser and stores settings locally in your browser.":
    "नहीं. यह टूल आधुनिक ब्राउजर में चलता है और सेटिंग आपके ब्राउजर में स्थानीय रूप से सेव करता है.",
  "Not medical care": "चिकित्सा देखभाल नहीं",
  "On this page": "इस पेज पर",
  Opacity: "अपारदर्शिता",
  Open: "खोलें",
  "Open FPS warmup": "FPS वॉर्मअप खोलें",
  "Open Smooth Pursuit": "स्मूद परसूट खोलें",
  "Open controls": "कंट्रोल खोलें",
  "Open guide": "गाइड खोलें",
  "Open the full guide": "पूरा गाइड खोलें",
  Overview: "ओवरव्यू",
  "Page navigation": "पेज नेविगेशन",
  "Path, direction changes, steady motion, speed waves, bursts, build/reset, and size pulse options are available depending on the drill.":
    "अभ्यास के हिसाब से रास्ता, दिशा बदलाव, स्थिर गति, स्पीड वेव, बर्स्ट, बनाना/रीसेट और आकार पल्स विकल्प मिलते हैं.",
  "Pattern pages start Smooth Pursuit with that path selected. Reaction jumps, Multiple Distractions, and Lilac Chaser have their own direct URLs.":
    "पैटर्न पेज स्मूद परसूट को उसी रास्ते के साथ शुरू करते हैं. रिएक्शन जंप्स, कई विचलन और लाइलैक चेजर के अपने सीधे URL हैं.",
  Pause: "रोकें",
  "Pause motion": "गति रोकें",
  "People on screens all day": "पूरे दिन स्क्रीन पर रहने वाले लोग",
  "Peripheral awareness": "किनारी जागरूकता",
  "Pick a drill and tune the target": "अभ्यास चुनें और लक्ष्य सेट करें",
  "Practice software, not medical care. Stop if you feel eye strain, dizziness, headache, nausea, or any other discomfort.":
    "यह अभ्यास सॉफ्टवेयर है, चिकित्सा देखभाल नहीं. आंखों में तनाव, चक्कर, सिरदर्द, मितली या कोई असुविधा हो तो रुकें.",
  "Practice the Bounce smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "बाउंस स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Circle smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "सर्कल स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Clover smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "क्लोवर स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Corner tour smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "कॉर्नर टूर स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Diagonal smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "तिरछा स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Diamond loop smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "डायमंड लूप स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Down-left sweep smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "नीचे-बाएं स्वीप स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Down-right sweep smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "नीचे-दाएं स्वीप स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Edge loop smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "किनारा लूप स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Ellipse smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "दीर्घवृत्त स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Figure eight smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "फिगर आठ स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Hard turns smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "हार्ड टर्न्स स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Horizontal sweep smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "क्षैतिज स्वीप स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Hourglass smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "आवरग्लास स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Lissajous smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "लिसाजू स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Random smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "रैंडम स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Stair steps smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "सीढ़ी स्टेप्स स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Vertical sweep smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "ऊर्ध्व स्वीप स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Wave smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "वेव स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  "Practice the Zigzag smooth pursuit pattern online. Adjust speed, target size, color, trail, and screen scale for short visual tracking sessions.":
    "जिगजैग स्मूद परसूट पैटर्न का ऑनलाइन अभ्यास करें. छोटे विजुअल ट्रैकिंग सेशन के लिए गति, लक्ष्य आकार, रंग, ट्रेल और स्क्रीन स्केल बदलें.",
  Predictive: "पूर्वानुमेय",
  Privacy: "गोपनीयता",
  "Privacy Policy": "गोपनीयता नीति",
  Question: "सवाल",
  "Quick answers": "जल्दी जवाब",
  "Quick refocus": "जल्दी दोबारा फोकस",
  "Raise the speed and reduce target size so you must find each new position faster.":
    "गति बढ़ाएं और लक्ष्य का आकार घटाएं ताकि आपको हर नई जगह जल्दी खोजनी पड़े.",
  "Raise the speed, reduce target size, or switch from an easy loop to a more demanding path.":
    "गति बढ़ाएं, लक्ष्य का आकार घटाएं या आसान लूप से ज्यादा कठिन रास्ते पर जाएं.",
  Random: "रैंडम",
  "Random Smooth Pursuit Drill": "रैंडम स्मूद परसूट अभ्यास",
  "Random Smooth Pursuit Eye Training": "रैंडम स्मूद परसूट आंख अभ्यास",
  "Random removes the comfort of a repeating loop. Because the target changes direction and position less predictably, the drill adds more target-search work than a simple circle or ellipse.":
    "रैंडम दोहराए जाने वाले लूप की आसानी हटाता है. लक्ष्य की दिशा और जगह का अंदाजा कम लगता है, इसलिए यह सरल सर्कल या दीर्घवृत्त से ज्यादा लक्ष्य खोजने का काम जोड़ता है.",
  "Reaction Jumps": "तेज़ फोकस शिफ्ट",
  "Reaction Jumps Eye Training": "रिएक्शन जंप्स आंख अभ्यास",
  "Reaction Jumps for quick refocus.": "जल्दी दोबारा फोकस के लिए रिएक्शन जंप्स.",
  "Reaction Jumps is best when you want to find a new target position quickly and lock on before the next move.":
    "नई लक्ष्य जगह जल्दी खोजकर अगले मूव से पहले उस पर टिकना हो तो रिएक्शन जंप्स सबसे अच्छा है.",
  "Reaction Jumps is for reaction time training, fast refocus, and target switching.":
    "रिएक्शन जंप्स प्रतिक्रिया समय अभ्यास, तेज दोबारा फोकस और लक्ष्य बदलने के लिए है.",
  "Reaction Jumps is the drill to use when moving smoothly is not the point. The target holds still, then jumps to a new location.":
    "जब सहज गति मुख्य बात न हो तो रिएक्शन जंप्स इस्तेमाल करें. लक्ष्य रुका रहता है, फिर नई जगह पर कूदता है.",
  "Reaction Jumps trains quick refocus. The target holds still, then jumps to a new spot so you can find it fast and focus before the next move.":
    "रिएक्शन जंप्स जल्दी दोबारा फोकस का अभ्यास कराता है. लक्ष्य रुका रहता है, फिर नई जगह पर जाता है ताकि आप उसे जल्दी खोजकर अगले मूव से पहले फोकस कर सकें.",
  "Reaction Jumps trains quick target acquisition and quick refocus from one target position to the next.":
    "रिएक्शन जंप्स एक लक्ष्य जगह से दूसरी तक जल्दी लक्ष्य पकड़ना और जल्दी दोबारा फोकस सिखाता है.",
  "Reaction Jumps trains quick target acquisition, saccadic eye movement, peripheral detection, and fast refocusing. It is useful when you want to react to a new visual target without moving your head first.":
    "रिएक्शन जंप्स जल्दी लक्ष्य पकड़ना, सैकैडिक आंख गति, किनारे पर पहचान और तेज दोबारा फोकस का अभ्यास कराता है. सिर पहले हिलाए बिना नए विजुअल लक्ष्य पर प्रतिक्रिया देनी हो तो यह उपयोगी है.",
  "Reaction Jumps works well as a short pre-game warmup or a fast visual reset between tasks.":
    "रिएक्शन जंप्स छोटे प्री-गेम वॉर्मअप या कामों के बीच तेज विजुअल रीसेट के रूप में अच्छा चलता है.",
  "Reaction jumps": "तेज़ फोकस शिफ्ट",
  "Reaction warm-up": "रिएक्शन वॉर्मअप",
  "Read full guide": "पूरा गाइड पढ़ें",
  "Read guide FAQ": "गाइड सवाल-जवाब पढ़ें",
  "Read source": "सोर्स पढ़ें",
  "Read the full guide": "पूरा गाइड पढ़ें",
  "Read the guide": "गाइड पढ़ें",
  Red: "लाल",
  References: "संदर्भ",
  Regular: "सामान्य",
  "Research and background reading": "रिसर्च और पृष्ठभूमि पढ़ाई",
  "Reset to defaults": "डिफॉल्ट पर रीसेट करें",
  "Restore the selected drill to its default behavior, visuals, calibration, and saved local settings.":
    "चुने हुए अभ्यास को उसके डिफॉल्ट व्यवहार, विजुअल, कैलिब्रेशन और सेव स्थानीय सेटिंग पर लौटाएं.",
  Resume: "फिर शुरू करें",
  "Resume motion": "गति फिर शुरू करें",
  "Reveal controls": "कंट्रोल दिखाएं",
  "Reverse motion direction": "गति की दिशा उलटें",
  Ring: "रिंग",
  "Role of peripheral vision in saccade planning":
    "सैकैड योजना में किनारी दृष्टि की भूमिका",
  "Saccadic reaction time factors": "सैकैडिक प्रतिक्रिया समय के कारक",
  Safety: "सुरक्षा",
  Scale: "स्केल",
  "Screen break": "स्क्रीन ब्रेक",
  "Screen reset": "स्क्रीन रीसेट",
  "Screen scale": "स्क्रीन स्केल",
  "Screen-work reset": "स्क्रीन काम रीसेट",
  "Selective attention": "चयनात्मक ध्यान",
  Semibold: "सेमीबोल्ड",
  Session: "सेशन",
  "Settings saved in your browser": "सेटिंग आपके ब्राउजर में सेव हैं",
  "Shape, color, opacity, trail length, and trail behavior are adjustable.":
    "आकार, रंग, अपारदर्शिता, ट्रेल लंबाई और ट्रेल व्यवहार बदले जा सकते हैं.",
  "Sharpen focus between code, logs, dashboards, terminals, tickets, and multi-monitor work.":
    "कोड, लॉग, डैशबोर्ड, टर्मिनल, टिकट और मल्टी-मॉनिटर काम के बीच फोकस तेज करें.",
  "Sharpen selective focus by tracking the brightest target through moving distractions.":
    "चलते विचलनों के बीच सबसे चमकीले लक्ष्य को ट्रैक करके चयनात्मक फोकस तेज करें.",
  "Sharpen your visual warmup before FPS games with tracking, refocus, peripheral awareness, and character movement reading drills.":
    "FPS गेम से पहले ट्रैकिंग, दोबारा फोकस, किनारी जागरूकता और कैरेक्टर मूवमेंट पढ़ने वाले अभ्यासों से अपना विजुअल वॉर्मअप तेज करें.",
  "Short bursts": "छोटे बर्स्ट",
  "Should I follow the outer shapes?": "क्या मुझे बाहरी आकारों को फॉलो करना चाहिए?",
  "Show target letters": "लक्ष्य अक्षर दिखाएं",
  "Show trail": "ट्रेल दिखाएं",
  Size: "आकार",
  "Size pulse": "आकार पल्स",
  "Smooth Pursuit": "स्मूद ट्रैकिंग",
  "Smooth Pursuit Eye Training": "स्मूद परसूट आंख अभ्यास",
  "Smooth Pursuit for one-target visual tracking.":
    "एक लक्ष्य की विजुअल ट्रैकिंग के लिए स्मूद परसूट.",
  "Smooth Pursuit helps train steady tracking, moving-target focus, and controlled eye movement across more of your usable range. Predictable paths build rhythm and control. Random paths and hard turns add more visual search and reaction demand.":
    "स्मूद परसूट स्थिर ट्रैकिंग, चलते लक्ष्य पर फोकस और आपके उपयोगी दायरे में नियंत्रित आंख गति का अभ्यास कराता है. अनुमानित रास्ते लय और कंट्रोल बनाते हैं. रैंडम रास्ते और हार्ड टर्न्स ज्यादा विजुअल खोज और प्रतिक्रिया मांग जोड़ते हैं.",
  "Smooth Pursuit is FoveaFlow's moving-target drill. Keep your head still, follow one target with your eyes, and stay smooth instead of jumping ahead.":
    "स्मूद परसूट, FoveaFlow का चलते लक्ष्य वाला अभ्यास है. सिर स्थिर रखें, एक लक्ष्य को आंखों से फॉलो करें और आगे कूदने के बजाय सहज रहें.",
  "Smooth Pursuit is a drill where you follow one moving target as steadily as you can with your eyes.":
    "स्मूद परसूट में आप एक चलते लक्ष्य को आंखों से जितना हो सके स्थिरता से फॉलो करते हैं.",
  "Smooth Pursuit is a moving-target eye training drill. Keep your head still and track the ball with your eyes. Predictable paths build steady control; random paths and hard turns add more target-search work.":
    "स्मूद परसूट चलते लक्ष्य का आंख अभ्यास है. सिर स्थिर रखें और गेंद को आंखों से ट्रैक करें. अनुमानित रास्ते स्थिर कंट्रोल बनाते हैं; रैंडम रास्ते और हार्ड टर्न्स ज्यादा लक्ष्य खोजने का काम जोड़ते हैं.",
  "Smooth Pursuit is for steady moving-target tracking.":
    "स्मूद परसूट स्थिर चलते लक्ष्य ट्रैकिंग के लिए है.",
  "Smooth Pursuit is the best starting point when your goal is following one moving target as steadily as possible.":
    "एक चलते लक्ष्य को जितना हो सके स्थिरता से फॉलो करना लक्ष्य हो तो स्मूद परसूट सबसे अच्छी शुरुआत है.",
  "Smooth Pursuit paths for moving-target tracking.":
    "चलते लक्ष्य ट्रैकिंग के लिए स्मूद परसूट रास्ते.",
  "Smooth Pursuit pattern": "स्मूद परसूट पैटर्न",
  "Smooth Pursuit pattern routes": "स्मूद परसूट पैटर्न रास्ते",
  "Smooth Pursuit, Reaction Jumps, Multiple Distractions, and Lilac Chaser are available as separate modes.":
    "स्मूद परसूट, रिएक्शन जंप्स, कई विचलन और लाइलैक चेजर अलग मोड के रूप में उपलब्ध हैं.",
  "Snap to the next target and train fast refocus.":
    "अगले लक्ष्य पर तुरंत जाएं और तेज दोबारा फोकस का अभ्यास करें.",
  Source: "सोर्स",
  "Source code": "सोर्स कोड",
  "Spatial allocation of attention during smooth pursuit":
    "स्मूद परसूट के दौरान ध्यान का स्थानिक वितरण",
  Speed: "गति",
  "Speed unit": "गति इकाई",
  "Speed units, target size, shape, color, opacity, trails, paths, motion behavior, distractors, letter overlays, viewing distance, screen scale, and mode-specific controls.":
    "गति इकाइयां, लक्ष्य आकार, आकार प्रकार, रंग, अपारदर्शिता, ट्रेल, रास्ते, गति व्यवहार, विचलन, अक्षर ओवरले, देखने की दूरी, स्क्रीन स्केल और मोड आधारित कंट्रोल.",
  "Speed wave": "स्पीड वेव",
  "Speed, size, shape, color, opacity, and trail change the feel of the moving drills. Lilac Chaser has its own ball color and scale controls.":
    "गति, आकार, आकार प्रकार, रंग, अपारदर्शिता और ट्रेल चलते अभ्यासों का एहसास बदलते हैं. लाइलैक चेजर के अपने गेंद रंग और स्केल कंट्रोल हैं.",
  "Speed, target size, and path matter most because they change difficulty quickly.":
    "गति, लक्ष्य आकार और रास्ता सबसे अहम हैं, क्योंकि वे कठिनाई जल्दी बदलते हैं.",
  Square: "वर्ग",
  "Stair Steps Smooth Pursuit Drill": "सीढ़ी स्टेप्स स्मूद परसूट अभ्यास",
  "Stair Steps Smooth Pursuit Eye Training": "सीढ़ी स्टेप्स स्मूद परसूट आंख अभ्यास",
  "Stair Steps creates a mechanical route with discrete directional segments that stays easier to predict than Random or Hard Turns.":
    "सीढ़ी स्टेप्स अलग-अलग दिशा खंडों वाला यांत्रिक रास्ता बनाता है. इसका अंदाजा रैंडम या हार्ड टर्न्स से आसान है.",
  "Stair steps": "सीढ़ी स्टेप्स",
  "Start Bounce": "बाउंस शुरू करें",
  "Start Circle": "सर्कल शुरू करें",
  "Start Clover": "क्लोवर शुरू करें",
  "Start Corner tour": "कॉर्नर टूर शुरू करें",
  "Start Diagonal": "तिरछा शुरू करें",
  "Start Diamond loop": "डायमंड लूप शुरू करें",
  "Start Distractor Tracking": "विचलन ट्रैकिंग शुरू करें",
  "Start Down-left sweep": "नीचे-बाएं स्वीप शुरू करें",
  "Start Down-right sweep": "नीचे-दाएं स्वीप शुरू करें",
  "Start Edge loop": "किनारा लूप शुरू करें",
  "Start Ellipse": "दीर्घवृत्त शुरू करें",
  "Start Figure eight": "फिगर आठ शुरू करें",
  "Start Hard turns": "हार्ड टर्न्स शुरू करें",
  "Start Horizontal sweep": "क्षैतिज स्वीप शुरू करें",
  "Start Hourglass": "आवरग्लास शुरू करें",
  "Start Lilac Chaser": "लाइलैक चेजर शुरू करें",
  "Start Lissajous": "लिसाजू शुरू करें",
  "Start Random": "रैंडम शुरू करें",
  "Start Reaction Jumps": "रिएक्शन जंप्स शुरू करें",
  "Start Smooth Pursuit": "स्मूद परसूट शुरू करें",
  "Start Stair steps": "सीढ़ी स्टेप्स शुरू करें",
  "Start Vertical sweep": "ऊर्ध्व स्वीप शुरू करें",
  "Start Wave": "वेव शुरू करें",
  "Start Zigzag": "जिगजैग शुरू करें",
  "Start the warmup": "वॉर्मअप शुरू करें",
  "Start with a comfortable speed and a medium target size, then raise difficulty only when you can stay on the target cleanly.":
    "आरामदायक गति और मध्यम लक्ष्य आकार से शुरू करें. लक्ष्य पर साफ टिक सकें तभी कठिनाई बढ़ाएं.",
  "Start with a predictable path such as Circle or Ellipse before moving to harder patterns.":
    "कठिन पैटर्न पर जाने से पहले सर्कल या दीर्घवृत्त जैसे अनुमानित रास्ते से शुरू करें.",
  "Start with fewer distractors, a slower speed, and a larger main target.":
    "कम विचलन, धीमी गति और बड़े मुख्य लक्ष्य से शुरू करें.",
  "Start with fewer distractors, then add more when you can keep the target cleanly.":
    "कम विचलनों से शुरू करें. लक्ष्य पर साफ टिक सकें तो और जोड़ें.",
  "Steady fixation": "स्थिर नजर",
  "Steady speed": "स्थिर गति",
  "Stop the session and rest. Do not push through discomfort.":
    "सेशन रोकें और आराम करें. असुविधा सहते न रहें.",
  "Target acquisition": "लक्ष्य पकड़ना",
  "Target appearance, trail display, distractor brightness, letter styling, viewing distance, and screen scale can be tuned.":
    "लक्ष्य का रूप, ट्रेल डिस्प्ले, विचलन चमक, अक्षर शैली, देखने की दूरी और स्क्रीन स्केल बदले जा सकते हैं.",
  "Target form": "आकार",
  "Target opacity": "लक्ष्य अपारदर्शिता",
  "Target size": "लक्ष्य आकार",
  Targets: "लक्ष्य",
  Terms: "शर्तें",
  "Terms of Use": "उपयोग की शर्तें",
  "Text size": "टेक्स्ट आकार",
  "The goal is steady fixation and better awareness of change away from the center of your gaze.":
    "लक्ष्य है स्थिर नजर और नजर के केंद्र से दूर बदलाव की बेहतर समझ.",
  "The path shape changes how predictable the movement feels and how often the target changes direction.":
    "रास्ते का आकार बदलता है कि गति कितनी अनुमानित लगेगी और लक्ष्य कितनी बार दिशा बदलेगा.",
  "The patterns are simple screen paths and timing drills. They are not a clinical program, and results will vary from person to person.":
    "ये पैटर्न सरल स्क्रीन रास्ते और टाइमिंग अभ्यास हैं. ये क्लिनिकल प्रोग्राम नहीं हैं और नतीजे व्यक्ति के हिसाब से बदलेंगे.",
  "The public browser page keeps controls simple: Show Grid, Darkmode, and pattern selection.":
    "सार्वजनिक ब्राउजर पेज कंट्रोल सरल रखता है: ग्रिड दिखाएं, डार्क मोड और पैटर्न चयन.",
  "The site is provided as is. It may change, break, or go offline. To the fullest extent allowed by law, FoveaFlow is provided without warranties of any kind.":
    "साइट जैसी है वैसी दी जाती है. यह बदल सकती है, टूट सकती है या ऑफलाइन हो सकती है. कानून में जितना अनुमति है, FoveaFlow किसी भी तरह की वारंटी के बिना दिया जाता है.",
  "The site runs on Cloudflare. Cloudflare may process request data such as IP address, user agent, requested URL, and timing data to deliver the site, protect it from abuse, and show basic traffic and performance metrics.":
    "साइट Cloudflare पर चलती है. साइट देने, दुरुपयोग से बचाने और बुनियादी ट्रैफिक व प्रदर्शन मीट्रिक दिखाने के लिए Cloudflare IP पता, यूजर एजेंट, मांगा गया URL और टाइमिंग डेटा जैसे रिक्वेस्ट डेटा प्रोसेस कर सकता है.",
  "The source code is public on GitHub under the license in the repository. These terms cover use of the hosted FoveaFlow site.":
    "सोर्स कोड GitHub पर रिपॉजिटरी की लाइसेंस शर्तों के तहत सार्वजनिक है. ये शर्तें होस्ट की गई FoveaFlow साइट के उपयोग पर लागू हैं.",
  "These pages cover the common search paths around eye trainer warmups and browser-based alternatives.":
    "ये पेज आई ट्रेनर वॉर्मअप और ब्राउजर आधारित विकल्पों से जुड़े आम खोज रास्तों को कवर करते हैं.",
  "These terms may be updated when the app or site changes. The date at the top shows the latest version.":
    "ऐप या साइट बदलने पर ये शर्तें अपडेट हो सकती हैं. ऊपर दी गई तारीख नया संस्करण दिखाती है.",
  "This mode is a strong fit when you want to practice selective attention and target identity under visual noise.":
    "विजुअल शोर में चयनात्मक ध्यान और लक्ष्य पहचान का अभ्यास करना हो तो यह मोड अच्छा फिट है.",
  "This mode is useful when you want faster target acquisition and cleaner refocusing. Start slower for accuracy, then raise the speed for a more demanding session.":
    "तेज लक्ष्य पकड़ना और साफ दोबारा फोकस चाहिए तो यह मोड उपयोगी है. सटीकता के लिए धीमे शुरू करें, फिर कठिन सेशन के लिए गति बढ़ाएं.",
  "This mode works best when you resist the urge to chase the disappearing gap. Keep your gaze centered, stay relaxed, and let the effect happen on its own.":
    "यह मोड तब सबसे अच्छा काम करता है जब आप गायब होती जगह के पीछे नजर नहीं दौड़ाते. नजर बीच में रखें, आराम से रहें और असर अपने आप होने दें.",
  "This page is specific to this free browser tool.":
    "यह पेज इसी मुफ्त ब्राउजर टूल के लिए है.",
  "Those settings stay in your browser unless your browser syncs, backs up, or exports its site data. You can remove them by clearing site data for foveaflow.com.":
    "ये सेटिंग आपके ब्राउजर में रहती हैं, जब तक आपका ब्राउजर साइट डेटा सिंक, बैकअप या एक्सपोर्ट न करे. foveaflow.com का साइट डेटा साफ करके आप इन्हें हटा सकते हैं.",
  "Track the ball as smoothly as you can instead of jumping ahead of it.":
    "गेंद से आगे कूदने के बजाय उसे जितना हो सके सहजता से ट्रैक करें.",
  "Train peripheral awareness by holding focus on the center cross.":
    "बीच के क्रॉस पर फोकस रखकर किनारी जागरूकता का अभ्यास करें.",
  "Train quick refocus by snapping your eyes to each new target position.":
    "हर नई लक्ष्य जगह पर नजर तुरंत ले जाकर जल्दी दोबारा फोकस का अभ्यास करें.",
  "Train smooth visual tracking by following one moving target.":
    "एक चलते लक्ष्य को फॉलो करके सहज विजुअल ट्रैकिंग का अभ्यास करें.",
  "Train visual tracking, quick refocus, peripheral awareness, and focus under distraction in your browser.":
    "अपने ब्राउजर में विजुअल ट्रैकिंग, जल्दी दोबारा फोकस, किनारी जागरूकता और विचलन के बीच फोकस का अभ्यास करें.",
  Triangle: "त्रिकोण",
  "Try FoveaFlow": "FoveaFlow आजमाएं",
  "Try Smooth Pursuit": "स्मूद परसूट आजमाएं",
  "Try Smooth Pursuit first": "पहले स्मूद परसूट आजमाएं",
  "Try another drill": "दूसरा अभ्यास आजमाएं",
  "Tune the session before you start": "शुरू करने से पहले सेशन सेट करें",
  Unit: "इकाई",
  Unpredictive: "अननुमेय",
  "Updated July 10, 2026": "10 जुलाई 2026 को अपडेट किया गया",
  "Updated July 10, 2026. FoveaFlow is practice software, not medical care. Stop if a session causes strain, dizziness, headache, nausea, or any other discomfort.":
    "10 जुलाई 2026 को अपडेट किया गया. FoveaFlow अभ्यास सॉफ्टवेयर है, चिकित्सा देखभाल नहीं. सेशन से तनाव, चक्कर, सिरदर्द, मितली या कोई असुविधा हो तो रुकें.",
  "Updated July 10, 2026. If the visual effect feels strange or uncomfortable, stop the session and rest.":
    "10 जुलाई 2026 को अपडेट किया गया. विजुअल असर अजीब या असहज लगे तो सेशन रोकें और आराम करें.",
  "Updated July 10, 2026. Keep sessions brief and controlled. If the drill causes strain or discomfort, stop.":
    "10 जुलाई 2026 को अपडेट किया गया. सेशन छोटे और नियंत्रित रखें. अभ्यास से तनाव या असुविधा हो तो रुकें.",
  "Updated July 10, 2026. Start with fewer distractors or a bigger target, and stop if the session becomes uncomfortable.":
    "10 जुलाई 2026 को अपडेट किया गया. कम विचलन या बड़े लक्ष्य से शुरू करें, और सेशन असहज हो तो रुकें.",
  "Updated July 10, 2026. This is a browser-based practice drill and not medical therapy.":
    "10 जुलाई 2026 को अपडेट किया गया. यह ब्राउजर आधारित अभ्यास है, चिकित्सा थेरेपी नहीं.",
  "Updated July 10, 2026. This is practice software, not medical care, so stop if the session feels uncomfortable.":
    "10 जुलाई 2026 को अपडेट किया गया. यह अभ्यास सॉफ्टवेयर है, चिकित्सा देखभाल नहीं, इसलिए सेशन असहज लगे तो रुकें.",
  "Use FoveaFlow as a free FPS eye training warmup for visual tracking, quick refocus, target switching, and focus under distraction.":
    "विजुअल ट्रैकिंग, जल्दी दोबारा फोकस, लक्ष्य बदलने और विचलन के बीच फोकस के लिए FoveaFlow को मुफ्त FPS आंख अभ्यास वॉर्मअप की तरह इस्तेमाल करें.",
  "Use FoveaFlow as a short FPS warmup, an active screen break, or a focused visual practice session.":
    "FoveaFlow को छोटा FPS वॉर्मअप, सक्रिय स्क्रीन ब्रेक या केंद्रित विजुअल अभ्यास सेशन की तरह इस्तेमाल करें.",
  "Use FoveaFlow as practice software, not medical care. Stop if a session causes eye strain, dizziness, headache, nausea, or discomfort.":
    "FoveaFlow को अभ्यास सॉफ्टवेयर की तरह इस्तेमाल करें, चिकित्सा देखभाल की तरह नहीं. सेशन से आंखों में तनाव, चक्कर, सिरदर्द, मितली या असुविधा हो तो रुकें.",
  "Use Lilac Chaser for a short fixation drill, a perceptual reset, or a quick change of pace between more active modes.":
    "छोटे नजर टिकाने वाले अभ्यास, धारणा रीसेट या ज्यादा सक्रिय मोड के बीच गति बदलने के लिए लाइलैक चेजर इस्तेमाल करें.",
  "Use Smooth Pursuit for short browser-based practice, a pre-game warmup, or a focused reset after dense screen work.":
    "छोटे ब्राउजर आधारित अभ्यास, प्री-गेम वॉर्मअप या भारी स्क्रीन काम के बाद केंद्रित रीसेट के लिए स्मूद परसूट इस्तेमाल करें.",
  "Use Smooth Pursuit to follow one moving target, Reaction Jumps to snap focus to new target positions, Multiple Distractions to track the right target through visual clutter, and Lilac Chaser to hold fixation while noticing peripheral change.":
    "एक चलते लक्ष्य को फॉलो करने के लिए स्मूद परसूट, नई लक्ष्य जगहों पर तुरंत फोकस करने के लिए रिएक्शन जंप्स, दृश्य भीड़ में सही लक्ष्य ट्रैक करने के लिए कई विचलन और किनारी बदलाव देखते हुए नजर टिकाने के लिए लाइलैक चेजर इस्तेमाल करें.",
  "Use dark theme": "डार्क थीम इस्तेमाल करें",
  "Use forward motion direction": "आगे वाली गति दिशा इस्तेमाल करें",
  "Use it as a quick visual warmup or active screen break, not as medical care.":
    "इसे तेज विजुअल वॉर्मअप या सक्रिय स्क्रीन ब्रेक की तरह इस्तेमाल करें, चिकित्सा देखभाल की तरह नहीं.",
  "Use it for short practice sessions. Stop if you feel eye strain, dizziness, headache, nausea, or discomfort.":
    "इसे छोटे अभ्यास सेशन के लिए इस्तेमाल करें. आंखों में तनाव, चक्कर, सिरदर्द, मितली या असुविधा हो तो रुकें.",
  "Use it when you want a busier, harder tracking task than Smooth Pursuit.":
    "स्मूद परसूट से ज्यादा व्यस्त और कठिन ट्रैकिंग चाहिए तो इसे इस्तेमाल करें.",
  "Use it when you want a fixation-focused drill rather than a moving-target tracking session.":
    "चलते लक्ष्य ट्रैकिंग सेशन के बजाय नजर टिकाने पर केंद्रित अभ्यास चाहिए तो इसे इस्तेमाल करें.",
  "Use it when you want steady moving-target practice without the extra clutter of other drills.":
    "दूसरे अभ्यासों की अतिरिक्त भीड़ के बिना स्थिर चलते लक्ष्य अभ्यास चाहिए तो इसे इस्तेमाल करें.",
  "Use predictable paths for steady tracking. Use random paths or hard turns when you want more target-search work.":
    "स्थिर ट्रैकिंग के लिए अनुमानित रास्ते इस्तेमाल करें. ज्यादा लक्ष्य खोजने का काम चाहिए तो रैंडम रास्ते या हार्ड टर्न्स इस्तेमाल करें.",
  "Use safely": "सुरक्षित इस्तेमाल करें",
  "Use slower speeds for clean refocusing. Raise the speed when you want a sharper reaction drill.":
    "साफ दोबारा फोकस के लिए धीमी गति इस्तेमाल करें. तेज प्रतिक्रिया अभ्यास चाहिए तो गति बढ़ाएं.",
  "Use the Bounce path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए बाउंस रास्ता इस्तेमाल करें.",
  "Use the Circle path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए सर्कल रास्ता इस्तेमाल करें.",
  "Use the Clover path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए क्लोवर रास्ता इस्तेमाल करें.",
  "Use the Corner tour path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए कॉर्नर टूर रास्ता इस्तेमाल करें.",
  "Use the Diagonal path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए तिरछा रास्ता इस्तेमाल करें.",
  "Use the Diamond loop path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए डायमंड लूप रास्ता इस्तेमाल करें.",
  "Use the Down-left sweep path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए नीचे-बाएं स्वीप रास्ता इस्तेमाल करें.",
  "Use the Down-right sweep path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए नीचे-दाएं स्वीप रास्ता इस्तेमाल करें.",
  "Use the Edge loop path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए किनारा लूप रास्ता इस्तेमाल करें.",
  "Use the Ellipse path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए दीर्घवृत्त रास्ता इस्तेमाल करें.",
  "Use the Figure eight path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए फिगर आठ रास्ता इस्तेमाल करें.",
  "Use the Hard turns path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए हार्ड टर्न्स रास्ता इस्तेमाल करें.",
  "Use the Horizontal sweep path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए क्षैतिज स्वीप रास्ता इस्तेमाल करें.",
  "Use the Hourglass path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए आवरग्लास रास्ता इस्तेमाल करें.",
  "Use the Lissajous path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए लिसाजू रास्ता इस्तेमाल करें.",
  "Use the Random path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए रैंडम रास्ता इस्तेमाल करें.",
  "Use the Stair steps path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए सीढ़ी स्टेप्स रास्ता इस्तेमाल करें.",
  "Use the Vertical sweep path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए ऊर्ध्व स्वीप रास्ता इस्तेमाल करें.",
  "Use the Wave path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए वेव रास्ता इस्तेमाल करें.",
  "Use the Zigzag path for short smooth pursuit practice.":
    "छोटे स्मूद परसूट अभ्यास के लिए जिगजैग रास्ता इस्तेमाल करें.",
  "Use the app in a safe place where you can stop easily. Do not use it while driving, walking around, operating equipment, or doing anything that needs your full attention.":
    "ऐप ऐसी सुरक्षित जगह इस्तेमाल करें जहां आप आसानी से रुक सकें. ड्राइविंग, चलते समय, उपकरण चलाते समय या ऐसा काम करते समय इस्तेमाल न करें जिसमें पूरा ध्यान चाहिए.",
  "Use this drill when Smooth Pursuit feels too clean and you want a more realistic visual-attention challenge.":
    "स्मूद परसूट बहुत साफ लगे और ज्यादा वास्तविक विजुअल ध्यान चुनौती चाहिए तो यह अभ्यास इस्तेमाल करें.",
  "Use this guide to choose the right FoveaFlow eye trainer drill for visual tracking, quick refocus, peripheral awareness, FPS warmups, or focus under distraction.":
    "विजुअल ट्रैकिंग, जल्दी दोबारा फोकस, किनारी जागरूकता, FPS वॉर्मअप या विचलन के बीच फोकस के लिए सही FoveaFlow आई ट्रेनर अभ्यास चुनने में इस गाइड का इस्तेमाल करें.",
  "Vertical Sweep Smooth Pursuit Drill": "ऊर्ध्व स्वीप स्मूद परसूट अभ्यास",
  "Vertical Sweep Smooth Pursuit Eye Training": "ऊर्ध्व स्वीप स्मूद परसूट आंख अभ्यास",
  "Vertical Sweep mirrors the simplicity of Horizontal Sweep but changes the direction of travel for straightforward up-down tracking.":
    "ऊर्ध्व स्वीप, क्षैतिज स्वीप की सरलता जैसा है, लेकिन सीधे ऊपर-नीचे ट्रैकिंग के लिए चलने की दिशा बदलता है.",
  "Vertical sweep": "ऊर्ध्व स्वीप",
  "Viewing distance": "देखने की दूरी",
  "Viewing distance and CSS pixels/cm help speed settings match your display setup more closely.":
    "देखने की दूरी और CSS पिक्सल/cm गति सेटिंग को आपके डिस्प्ले सेटअप से बेहतर मिलाने में मदद करते हैं.",
  "Viewing distance and screen scale controls help match motion to your setup.":
    "देखने की दूरी और स्क्रीन स्केल कंट्रोल गति को आपके सेटअप से मिलाने में मदद करते हैं.",
  "Visit BlinkCamp": "BlinkCamp पर जाएं",
  "Visit EyeTrainer.gg": "EyeTrainer.gg पर जाएं",
  "Visual clutter": "दृश्य भीड़",
  "Visual guidance of smooth pursuit eye movements":
    "स्मूद परसूट आंख गतियों का विजुअल मार्गदर्शन",
  "Visual learning in multiple-object tracking":
    "कई वस्तुओं की ट्रैकिंग में विजुअल सीखना",
  "Visual practice": "विजुअल अभ्यास",
  "Visual tracking": "विजुअल ट्रैकिंग",
  Wave: "वेव",
  "Wave Smooth Pursuit Drill": "वेव स्मूद परसूट अभ्यास",
  "Wave Smooth Pursuit Eye Training": "वेव स्मूद परसूट आंख अभ्यास",
  "Wave introduces a repeating up-and-down rhythm on top of horizontal movement. It stays readable and smooth without the abrupt feel of hard corners.":
    "वेव क्षैतिज गति के साथ दोहराई जाने वाली ऊपर-नीचे लय जोड़ता है. यह हार्ड कोनों जैसी अचानक अनुभूति के बिना साफ और सहज रहता है.",
  Weight: "मोटाई",
  "What FoveaFlow includes": "FoveaFlow में क्या शामिल है",
  "What does Multiple Distractions train?": "कई विचलन किसका अभ्यास कराता है?",
  "What does Reaction Jumps train?": "रिएक्शन जंप्स किसका अभ्यास कराता है?",
  "What if the effect feels strange or uncomfortable?":
    "असर अजीब या असहज लगे तो क्या करें?",
  "What is FoveaFlow?": "FoveaFlow क्या है?",
  "What is Lilac Chaser mode?": "लाइलैक चेजर मोड क्या है?",
  "What is Lilac Chaser?": "लाइलैक चेजर क्या है?",
  "What is Multiple Distractions mode?": "कई विचलन मोड क्या है?",
  "What is Reaction Jumps mode?": "रिएक्शन जंप्स मोड क्या है?",
  "What is Smooth Pursuit mode?": "स्मूद परसूट मोड क्या है?",
  "What is Smooth Pursuit?": "स्मूद परसूट क्या है?",
  "What is distractor tracking?": "विचलन ट्रैकिंग क्या है?",
  "What is the Bounce drill?": "बाउंस अभ्यास क्या है?",
  "What is the Circle drill?": "सर्कल अभ्यास क्या है?",
  "What is the Clover drill?": "क्लोवर अभ्यास क्या है?",
  "What is the Corner tour drill?": "कॉर्नर टूर अभ्यास क्या है?",
  "What is the Diagonal drill?": "तिरछा अभ्यास क्या है?",
  "What is the Diamond loop drill?": "डायमंड लूप अभ्यास क्या है?",
  "What is the Down-left sweep drill?": "नीचे-बाएं स्वीप अभ्यास क्या है?",
  "What is the Down-right sweep drill?": "नीचे-दाएं स्वीप अभ्यास क्या है?",
  "What is the Edge loop drill?": "किनारा लूप अभ्यास क्या है?",
  "What is the Ellipse drill?": "दीर्घवृत्त अभ्यास क्या है?",
  "What is the Figure eight drill?": "फिगर आठ अभ्यास क्या है?",
  "What is the Hard turns drill?": "हार्ड टर्न्स अभ्यास क्या है?",
  "What is the Horizontal sweep drill?": "क्षैतिज स्वीप अभ्यास क्या है?",
  "What is the Hourglass drill?": "आवरग्लास अभ्यास क्या है?",
  "What is the Lissajous drill?": "लिसाजू अभ्यास क्या है?",
  "What is the Random drill?": "रैंडम अभ्यास क्या है?",
  "What is the Stair steps drill?": "सीढ़ी स्टेप्स अभ्यास क्या है?",
  "What is the Vertical sweep drill?": "ऊर्ध्व स्वीप अभ्यास क्या है?",
  "What is the Wave drill?": "वेव अभ्यास क्या है?",
  "What is the Zigzag drill?": "जिगजैग अभ्यास क्या है?",
  "What is the goal of this mode?": "इस मोड का लक्ष्य क्या है?",
  "What it trains:": "यह क्या अभ्यास कराता है:",
  "What makes the Bounce path different?": "बाउंस रास्ता अलग कैसे है?",
  "What makes the Circle path different?": "सर्कल रास्ता अलग कैसे है?",
  "What makes the Clover path different?": "क्लोवर रास्ता अलग कैसे है?",
  "What makes the Corner tour path different?": "कॉर्नर टूर रास्ता अलग कैसे है?",
  "What makes the Diagonal path different?": "तिरछा रास्ता अलग कैसे है?",
  "What makes the Diamond loop path different?": "डायमंड लूप रास्ता अलग कैसे है?",
  "What makes the Down-left sweep path different?":
    "नीचे-बाएं स्वीप रास्ता अलग कैसे है?",
  "What makes the Down-right sweep path different?":
    "नीचे-दाएं स्वीप रास्ता अलग कैसे है?",
  "What makes the Edge loop path different?": "किनारा लूप रास्ता अलग कैसे है?",
  "What makes the Ellipse path different?": "दीर्घवृत्त रास्ता अलग कैसे है?",
  "What makes the Figure eight path different?": "फिगर आठ रास्ता अलग कैसे है?",
  "What makes the Hard turns path different?": "हार्ड टर्न्स रास्ता अलग कैसे है?",
  "What makes the Horizontal sweep path different?":
    "क्षैतिज स्वीप रास्ता अलग कैसे है?",
  "What makes the Hourglass path different?": "आवरग्लास रास्ता अलग कैसे है?",
  "What makes the Lissajous path different?": "लिसाजू रास्ता अलग कैसे है?",
  "What makes the Random path different?": "रैंडम रास्ता अलग कैसे है?",
  "What makes the Stair steps path different?": "सीढ़ी स्टेप्स रास्ता अलग कैसे है?",
  "What makes the Vertical sweep path different?": "ऊर्ध्व स्वीप रास्ता अलग कैसे है?",
  "What makes the Wave path different?": "वेव रास्ता अलग कैसे है?",
  "What makes the Zigzag path different?": "जिगजैग रास्ता अलग कैसे है?",
  "What settings can I change?": "मैं कौन सी सेटिंग बदल सकता हूं?",
  "What the app is": "ऐप क्या है",
  "When it jumps, find the new location and actually focus on it before the next jump.":
    "जब यह जंप करे, तो अगली जंप से पहले नई जगह खोजें और सच में उस पर फोकस करें.",
  "When should I choose this over Smooth Pursuit?":
    "स्मूद परसूट के बजाय इसे कब चुनना चाहिए?",
  "When should I use Smooth Pursuit?": "स्मूद परसूट कब इस्तेमाल करना चाहिए?",
  "When should I use this mode?": "यह मोड कब इस्तेमाल करना चाहिए?",
  "Which drill is best for fixation and edge-of-vision awareness?":
    "नजर टिकाने और दृष्टि के किनारे की जागरूकता के लिए कौन सा अभ्यास सबसे अच्छा है?",
  "Which drill is best for quick refocus?":
    "जल्दी दोबारा फोकस के लिए कौन सा अभ्यास सबसे अच्छा है?",
  "Which drill is best for steady tracking?":
    "स्थिर ट्रैकिंग के लिए कौन सा अभ्यास सबसे अच्छा है?",
  "Which drill is best when the screen feels busy?":
    "स्क्रीन व्यस्त लगे तो कौन सा अभ्यास सबसे अच्छा है?",
  "Which drill set is broader?": "किसका अभ्यास सेट ज्यादा व्यापक है?",
  "Which drill to use": "कौन सा अभ्यास इस्तेमाल करें",
  "Which drills are available?": "कौन से अभ्यास उपलब्ध हैं?",
  "Which path is best for beginners?":
    "शुरुआती लोगों के लिए कौन सा रास्ता सबसे अच्छा है?",
  "Which settings matter most?": "कौन सी सेटिंग सबसे अहम हैं?",
  "Which settings should I change first?": "मुझे पहले कौन सी सेटिंग बदलनी चाहिए?",
  Yes: "हां",
  "Yes, but a larger screen gives the moving target more room. A desktop, laptop, or tablet usually feels better for longer paths.":
    "हां, लेकिन बड़ी स्क्रीन चलते लक्ष्य को ज्यादा जगह देती है. लंबे रास्तों के लिए डेस्कटॉप, लैपटॉप या टैबलेट आम तौर पर बेहतर लगता है.",
  "Yes. BlinkCamp also links to GitHub.": "हां. BlinkCamp भी GitHub लिंक देता है.",
  "Yes. BlinkCamp exposes simple speed and size controls.":
    "हां. BlinkCamp सरल गति और आकार कंट्रोल देता है.",
  "Yes. BlinkCamp is also a free browser-based eye training tool.":
    "हां. BlinkCamp भी मुफ्त ब्राउजर आधारित आंख अभ्यास टूल है.",
  "Yes. EyeTrainer.gg is also positioned around FPS eye training.":
    "हां. EyeTrainer.gg भी FPS आंख अभ्यास के आसपास केंद्रित है.",
  "Yes. FoveaFlow includes FPS warmup-friendly drills for tracking, refocus, and focus under clutter.":
    "हां. FoveaFlow में ट्रैकिंग, दोबारा फोकस और दृश्य भीड़ में फोकस के लिए FPS वॉर्मअप अभ्यास हैं.",
  "Yes. FoveaFlow is free to use, with no account, subscription, or paid plan.":
    "हां. FoveaFlow मुफ्त है. कोई खाता, सब्सक्रिप्शन या पेड प्लान नहीं.",
  "Yes. It can be a short warmup for games that reward fast visual reactions.":
    "हां. तेज विजुअल प्रतिक्रिया काम आने वाले गेम के लिए यह छोटा वॉर्मअप हो सकता है.",
  "Yes. It can be useful as a short warmup for games where visual clutter matters.":
    "हां. जिन गेम में दृश्य भीड़ मायने रखती है, उनके लिए यह छोटा वॉर्मअप उपयोगी हो सकता है.",
  "Yes. It gives developers, sysadmins, and support teams a short visual reset between code, logs, dashboards, terminals, tickets, and multiple windows.":
    "हां. यह डेवलपर, सिस्टम एडमिन और सपोर्ट टीमों को कोड, लॉग, डैशबोर्ड, टर्मिनल, टिकट और कई विंडो के बीच छोटा विजुअल रीसेट देता है.",
  "Yes. It works well as a short visual warmup before games or demanding screen tasks.":
    "हां. गेम या कठिन स्क्रीन काम से पहले यह छोटा विजुअल वॉर्मअप अच्छा रहता है.",
  "Yes. Main modes and Smooth Pursuit patterns have direct URLs.":
    "हां. मुख्य मोड और स्मूद परसूट पैटर्न के सीधे URL हैं.",
  "Yes. Speed can be tuned in deg/s, cm/s, or screen/s, and target size can be changed per session.":
    "हां. गति को deg/s, cm/s या screen/s में सेट किया जा सकता है, और लक्ष्य आकार हर सेशन में बदला जा सकता है.",
  "Yes. The GitHub repository is linked from the app.":
    "हां. ऐप से GitHub रिपॉजिटरी लिंक है.",
  "Yes. The app runs in the browser with no account or install.":
    "हां. ऐप बिना खाता या इंस्टॉल ब्राउजर में चलता है.",
  "Yes. The public page includes simple browser patterns and also promotes a Steam wishlist.":
    "हां. सार्वजनिक पेज में सरल ब्राउजर पैटर्न हैं और Steam wishlist भी दिखाई गई है.",
  "Yes. Use FoveaFlow as a quick visual warmup before FPS games or any game where tracking targets and reading movement matters.":
    "हां. FPS गेम या ऐसे किसी भी गेम से पहले FoveaFlow को तेज विजुअल वॉर्मअप की तरह इस्तेमाल करें जहां लक्ष्य ट्रैक करना और मूवमेंट पढ़ना मायने रखता है.",
  "You can adjust the mode, motion path, target size, speed, shape, color, opacity, trail, distractor count, viewing distance, screen scale, and Lilac Chaser size and color.":
    "आप मोड, गति रास्ता, लक्ष्य आकार, गति, आकार प्रकार, रंग, अपारदर्शिता, ट्रेल, विचलन संख्या, देखने की दूरी, स्क्रीन स्केल और लाइलैक चेजर का आकार व रंग बदल सकते हैं.",
  "You can clear saved FoveaFlow settings from your browser's site data controls. You can also use browser or extension settings to block optional analytics scripts.":
    "आप ब्राउजर के साइट डेटा कंट्रोल से सेव FoveaFlow सेटिंग साफ कर सकते हैं. वैकल्पिक एनालिटिक्स स्क्रिप्ट रोकने के लिए ब्राउजर या एक्सटेंशन सेटिंग भी इस्तेमाल कर सकते हैं.",
  "You choose the settings and session length. Keep sessions short if you are unsure, and take breaks.":
    "सेटिंग और सेशन की लंबाई आप चुनते हैं. पक्का न हो तो सेशन छोटा रखें और ब्रेक लें.",
  "You do not need to create an account to use FoveaFlow. The app does not ask for your name, email address, payment details, or health records.":
    "FoveaFlow इस्तेमाल करने के लिए खाता बनाने की जरूरत नहीं. ऐप आपका नाम, ईमेल पता, भुगतान जानकारी या स्वास्थ्य रिकॉर्ड नहीं मांगता.",
  "Your choices": "आपकी पसंद",
  "Your practice choices are not uploaded to a FoveaFlow account because there are no accounts.":
    "आपकी अभ्यास पसंद किसी FoveaFlow खाते पर अपलोड नहीं होती, क्योंकि खाते हैं ही नहीं.",
  Zigzag: "जिगजैग",
  "Zigzag Smooth Pursuit Drill": "जिगजैग स्मूद परसूट अभ्यास",
  "Zigzag Smooth Pursuit Eye Training": "जिगजैग स्मूद परसूट आंख अभ्यास",
  "Zigzag adds frequent directional switching, so it feels more aggressive than Wave or Diagonal.":
    "जिगजैग में दिशा बार-बार बदलती है, इसलिए यह वेव या तिरछा से ज्यादा आक्रामक लगता है.",
  constant: "स्थिर",
  forward: "आगे",
  guide: "गाइड",
  overview: "ओवरव्यू",
  paused: "रुका हुआ",
  playing: "चल रहा है",
  reverse: "उल्टा",
} satisfies Record<keyof typeof en, string>;
